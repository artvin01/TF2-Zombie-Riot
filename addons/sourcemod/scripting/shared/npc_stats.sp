...
void OnMapStart_NPC_Base()
{
    ...
    {
        PrecacheSound(g_GibSound[i]);
    }
    for (int i = 0; i < (sizeof(g_GibSoundMetal)); i++)
    {
        PrecacheSound(g_GibSoundMetal[i]);
    }
    for (int i = 0; i < (sizeof(g_CombineSoldierStepSound)); i++)
    {
        PrecacheSound(g_CombineSoldierStepSound[i]);
    }
    for (int i = 0; i < (sizeof(g_DefaultStepSound)); i++)
    {
        PrecacheSound(g_DefaultStepSound[i]);
    }
    for (int i = 0; i < (sizeof(g_CombineMetroStepSound)); i++)
    {
        PrecacheSound(g_CombineMetroStepSound[i]);
    }
    for (int i = 0; i < (sizeof(g_ArrowHitSoundSuccess)); i++)
    {
        PrecacheSound(g_ArrowHitSoundSuccess[i]);
    }
    for (int i = 0; i < (sizeof(g_ArrowHitSoundMiss)); i++)
    {
        PrecacheSound(g_ArrowHitSoundMiss[i]);
    }
    for (int i = 0; i < (sizeof(g_PanzerStepSound)); i++)
    {
        PrecacheSound(g_PanzerStepSound[i]);
    }
    for (int i = 0; i < (sizeof(g_HurtArmorSounds)); i++)
    {
        PrecacheSound(g_HurtArmorSounds[i]);
    }
#if defined ZR
    for (int i = 0; i < (sizeof(g_TankStepSound)); i++)
    {
        PrecacheSoundCustom(g_TankStepSound[i]);
    }
#endif
    for (int i = 0; i < (sizeof(g_RobotStepSound)); i++)
    {
        PrecacheSound(g_RobotStepSound[i]);
    }
#if defined BONEZONE_BASE
...
void Npc_DoGibLogic(int pThis, float GibAmount = 1.0)
{
    ...
    for (int GibLoop; GibLoop < 3; GibLoop++)
    {
        ...
        //Skeleton gibs should be the same color as the skeleton they came from:
        if (npc.m_iBleedType == BLEEDTYPE_SKELETON)
            SetEntProp(prop, Prop_Send, "m_nSkin", GetEntProp(npc.index, Prop_Send, "m_nSkin"));
    ...
    */
    if (customlogic == 1 || customlogic == 2 || flDistanceToTarget < ((extra_backoff * extra_backoff)) / 2.0)
    {
        ...
        if (Direction == 1)
        {
            ...
            float vecSwingStart_2[3];
            vecSwingStart_2 = botPos;
