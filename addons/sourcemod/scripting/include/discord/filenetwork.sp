...
#define TIMEOUT_PERIOD	10	// How long until we consider the file to be timed out
...
int StartedSendingAt[MAXPLAYERS+1];
...
public Action Timer_SendingClient(Handle timer, int client)
{
	...
	if(CurrentlySending[client][0])
	{
		...
		{
			if((StartedSendingAt[client] + TIMEOUT_PERIOD) > GetTime())
				return Plugin_Continue;
			
			// We timed out, resend this file
		}
		else
		{
	...
	for(int i; i < length; i++)
	{
		...
		if(info.Client == client)
		{
			if(chan.SendFile(info.Filename))
			{
				...
				StartedSendingAt[client] = GetTime();
			}
			else
			{
