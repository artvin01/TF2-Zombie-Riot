...
{
	// +0.05% every X
	WeaponMulti(map, "2", float(amount) * 0.0005);
	WeaponMulti(map, "410", float(amount) * 0.0005);
}

public void SkillWeapon_DamageUp_AllHigh(int entity, StringMap map, int amount, int client)
{
	// +0.2% every X
	WeaponMulti(map, "2", float(amount) * 0.002);
	WeaponMulti(map, "410", float(amount) * 0.002);
}
public void SkillWeapon_DamageUp_Kahmlstein(int entity, StringMap map, int amount, int client)
{
	// +1% every X
	WeaponMulti(map, "2", float(amount) * 0.01);
	WeaponMulti(map, "410", float(amount) * 0.01);
}
public void SkillWeapon_TwirlHairpin(int entity, StringMap map, int amount, int client)
{
	// +1% every X
	WeaponMulti(map, "4019", float(amount) * 0.005);
	WeaponMulti(map, "4020", float(amount) * 0.005);
}
...
{
	// +0.2% every X
	WeaponMulti(map, "6", float(amount) * 0.002);
	WeaponMulti(map, "97", float(amount) * 0.002);
}
...
public void SkillWeapon_RepairSpeed(int entity, StringMap map, int amount, int client)
{
	...
	WeaponMulti(map, "95", float(amount) * 0.01);
}