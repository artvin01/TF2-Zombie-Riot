...
enum
{
	...
	BONEZONE_BUCCANEER				= 375,
	BONEZONE_BUFFED_BUCCANEER 		= 376,
	BONEZONE_SUPREME				= 377,
...
any Npc_Create(int Index_Of_Npc, int client, float vecPos[3], float vecAng[3], bool ally, const char[] data="") //dmg mult only used for summonings
{
	...
	switch(Index_Of_Npc)
	{
		...
		case BONEZONE_ARCHMAGE:
			entity = ArchmageBones(client, vecPos, vecAng, ally, false);
			
		case BONEZONE_BUFFED_ARCHMAGE:
			entity = ArchmageBones(client, vecPos, vecAng, ally, true);
			
		case BONEZONE_SAINTBONES:
			entity = SaintBones(client, vecPos, vecAng, ally, false);
		...
		case BONEZONE_BUCCANEER:
			entity = BuccaneerBones(client, vecPos, vecAng, ally, false);
			
		case BONEZONE_BUFFED_BUCCANEER:
			entity = BuccaneerBones(client, vecPos, vecAng, ally, true);

		case BONEZONE_SUPREME:
			entity = SupremeSpookmasterBones(client, vecPos, vecAng, ally);
...
public void NPCDeath(int entity)
{
	...
	switch(i_NpcInternalId[entity])
	{
		...
		case BONEZONE_BUCCANEER:
			BuccaneerBones_NPCDeath(entity);
			
		case BONEZONE_BUFFED_BUCCANEER:
			BuccaneerBones_NPCDeath(entity);

		case BONEZONE_SUPREME:
			SupremeSpookmasterBones_NPCDeath(entity);
...
Action NpcSpecificOnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	//		MedivalRam_OnTakeDamage(victim, attacker, inflictor, damage, damagetype, weapon, damageForce, damagePosition, damagecustom);
		...
		case BONEZONE_ARCHMAGE:
			ArchmageBones_OnTakeDamage(victim, attacker, inflictor, damage, damagetype, weapon, damageForce, damagePosition, damagecustom);
		...
		case BONEZONE_BUCCANEER:
			BuccaneerBones_OnTakeDamage(victim, attacker, inflictor, damage, damagetype, weapon, damageForce, damagePosition, damagecustom);
			
		case BONEZONE_BUFFED_BUCCANEER:
			BuccaneerBones_OnTakeDamage(victim, attacker, inflictor, damage, damagetype, weapon, damageForce, damagePosition, damagecustom);

		case BONEZONE_SUPREME:
			SupremeSpookmasterBones_OnTakeDamage(victim, attacker, inflictor, damage, damagetype, weapon, damageForce, damagePosition, damagecustom);
