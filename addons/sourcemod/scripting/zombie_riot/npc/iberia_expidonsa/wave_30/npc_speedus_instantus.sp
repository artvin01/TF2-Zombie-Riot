...
void IberiaSpeedusInstantus_OnMapStart_NPC()
{
...
methodmap SpeedusInstantus < CClotBody
{
	...
	public SpeedusInstantus(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = SpeedusInstantus_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = SpeedusInstantus_OnTakeDamage;
		func_NPCThink[npc.index] = SpeedusInstantus_ClotThink;
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/scout/hwn2019_fuel_injector/hwn2019_fuel_injector.mdl");
		SetVariantString("1.0");
...
public void SpeedusInstantus_ClotThink(int iNPC)
{
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		SpeedusInstantusSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
		npc.m_flGetClosestTargetTime = 0.0;
...
void SpeedusInstantusSelfDefense(SpeedusInstantus npc, float gameTime, int target, float distance)
{
...
public Action SpeedusInstantus_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	SpeedusInstantus npc = view_as<SpeedusInstantus>(victim);
...
public void SpeedusInstantus_NPCDeath(int entity)
{
	SpeedusInstantus npc = view_as<SpeedusInstantus>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
