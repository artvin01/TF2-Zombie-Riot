...
void AgentChase_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Chase");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_chase");
	strcopy(data.Icon, sizeof(data.Icon), "matrix_sniper_agility");
...
methodmap AgentChase < CClotBody
{
	...
	public AgentChase(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentChase_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentChase_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentChase_ClotThink);
		...
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/sniper/sum23_preventative_measure/sum23_preventative_measure.mdl");

		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/sniper/sum23_cranium_cover_style1/sum23_cranium_cover_style1.mdl");
...
public void AgentChase_ClotThink(int iNPC)
{
	AgentChase npc = view_as<AgentChase>(iNPC);
	...
	npc.m_flSpeed = (25.0 * TimeMultiplier);
...
public Action AgentChase_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentChase npc = view_as<AgentChase>(victim);
...
public void AgentChase_NPCDeath(int entity)
{
	AgentChase npc = view_as<AgentChase>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
