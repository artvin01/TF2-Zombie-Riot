...
void AgentJames_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent James");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_james");
...
{
	return AgentJames(client, vecPos, vecAng, ally);
}
methodmap AgentJames < CClotBody
{
	...
	public AgentJames(int client, float vecPos[3], float vecAng[3], int ally)
	{
		AgentJames npc = view_as<AgentJames>(CClotBody(vecPos, vecAng, "models/player/spy.mdl", "1.0", "700", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentJames_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentJames_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentJames_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/spy/spr17_the_upgrade/spr17_the_upgrade.mdl");
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/player/items/spy/spy_openjacket.mdl");
...
public void AgentJames_ClotThink(int iNPC)
{
	AgentJames npc = view_as<AgentJames>(iNPC);
...
public Action AgentJames_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentJames npc = view_as<AgentJames>(victim);
...
public void AgentJames_NPCDeath(int entity)
{
	AgentJames npc = view_as<AgentJames>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
