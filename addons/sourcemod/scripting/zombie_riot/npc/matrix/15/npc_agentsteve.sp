...
void AgentSteve_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Steve");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_steve");
...
methodmap AgentSteve < CClotBody
{
	...
	public AgentSteve(int client, float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentSteve_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentSteve_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentSteve_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/soldier/sum23_close_quarters_style3/sum23_close_quarters_style3.mdl");
		
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/all_class/jul13_sweet_shades_s1/jul13_sweet_shades_s1_soldier.mdl");

		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/soldier/dec23_trench_warefarer/dec23_trench_warefarer.mdl");
...
public void AgentSteve_ClotThink(int iNPC)
{
	AgentSteve npc = view_as<AgentSteve>(iNPC);
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
		...
		if(npc.m_flNextRangedAttack < GetGameTime(npc.index) && flDistanceToTarget > 62500 && flDistanceToTarget < 122500 && npc.m_flReloadDelay < GetGameTime(npc.index))
		{
			...
			else
			{
				...
				FireBullet(npc.index, npc.m_iWearable1, WorldSpaceVec, vecDir, EscapeModeForNpc ? 5.0 : 5.0, 9000.0, DMG_BULLET, "bullet_tracer01_red");
...
public Action AgentSteve_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentSteve npc = view_as<AgentSteve>(victim);
...
public void AgentSteve_NPCDeath(int entity)
{
	AgentSteve npc = view_as<AgentSteve>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
