...
void AgentGraham_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Graham");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_graham");
	strcopy(data.Icon, sizeof(data.Icon), "matrix_demoman_plague");
...
methodmap AgentGraham < CClotBody
{
	...
	public AgentGraham(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentGraham_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentGraham_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentGraham_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/player/items/demo/tamoshanter.mdl");
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/demo/dec22_glasgow_bankroll_style3/dec22_glasgow_bankroll_style3.mdl");
...
public void AgentGraham_ClotThink(int iNPC)
{
	AgentGraham npc = view_as<AgentGraham>(iNPC);
...
public Action AgentGraham_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentGraham npc = view_as<AgentGraham>(victim);
...
public void AgentGraham_NPCDeath(int entity)
{
	AgentGraham npc = view_as<AgentGraham>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
