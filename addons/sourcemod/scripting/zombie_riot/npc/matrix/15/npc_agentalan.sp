...
void AgentAlan_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Alan");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_alan");
...
{
	return AgentAlan(client, vecPos, vecAng, ally);
}

static float fl_Cooldown[MAXENTITIES];
static float fl_Timer[MAXENTITIES];

methodmap AgentAlan < CClotBody
{
	...
	public AgentAlan(int client, float vecPos[3], float vecAng[3], int ally)
	{
		AgentAlan npc = view_as<AgentAlan>(CClotBody(vecPos, vecAng, "models/player/medic.mdl", "1.0", "700", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentAlan_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentAlan_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentAlan_ClotThink);
		npc.f_Timer = 0.0;
		npc.f_Cooldown = 0.0;
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/medic/medic_mask/medic_mask.mdl");
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/medic/dec15_bunnyhoppers_ballistics_vest/dec15_bunnyhoppers_ballistics_vest.mdl");
...
public void AgentAlan_ClotThink(int iNPC)
{
	AgentAlan npc = view_as<AgentAlan>(iNPC);
...
public Action AgentAlan_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentAlan npc = view_as<AgentAlan>(victim);
...
public void AgentAlan_NPCDeath(int entity)
{
	AgentAlan npc = view_as<AgentAlan>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
