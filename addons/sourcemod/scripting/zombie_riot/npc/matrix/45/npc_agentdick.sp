...
{
	return AgentDick(client, vecPos, vecAng, ally);
}
methodmap AgentDick < CClotBody
{
	...
	public AgentDick(int client, float vecPos[3], float vecAng[3], int ally)
	{
		AgentDick npc = view_as<AgentDick>(CClotBody(vecPos, vecAng, "models/player/soldier.mdl", "1.0", "700", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentDick_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentDick_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentDick_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/player/items/soldier/grenadier_softcap.mdl");
...
public void AgentDick_ClotThink(int iNPC)
{
	AgentDick npc = view_as<AgentDick>(iNPC);
...
public Action AgentDick_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentDick npc = view_as<AgentDick>(victim);
...
public void AgentDick_NPCDeath(int entity)
{
	AgentDick npc = view_as<AgentDick>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
