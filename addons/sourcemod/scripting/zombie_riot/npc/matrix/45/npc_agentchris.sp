...
void AgentChris_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Chris");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_chris");
...
{
	return AgentChris(client, vecPos, vecAng, ally);
}
methodmap AgentChris < CClotBody
{
	...
	public AgentChris(int client, float vecPos[3], float vecAng[3], int ally)
	{
		AgentChris npc = view_as<AgentChris>(CClotBody(vecPos, vecAng, "models/player/spy.mdl", "1.0", "700", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentChris_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentChris_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentChris_ClotThink);
		...
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/spy/sum23_cleaners_cap_style1/sum23_cleaners_cap_style1.mdl");

		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/spy/hwn2022_turncoat/hwn2022_turncoat.mdl");
...
public void AgentChris_ClotThink(int iNPC)
{
	AgentChris npc = view_as<AgentChris>(iNPC);
...
public Action AgentChris_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentChris npc = view_as<AgentChris>(victim);
...
public void AgentChris_NPCDeath(int entity)
{
	AgentChris npc = view_as<AgentChris>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
