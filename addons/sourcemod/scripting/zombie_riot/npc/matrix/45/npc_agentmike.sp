...
void AgentMike_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Mike");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_mike");
...
methodmap AgentMike < CClotBody
{
	...
	public AgentMike(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentMike_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentMike_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentMike_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/all_class/sum21_manndatory_attire_style3/sum21_manndatory_attire_style3_pyro.mdl");
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/pyro/sum24_scorched_stompers_style2/sum24_scorched_stompers_style2.mdl");

		npc.m_iWearable5 = npc.EquipItem("head", "models/workshop/player/items/pyro/sum24_invisible_flame_style2/sum24_invisible_flame_style2.mdl");
...
public void AgentMike_ClotThink(int iNPC)
{
	AgentMike npc = view_as<AgentMike>(iNPC);
						...
						if(npc.DoSwingTrace(swingTrace, PrimaryThreatIndex))
							{
								...
								if(target > 0) 
								{
									...
									Elemental_AddCorruptionDamage(target, npc.index, npc.index ? 85 : 10);
...
public Action AgentMike_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentMike npc = view_as<AgentMike>(victim);
...
public void AgentMike_NPCDeath(int entity)
{
	AgentMike npc = view_as<AgentMike>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
