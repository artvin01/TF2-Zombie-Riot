...
void AgentIan_OnMapStart_NPC()
{
	...
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_ian");
...
methodmap AgentIan < CClotBody
{
	...
	public AgentIan(int client, float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentIan_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentIan_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentIan_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/player/items/heavy/coupe_disaster.mdl");
		
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop_partner/player/items/heavy/hm_shirt/hm_shirt.mdl");

		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/heavy/cc_summer2015_el_duderino/cc_summer2015_el_duderino.mdl");
...
public void AgentIan_ClotThink(int iNPC)
{
	AgentIan npc = view_as<AgentIan>(iNPC);
...
public Action AgentIan_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	if(attacker <= 0)
	return Plugin_Continue;

	AgentIan npc = view_as<AgentIan>(victim);
	...
	if(npc.m_flDead_Ringer_Invis >= gameTime)
    {
		if(fl_MatrixReflect[attacker] <= GetGameTime())
		{
			...
			float parrydamage = GetRandomFloat(20.0, 30.0);
	...
	{
		AgentIan_Reflect_Enable(npc);
	}
	
	return Plugin_Changed;
}
//did this for you so it's simpler to learn
static void AgentIan_Reflect_Enable(AgentIan npc)
{
	...
	npc.m_flDead_Ringer_Invis = GetGameTime(npc.index) + 5.0;
...
static void AgentIan_Reflect_Disable(AgentIan npc)
{
...
static void AgentIan_NPCDeath(int entity)
{
	AgentIan npc = view_as<AgentIan>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
