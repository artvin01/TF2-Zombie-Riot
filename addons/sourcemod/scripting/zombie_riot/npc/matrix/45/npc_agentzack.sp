...
void AgentZack_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Zack");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_zack");
...
methodmap AgentZack < CClotBody
{
	...
	public AgentZack(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentZack_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentZack_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentZack_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/engineer/dec2014_engineer_detectiveglasses/dec2014_engineer_detectiveglasses.mdl");
...
public void AgentZack_ClotThink(int iNPC)
{
	AgentZack npc = view_as<AgentZack>(iNPC);
	...
	npc.m_flSpeed = (75.0 * TimeMultiplier);
...
public Action AgentZack_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentZack npc = view_as<AgentZack>(victim);
...
public void AgentZack_NPCDeath(int entity)
{
	AgentZack npc = view_as<AgentZack>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
