...
void AgentJackson_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Jackson");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_jackson");
	strcopy(data.Icon, sizeof(data.Icon), "matrix_agent_jackson");
...
methodmap AgentJackson < CClotBody
{
	...
	public AgentJackson(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentJackson_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentJackson_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentJackson_ClotThink);
		...
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/all_class/sum21_manndatory_attire_style3/sum21_manndatory_attire_style3_soldier.mdl");

		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/all_class/xms2013_jacket/xms2013_jacket_soldier.mdl");
...
public void AgentJackson_ClotThink(int iNPC)
{
	AgentJackson npc = view_as<AgentJackson>(iNPC);
...
public Action AgentJackson_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentJackson npc = view_as<AgentJackson>(victim);
...
public void AgentJackson_NPCDeath(int entity)
{
	AgentJackson npc = view_as<AgentJackson>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
