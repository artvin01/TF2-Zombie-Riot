...
void AgentEric_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Eric");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_eric");
...
methodmap AgentEric < CClotBody
{
	...
	public AgentEric(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentEric_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentEric_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentEric_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/player/items/soldier/drg_hair_beard.mdl");
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/soldier/sum22_chaser_style2/sum22_chaser_style2.mdl");

		npc.m_iWearable5 = npc.EquipItem("head", "models/workshop/player/items/all_class/sum21_manndatory_attire_style3/sum21_manndatory_attire_style3_soldier.mdl");
...
public void AgentEric_ClotThink(int iNPC)
{
	AgentEric npc = view_as<AgentEric>(iNPC);
...
public Action AgentEric_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentEric npc = view_as<AgentEric>(victim);
...
public void AgentEric_NPCDeath(int entity)
{
	AgentEric npc = view_as<AgentEric>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
