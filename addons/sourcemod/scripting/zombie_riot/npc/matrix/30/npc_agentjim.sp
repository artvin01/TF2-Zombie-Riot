...
void AgentJim_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Jim");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_jim");
...
{
	return AgentJim(client, vecPos, vecAng, ally);
}
methodmap AgentJim < CClotBody
{
	...
	public AgentJim(int client, float vecPos[3], float vecAng[3], int ally)
	{
		AgentJim npc = view_as<AgentJim>(CClotBody(vecPos, vecAng, "models/player/spy.mdl", "1.0", "700", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentJim_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentJim_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentJim_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/spy/sf14_nightmare_fedora/sf14_nightmare_fedora.mdl");
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/spy/dec23_strasbourg_scholar_style2/dec23_strasbourg_scholar_style2.mdl");
...
public void AgentJim_ClotThink(int iNPC)
{
	AgentJim npc = view_as<AgentJim>(iNPC);
...
public Action AgentJim_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentJim npc = view_as<AgentJim>(victim);
...
public void AgentJim_NPCDeath(int entity)
{
	AgentJim npc = view_as<AgentJim>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
