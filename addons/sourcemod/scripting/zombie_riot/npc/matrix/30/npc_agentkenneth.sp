...
void AgentKenneth_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Kenneth");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_kenneth");
...
{
	return AgentKenneth(client, vecPos, vecAng, ally);
}
methodmap AgentKenneth < CClotBody
{
	...
	public AgentKenneth(int client, float vecPos[3], float vecAng[3], int ally)
	{
		AgentKenneth npc = view_as<AgentKenneth>(CClotBody(vecPos, vecAng, "models/player/engineer.mdl", "1.0", "700", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentKenneth_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentKenneth_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentKenneth_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/engineer/dec23_clue_hairdo_style3/dec23_clue_hairdo_style3.mdl");
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/engineer/dec23_sleuth_suit/dec23_sleuth_suit.mdl");
...
public void AgentKenneth_ClotThink(int iNPC)
{
	AgentKenneth npc = view_as<AgentKenneth>(iNPC);
...
public Action AgentKenneth_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentKenneth npc = view_as<AgentKenneth>(victim);
...
public void AgentKenneth_NPCDeath(int entity)
{
	AgentKenneth npc = view_as<AgentKenneth>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
