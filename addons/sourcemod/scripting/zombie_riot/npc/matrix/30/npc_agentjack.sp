...
void AgentJack_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Jack");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_jack");
...
methodmap AgentJack < CClotBody
{
	...
	public AgentJack(int client, float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentJack_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentJack_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentJack_ClotThink);
...
public void AgentJack_ClotThink(int iNPC)
{
	AgentJack npc = view_as<AgentJack>(iNPC);
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
		...
		if(npc.m_flNextRangedAttack < GetGameTime(npc.index) && flDistanceToTarget > 62500 && flDistanceToTarget < 122500 && npc.m_flReloadDelay < GetGameTime(npc.index))
		{
			...
			else
			{
				...
				FireBullet(npc.index, npc.m_iWearable1, WorldSpaceVec, vecDir, EscapeModeForNpc ? 10.0 : 10.0, 9000.0, DMG_BULLET, "bullet_tracer01_red");
...
public Action AgentJack_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentJack npc = view_as<AgentJack>(victim);
...
public void AgentJack_NPCDeath(int entity)
{
	AgentJack npc = view_as<AgentJack>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
