...
void AgentPaul_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Paul");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_paul");
...
methodmap AgentPaul < CClotBody
{
	...
	public AgentPaul(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentPaul_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentPaul_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentPaul_ClotThink);
		npc.f_Timer = 0.0;
		npc.f_Cooldown = 0.0;
...
public void AgentPaul_ClotThink(int iNPC)
{
	AgentPaul npc = view_as<AgentPaul>(iNPC);
...
public Action AgentPaul_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentPaul npc = view_as<AgentPaul>(victim);
...
public void AgentPaul_NPCDeath(int entity)
{
	AgentPaul npc = view_as<AgentPaul>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
