...
void Merovingian_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Merovingian");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_merovingian");
	strcopy(data.Icon, sizeof(data.Icon), "matrix_merovingian");
...
methodmap Merovingian < CClotBody
{
	...
	public Merovingian(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCThink[npc.index] = view_as<Function>(Merovingian_ClotThink);
		...
		npc.m_iWearable1 = npc.EquipItem("head", "models/weapons/c_models/c_revolver/c_revolver.mdl");
		SetVariantString("1.0");
		AcceptEntityInput(npc.m_iWearable1, "SetModelScale");

		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/all_class/sum21_manndatory_attire_style3/sum21_manndatory_attire_style3_spy.mdl");
		npc.m_iWearable3 = npc.EquipItem("head", "models/player/items/spy/spy_beret.mdl");
...
public void Merovingian_ClotThink(int iNPC)
{
	Merovingian npc = view_as<Merovingian>(iNPC);
						...
						if(npc.DoSwingTrace(swingTrace, PrimaryThreatIndex))
							{
								...
								if(target > 0) 
								{
									...
									Elemental_AddCorruptionDamage(target, npc.index, npc.index ? 75 : 10);
...
public Action Merovingian_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Merovingian npc = view_as<Merovingian>(victim);
...
public void Merovingian_NPCDeath(int entity)
{
	Merovingian npc = view_as<Merovingian>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
