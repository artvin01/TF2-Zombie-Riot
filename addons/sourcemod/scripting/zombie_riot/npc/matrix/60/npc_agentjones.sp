...
void AgentJones_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Jones");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_jones");
	strcopy(data.Icon, sizeof(data.Icon), "matrix_agent_jones");
...
{
	return AgentJones(client, vecPos, vecAng, ally);
}

methodmap AgentJones < CClotBody
{
	...
	public AgentJones(int client, float vecPos[3], float vecAng[3], int ally)
	{
		AgentJones npc = view_as<AgentJones>(CClotBody(vecPos, vecAng, "models/player/sniper.mdl", "1.0", "700", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentJones_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentJones_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentJones_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/all_class/sum21_manndatory_attire_style3/sum21_manndatory_attire_style3_sniper.mdl");
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/all_class/spr17_wingman/spr17_wingman_sniper.mdl");
...
public void AgentJones_ClotThink(int iNPC)
{
	AgentJones npc = view_as<AgentJones>(iNPC);
	...
	float vecTarget2[3], VecSelfNpc2[3];// WorldSpaceCenter(target2, vecTarget2);// WorldSpaceCenter(npc.index, VecSelfNpc2);
	...
	for(int EnemyLoop; EnemyLoop < MAXENTITIES; EnemyLoop ++)
	{
		if(IsValidEnemy(npc.index, EnemyLoop, true, true))
		{
			...
			{
				WorldSpaceCenter(EnemyLoop, vecTarget2);
				WorldSpaceCenter(npc.index, VecSelfNpc2);
				float distance = GetVectorDistance(vecTarget2, VecSelfNpc2, true);
...
public Action AgentJones_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentJones npc = view_as<AgentJones>(victim);
...
public void AgentJones_NPCDeath(int entity)
{
	AgentJones npc = view_as<AgentJones>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
