...
void AgentJeremy_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Jeremy");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_jeremy");
	strcopy(data.Icon, sizeof(data.Icon), "matrix_pyro_plague");
...
methodmap AgentJeremy < CClotBody
{
	...
	public AgentJeremy(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentJeremy_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentJeremy_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentJeremy_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/pyro/jul13_soot_suit/jul13_soot_suit.mdl");
		
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/all_class/jul13_sweet_shades_s1/jul13_sweet_shades_s1_pyro.mdl");

		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/pyro/dec2014_black_knights_bascinet/dec2014_black_knights_bascinet.mdl");

		npc.m_iWearable5 = npc.EquipItem("head", "models/workshop/player/items/pyro/dec2014_armoured_appendages/dec2014_armoured_appendages.mdl");
...
public void AgentJeremy_ClotThink(int iNPC)
{
	AgentJeremy npc = view_as<AgentJeremy>(iNPC);
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
			...
			if(npc.m_flNextRangedAttack < GetGameTime(npc.index) && flDistanceToTarget > 62500 && flDistanceToTarget < 122500 && npc.m_flReloadDelay < GetGameTime(npc.index))
			{
				...
				else
				{
					...
					{
						FireBullet(npc.index, npc.m_iWearable1, WorldSpaceVec, vecDir, 35.0, 9000.0, DMG_BULLET, "bullet_tracer01_red");
					}
						...
						if(npc.DoSwingTrace(swingTrace, PrimaryThreatIndex))
							{
								...
								if(target > 0) 
								{
									...
									Elemental_AddCorruptionDamage(target, npc.index, npc.index ? 95 : 10);
...
public Action AgentJeremy_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentJeremy npc = view_as<AgentJeremy>(victim);
...
public void AgentJeremy_NPCDeath(int entity)
{
	AgentJeremy npc = view_as<AgentJeremy>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
