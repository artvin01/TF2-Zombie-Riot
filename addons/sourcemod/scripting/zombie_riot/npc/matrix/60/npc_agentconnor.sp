...
void AgentConnor_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Connor");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_connor");
	strcopy(data.Icon, sizeof(data.Icon), "matrix_spy");
...
{
	return AgentConnor(client, vecPos, vecAng, ally);
}
methodmap AgentConnor < CClotBody
{
	...
	public AgentConnor(int client, float vecPos[3], float vecAng[3], int ally)
	{
		AgentConnor npc = view_as<AgentConnor>(CClotBody(vecPos, vecAng, "models/player/spy.mdl", "1.0", "700", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentConnor_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentConnor_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentConnor_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/player/items/spy/spy_detective_noir.mdl");
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/spy/spr18_assassins_attire/spr18_assassins_attire.mdl");
...
public void AgentConnor_ClotThink(int iNPC)
{
	AgentConnor npc = view_as<AgentConnor>(iNPC);
...
public Action AgentConnor_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentConnor npc = view_as<AgentConnor>(victim);
...
public void AgentConnor_NPCDeath(int entity)
{
	AgentConnor npc = view_as<AgentConnor>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
