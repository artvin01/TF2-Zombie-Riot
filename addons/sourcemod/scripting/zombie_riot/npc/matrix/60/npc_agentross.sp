...
void AgentRoss_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Ross");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_ross");
...
methodmap AgentRoss < CClotBody
{
	...
	public AgentRoss(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentRoss_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentRoss_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentRoss_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/sniper/dec17_handsome_hitman/dec17_handsome_hitman.mdl");
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/sniper/fall17_riflemans_regalia/fall17_riflemans_regalia.mdl");
...
public void AgentRoss_ClotThink(int iNPC)
{
	AgentRoss npc = view_as<AgentRoss>(iNPC);
...
public Action AgentRoss_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentRoss npc = view_as<AgentRoss>(victim);
...
{
	AgentRoss npc = view_as<AgentRoss>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
