...
void AgentSpencer_OnMapStart_NPC()
{
	...
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_spencer");
...
methodmap AgentSpencer < CClotBody
{
	...
	public AgentSpencer(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentSpencer_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentSpencer_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentSpencer_ClotThink);
...
public void AgentSpencer_ClotThink(int iNPC)
{
	AgentSpencer npc = view_as<AgentSpencer>(iNPC);
	...
	{
		AgentSpencer_Reflect_Disable(npc);
	}
...
public Action AgentSpencer_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	if(attacker <= 0)
	return Plugin_Continue;

	AgentSpencer npc = view_as<AgentSpencer>(victim);
	...
	{
		AgentSpencer_Reflect_Enable(npc);
	}
	
	return Plugin_Changed;
}
//did this for you so it's simpler to learn
static void AgentSpencer_Reflect_Enable(AgentSpencer npc)
{
...
static void AgentSpencer_Reflect_Disable(AgentSpencer npc)
{
...
static void AgentSpencer_NPCDeath(int entity)
{
	AgentSpencer npc = view_as<AgentSpencer>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
