...
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Agent Henry");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_agent_henry");
...
methodmap AgentHenry < CClotBody
{
	...
	public AgentHenry(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(AgentHenry_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(AgentHenry_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(AgentHenry_ClotThink);
		npc.f_Timer = 0.0;
		npc.f_Cooldown = 0.0;
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/medic/jul13_madmans_mop/jul13_madmans_mop.mdl");

		npc.m_iWearable5 = npc.EquipItem("head", "models/workshop/player/items/all_class/sum21_manndatory_attire_style3/sum21_manndatory_attire_style3_medic.mdl");
...
public void AgentHenry_ClotThink(int iNPC)
{
	AgentHenry npc = view_as<AgentHenry>(iNPC);
	...
	if(distance > (NORMAL_ENEMY_MELEE_RANGE_FLOAT_SQUARED * 0.1) && distance < 10500.0)
	{
		vecTarget2[2] += 300.0;
		...
		ApplyStatusEffect(npc.index, npc.index, "False Therapy", 5.0);
...
public Action AgentHenry_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	AgentHenry npc = view_as<AgentHenry>(victim);
...
public void AgentHenry_NPCDeath(int entity)
{
	AgentHenry npc = view_as<AgentHenry>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
