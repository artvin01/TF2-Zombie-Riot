...
static const char g_DeathSounds[][] = {
	"cof/sewmo/sewmo_die1.mp3",
};

static const char g_HurtSounds[][] = {
	"cof/sewmo/sewmo_pain1.mp3",
	"cof/sewmo/sewmo_pain2.mp3",
};

static const char g_IdleAlertedSounds[][] = {
	"cof/sewmo/sewmo_alert1.mp3",
	"cof/sewmo/sewmo_alert2.mp3",
};

static const char g_MeleeAttackSounds[][] = {
	"cof/sewmo/sewmo_attack1.mp3",
	"cof/sewmo/sewmo_attack2.mp3",
};

static const char g_MeleeHitSounds[][] = {
	"cof/sewmo/sewmo_hit1.mp3",
	"cof/sewmo/sewmo_hit2.mp3",
};

void Sewmo_OnMapStart_NPC()
{
	PrecacheModel("models/zombie_riot/cof/sewmo.mdl");
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Sewmo");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_sewmo");
	strcopy(data.Icon, sizeof(data.Icon), "sewmo");
...
methodmap Sewmo < CClotBody
{
	...
	public Sewmo(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(Sewmo_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(Sewmo_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(Sewmo_ClotThink);
...
public void Sewmo_ClotThink(int iNPC)
{
	Sewmo npc = view_as<Sewmo>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		SewmoSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
...
public Action Sewmo_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Sewmo npc = view_as<Sewmo>(victim);
...
public void Sewmo_NPCDeath(int entity)
{
	Sewmo npc = view_as<Sewmo>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
//		GetEntPropString(client, Prop_Data, "m_ModelName", model, sizeof(model));
		DispatchKeyValue(entity_death, "model", "models/zombie_riot/cof/sewmo.mdl");
		...
		CreateTimer(2.0, Timer_RemoveEntitySewmo, EntIndexToEntRef(entity_death), TIMER_FLAG_NO_MAPCHANGE);
	}
}

public Action Timer_RemoveEntitySewmo(Handle timer, any entid)
{
...
void SewmoSelfDefense(Sewmo npc, float gameTime, int target, float distance)
{
