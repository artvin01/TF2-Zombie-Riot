...
static const char g_DeathSounds[][] = {
	"cof/baby/baby_death1.mp3",
};

static const char g_HurtSounds[][] = {
	"cof/baby/baby_pain1.mp3",
};

static const char g_IdleAlertedSounds[][] = {
	"cof/baby/baby_alert1.mp3",
	"cof/baby/baby_alert2.mp3",
};

static const char g_MeleeHitSounds[][] = {
	"cof/baby/baby_attack1.mp3",
	"cof/baby/baby_attack2.mp3",
};

static float fl_DefaultSpeed_Baby = 150.0;
static float fl_DefaultSpeed_Baby_Nightmare = 180.0;
static bool b_IsNightmare[MAXENTITIES];

void Baby_OnMapStart_NPC()
{
	PrecacheModel("models/zombie_riot/cof/baby/baby.mdl");
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Baby");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_baby");
	strcopy(data.Icon, sizeof(data.Icon), "baby");
...
{
	return Baby(vecPos, vecAng, team, data);
}
methodmap Baby < CClotBody
{
	...
	public Baby(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		Baby npc = view_as<Baby>(CClotBody(vecPos, vecAng, "models/zombie_riot/cof/baby/baby.mdl", "1.10", "300", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(Baby_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(Baby_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(Baby_ClotThink);
		...
		if(nightmare)
		{
			...
			npc.m_flSpeed = fl_DefaultSpeed_Baby_Nightmare;
		}
		else
		{
			npc.m_flSpeed = fl_DefaultSpeed_Baby;
		}
...
public void Baby_ClotThink(int iNPC)
{
	Baby npc = view_as<Baby>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		BabySelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
...
public Action Baby_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Baby npc = view_as<Baby>(victim);
...
public void Baby_NPCDeath(int entity)
{
	Baby npc = view_as<Baby>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
//		GetEntPropString(client, Prop_Data, "m_ModelName", model, sizeof(model));
		DispatchKeyValue(entity_death, "model", "models/zombie_riot/cof/baby/baby.mdl");
		...
		CreateTimer(1.0, Timer_RemoveEntityBaby, EntIndexToEntRef(entity_death), TIMER_FLAG_NO_MAPCHANGE);
	}
}

public Action Timer_RemoveEntityBaby(Handle timer, any entid)
{
...
void BabySelfDefense(Baby npc, float gameTime, int target, float distance)
{
