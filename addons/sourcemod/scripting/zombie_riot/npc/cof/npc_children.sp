...
static const char g_DeathSounds[][] = {
	"cof/children/child_pain2.mp3",
};

static const char g_HurtSounds[][] = {
	"cof/children/child_pain1.mp3",
	"cof/children/child_pain2.mp3",
};

static const char g_IdleAlertedSounds[][] = {
	"cof/children/child_alert1.mp3",
	"cof/children/child_alert2.mp3",
};

static const char g_MeleeAttackSounds[][] = {
	"cof/children/child_attack1.mp3",
	"cof/children/child_attack2.mp3",
};
...
static float fl_DefaultSpeed_Children = 250.0;
static float fl_DefaultSpeed_Children_Nightmare = 300.0;
static bool b_IsNightmare[MAXENTITIES];

void Children_OnMapStart_NPC()
{
	PrecacheModel("models/zombie_riot/cof/children/children.mdl");
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Children");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_children");
...
{
	return Children(vecPos, vecAng, team, data);
}
methodmap Children < CClotBody
{
	...
	public Children(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		Children npc = view_as<Children>(CClotBody(vecPos, vecAng, "models/zombie_riot/cof/children/children.mdl", "1.10", "300", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(Children_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(Children_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(Children_ClotThink);
		...
		if(nightmare)
		{
			...
			npc.m_flSpeed = fl_DefaultSpeed_Children_Nightmare;
		}
		else
		{
			npc.m_flSpeed = fl_DefaultSpeed_Children;
		}
...
public void Children_ClotThink(int iNPC)
{
	Children npc = view_as<Children>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		ChildrenSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
...
public Action Children_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Children npc = view_as<Children>(victim);
...
public void Children_NPCDeath(int entity)
{
	Children npc = view_as<Children>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
//		GetEntPropString(client, Prop_Data, "m_ModelName", model, sizeof(model));
		DispatchKeyValue(entity_death, "model", "models/zombie_riot/cof/children/children.mdl");
		...
		CreateTimer(1.5, Timer_RemoveEntityChildren, EntIndexToEntRef(entity_death), TIMER_FLAG_NO_MAPCHANGE);
	}
}

public Action Timer_RemoveEntityChildren(Handle timer, any entid)
{
...
void ChildrenSelfDefense(Children npc, float gameTime, int target, float distance)
{
