...
methodmap TrueGoggles < CClotBody
{
	...
	public void PlayIdleSound()
	{
		...
		this.m_flNextIdleSound = GetGameTime(this.index) + GetRandomFloat(15.0, 20.0);
	}
	...
	public TrueGoggles(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		TrueGoggles npc = view_as<TrueGoggles>(CClotBody(vecPos, vecAng, "models/player/sniper.mdl", "1.35", "25000", ally, false, true, true,true)); //giant!
		
		i_NpcInternalId[npc.index] = RAIDMODE_TRUE_GOGGLES;
		...
		for(int client_check=1; client_check<=MaxClients; client_check++)
		{
			if(IsClientInGame(client_check) && !IsFakeClient(client_check))
			{
				...
				ShowGameText(client_check, "item_armor", 1, "%t", "True Blue Goggles Spawn");
			}
		}
		...
		SDKHook(npc.index, SDKHook_Think, TrueGoggles_ClotThink);
		SDKHook(npc.index, SDKHook_OnTakeDamage, TrueGoggles_ClotDamaged);
		...
		npc.m_iWearable1 = ParticleEffectAt_Parent(flPos, "unusual_symbols_parent_ice", npc.index, "head", {0.0,0.0,0.0});
		
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/all_class/xms2013_polar_pullover/xms2013_polar_pullover_sniper.mdl");
		SetVariantString("1.0");
		AcceptEntityInput(npc.m_iWearable2, "SetModelScale");
		
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/sniper/sbox2014_sniper_quiver/sbox2014_sniper_quiver.mdl");
		SetVariantString("1.0");
		...
		npc.m_iWearable5 = npc.EquipItem("head","models/workshop/player/items/sniper/fall17_cammy_jammies/fall17_cammy_jammies.mdl");
		SetVariantString("1.0");
		AcceptEntityInput(npc.m_iWearable5, "SetModelScale");
		
		SetEntProp(npc.m_iWearable5, Prop_Send, "m_nSkin", 1);

		//npc.m_iWearable6 = Weapon
		...
		SetEntityRenderColor(npc.m_iWearable3, 192, 192, 192, 125);
...
//TODO 
//Rewrite
public void TrueGoggles_ClotThink(int iNPC)
{
	TrueGoggles npc = view_as<TrueGoggles>(iNPC);
	...
	if(RaidModeTime < GetGameTime())
	{
		...
		SDKUnhook(npc.index, SDKHook_Think, TrueGoggles_ClotThink);
	}
	...
	//		if(flDistanceToTarget < Pow(110.0,2.0))
			...
			if(ZR_GetWaveCount()+1 > 25)
			{
				if(npc.m_flTimebeforekamehameha < GetGameTime(npc.index) && !npc.Anger)
				{
					...
					npc.m_bInKame = true;
					TrueGoggles_TBB_Ability(npc.index);
				}
				else if(npc.m_flTimebeforekamehameha < GetGameTime(npc.index) && npc.Anger)
				{
					npc.m_flTimebeforekamehameha = GetGameTime(npc.index) + 35.0;
					npc.m_bInKame = true;
					TrueGoggles_TBB_Ability_Anger(npc.index);
				}
			}
			if(npc.m_bInKame)
			{
			...
			if(npc.m_flNextRangedBarrage_Spam < GetGameTime(npc.index) && npc.m_flNextRangedBarrage_Singular < GetGameTime(npc.index) && flDistanceToTarget < Pow(500.0, 2.0) || (npc.m_bInKame && npc.m_flNextRangedAttack < GetGameTime(npc.index)))
			{
				if (!npc.Anger)
				{
					...
					if(ZR_GetWaveCount()+1 > 55)
						TrueGoggles_IOC_Invoke(EntIndexToEntRef(npc.index), closest);
...
public Action TrueGoggles_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	TrueGoggles npc = view_as<TrueGoggles>(victim);
	...
	if(ZR_GetWaveCount()+1 > 55 && !b_angered_twice[npc.index])
	{
		if((GetEntProp(npc.index, Prop_Data, "m_iMaxHealth")/20) >= GetEntProp(npc.index, Prop_Data, "m_iHealth")) //npc.Anger after half hp/400 hp
		{
			...
			SDKUnhook(npc.index, SDKHook_Think, TrueGoggles_TBB_Tick);
...
public void TrueGoggles_NPCDeath(int entity)
{
	TrueGoggles npc = view_as<TrueGoggles>(entity);
	...
	StopSound(entity, SNDCHAN_STATIC, "weapons/physcannon/energy_sing_loop4.wav");
	SDKUnhook(npc.index, SDKHook_Think, TrueGoggles_ClotThink);
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, TrueGoggles_ClotDamaged);
...
void TrueGoggles_TBB_Ability_Anger(int client)
{
	...
	CreateTimer(5.0, TrueGoggles_TBB_Timer, client, TIMER_FLAG_NO_MAPCHANGE);
	SDKHook(client, SDKHook_Think, TrueGoggles_TBB_Tick);
}


void TrueGoggles_TBB_Ability(int client)
{
	ParticleEffectAt(WorldSpaceCenter(client), "eyeboss_death_vortex", 2.0);
...
public Action TrueGoggles_TBB_Timer(Handle timer, int client)
{
...
static void FusionWarrior_GetBeamDrawStartPoint(int client, float startPoint[3])
{
	...
	TrueGoggles npc = view_as<TrueGoggles>(client);
	int iPitch = npc.LookupPoseParameter("body_pitch");
	if(iPitch < 0)
			return;	
	float flPitch = npc.GetPoseParameter(iPitch);
...
public void TrueGoggles_IOC_Invoke(int ref, int enemy)
{
	...
	if(IsValidEntity(entity))
	{
		...
		ResetPack(data);
		TrueGoggles_IonAttack(data);
	}
}

public Action TrueGoggles_DrawIon(Handle Timer, any data)
{
	TrueGoggles_IonAttack(data);
...
public void TrueGoggles_DrawIonBeam(float startPosition[3], const int color[4])
{
	float position[3];
	...
	public void TrueGoggles_IonAttack(Handle &data)
	{
		...
		if (Iondistance > 0)
		{
			...
			position[0] += s;
			position[1] += c;
			TrueGoggles_DrawIonBeam(position, {212, 175, 55, 255});
		...
		if (Iondistance > -30)
		CreateTimer(0.1, TrueGoggles_DrawIon, nData, TIMER_FLAG_NO_MAPCHANGE|TIMER_DATA_HNDL_CLOSE);
		else
		{
