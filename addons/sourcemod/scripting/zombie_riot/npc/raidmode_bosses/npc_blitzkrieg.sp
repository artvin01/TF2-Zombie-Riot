	...
	{	//75%-50%
		...
		if(b_pureblitz)
		{
			switch(GetRandomInt(1, 4))
			{
				...
				case 3:
				{
					CPrintToChatAll("{crimson}%s{default}: AAAAAAHHHHHAAAAAAAAAAAAAAAA!", c_NpcName[npc.index]);
				}
				case 4:
				{
					CPrintToChatAll("{crimson}%s{default}: ICH WERD DEIN DRECKS KOPF ZERSTÜCKELN!", c_NpcName[npc.index]);
				}
			}
		}
