...
public void GenghiKhan_OnMapStart()
{
...
methodmap GenghiKhan < CClotBody
{
	...
	public GenghiKhan(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		GenghiKhan npc = view_as<GenghiKhan>(CClotBody(vecPos, vecAng, "models/player/medic.mdl", "1.35", "25000", ally, false, true, true,true)); //giant!
		...
		SDKHook(npc.index, SDKHook_Think, GenghiKhan_ClotThink);
		SDKHook(npc.index, SDKHook_OnTakeDamage, GenghiKhan_ClotDamaged);
...
//TODO 
//Rewrite
public void GenghiKhan_ClotThink(int iNPC)
{
	GenghiKhan npc = view_as<GenghiKhan>(iNPC);
	...
	if(RaidModeTime < GetGameTime())
	{
		...
		SDKUnhook(npc.index, SDKHook_Think, GenghiKhan_ClotThink);
	}
	...
	//		if(flDistanceToTarget < Pow(110.0,2.0))
			...
			if(ZR_GetWaveCount()+1 > 25)
			{
				if(npc.m_flTimebeforekamehameha < GetGameTime(npc.index) && !npc.Anger)
				{
					...
					npc.m_bInKame = true;
					GenghiKhan_TBB_Ability(npc.index);
				}
				else if(npc.m_flTimebeforekamehameha < GetGameTime(npc.index) && npc.Anger)
				{
					npc.m_flTimebeforekamehameha = GetGameTime(npc.index) + 35.0;
					npc.m_bInKame = true;
					GenghiKhan_TBB_Ability_Anger(npc.index);
				}
			}
			if(npc.m_bInKame)
			{
			...
			if(npc.m_flNextRangedBarrage_Spam < GetGameTime(npc.index) && npc.m_flNextRangedBarrage_Singular < GetGameTime(npc.index) && flDistanceToTarget > Pow(110.0, 2.0) && flDistanceToTarget < Pow(500.0, 2.0) || (npc.m_bInKame && npc.m_flNextRangedAttack < GetGameTime(npc.index)))
			{
				if (!npc.Anger)
				{
					...
					if(ZR_GetWaveCount()+1 > 55)
						GenghiKhan_IOC_Invoke(EntIndexToEntRef(npc.index), closest);
...
public Action GenghiKhan_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	GenghiKhan npc = view_as<GenghiKhan>(victim);
	...
	if(ZR_GetWaveCount()+1 > 55 && !b_angered_twice[npc.index])
	{
		if((GetEntProp(npc.index, Prop_Data, "m_iMaxHealth")/20) >= GetEntProp(npc.index, Prop_Data, "m_iHealth")) //npc.Anger after half hp/400 hp
		{
			...
			SDKUnhook(npc.index, SDKHook_Think, GenghiKhan_TBB_Tick);
...
public void GenghiKhan_NPCDeath(int entity)
{
	GenghiKhan npc = view_as<GenghiKhan>(entity);
	...
	StopSound(entity, SNDCHAN_STATIC, "weapons/physcannon/energy_sing_loop4.wav");
	SDKUnhook(npc.index, SDKHook_Think, GenghiKhan_ClotThink);
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, GenghiKhan_ClotDamaged);
...
void GenghiKhan_TBB_Ability_Anger(int client)
{
	...
	CreateTimer(5.0, GenghiKhan_TBB_Timer, client, TIMER_FLAG_NO_MAPCHANGE);
	SDKHook(client, SDKHook_Think, GenghiKhan_TBB_Tick);
}


void GenghiKhan_TBB_Ability(int client)
{
	for (int building = 1; building < MaxClients; building++)
	{
		FusionWarrior_BEAM_BuildingHit[building] = false;
...
public Action GenghiKhan_TBB_Timer(Handle timer, int client)
{
...
static void FusionWarrior_GetBeamDrawStartPoint(int client, float startPoint[3])
{
	...
	GenghiKhan npc = view_as<GenghiKhan>(client);
	int iPitch = npc.LookupPoseParameter("body_pitch");
	if(iPitch < 0)
			return;	
	float flPitch = npc.GetPoseParameter(iPitch);
...
public void GenghiKhan_IOC_Invoke(int ref, int enemy)
{
	...
	if(IsValidEntity(entity))
	{
		...
		ResetPack(data);
		GenghiKhan_IonAttack(data);
	}
}

public Action GenghiKhan_DrawIon(Handle Timer, any data)
{
	GenghiKhan_IonAttack(data);
...
public void GenghiKhan_DrawIonBeam(float startPosition[3], const int color[4])
{
	float position[3];
	...
	public void GenghiKhan_IonAttack(Handle &data)
	{
		...
		if (Iondistance > 0)
		{
			...
			position[0] += s;
			position[1] += c;
			GenghiKhan_DrawIonBeam(position, {212, 175, 55, 255});
		...
		if (Iondistance > -30)
		CreateTimer(0.1, GenghiKhan_DrawIon, nData, TIMER_FLAG_NO_MAPCHANGE|TIMER_DATA_HNDL_CLOSE);
		else
		{
