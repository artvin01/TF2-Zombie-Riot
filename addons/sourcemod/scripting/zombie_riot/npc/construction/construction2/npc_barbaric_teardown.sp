...
{
	return BarbaricTeardown(vecPos, vecAng, team);
}

methodmap BarbaricTeardown < CClotBody
{
	...
	public BarbaricTeardown(float vecPos[3], float vecAng[3], int ally)
	{
		BarbaricTeardown npc = view_as<BarbaricTeardown>(CClotBody(vecPos, vecAng, "models/player/soldier.mdl", "1.0", "1000", ally));
...
{
	BarbaricTeardown npc = view_as<BarbaricTeardown>(iNPC);
...
void BarbaricDealtaker_SelfDefense(BarbaricTeardown npc, float distance, float vecTarget[3], float gameTime)
{
...
{
	BarbaricTeardown npc = view_as<BarbaricTeardown>(entity);
	if(!npc.m_bGib)
		npc.PlayDeathSound();
