...
void BattlefieldSupplier_OnMapStart_NPC()
{
...
methodmap BattlefieldSupplier < CClotBody
{
	...
	property float m_flReplaceSupplies
	{
		public get()							{ return fl_AbilityOrAttack[this.index][1]; }
		public set(float TempValueForProperty) 	{ fl_AbilityOrAttack[this.index][1] = TempValueForProperty; }
	}
	property float m_flWaitOnPlacing
	{
	...
	public BattlefieldSupplier(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(BattlefieldSupplier_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(BattlefieldSupplier_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(BattlefieldSupplier_ClotThink);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/engineer/hwn2025_heat_shield/hwn2025_heat_shield.mdl");
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/engineer/hwn2025_service_satchel/hwn2025_service_satchel.mdl");
...
public void BattlefieldSupplier_ClotThink(int iNPC)
{
	BattlefieldSupplier npc = view_as<BattlefieldSupplier>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		{
			BattlefieldSupplierBuildObject(npc, flDistanceToTarget); 
		}
		else
		{
			BattlefieldSupplierSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
		}
	}
	else
	{
...
public Action BattlefieldSupplier_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	BattlefieldSupplier npc = view_as<BattlefieldSupplier>(victim);
...
public void BattlefieldSupplier_NPCDeath(int entity)
{
	BattlefieldSupplier npc = view_as<BattlefieldSupplier>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void BattlefieldSupplierBuildObject(BattlefieldSupplier npc, float distance)
{
	if(distance < (NORMAL_ENEMY_MELEE_RANGE_FLOAT_SQUARED * 3.5) && npc.m_flWaitOnPlacing < GetGameTime(npc.index))
	{
		...
		if(IsValidEnemy(npc.index, Enemy_I_See))
		{
			...
			npc.m_flReplaceSupplies = GetGameTime(npc.index) + 15.0;
		}
	}
}
void BattlefieldSupplierSelfDefense(BattlefieldSupplier npc, float gameTime, int target, float distance)
{
	...
	if(npc.m_flReplaceSupplies < gameTime)
	{
		...
		fl_TotalArmor[npc.index] = 0.5;
		npc.SetActivity("ACT_MP_RUN_BUILDING_DEPLOYED");
		npc.m_flWaitOnPlacing = gameTime + 1.0;
	}
}