...
{
	return MaterialIron(vecPos, vecAng, team);
}

methodmap MaterialIron < CClotBody
{
	public MaterialIron(float vecPos[3], float vecAng[3], int team)
	{
...
static void ClotTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	if(attacker > 0)
	{
		MaterialIron npc = view_as<MaterialIron>(victim);
		Construction_OnTakeDamage("iron", 35, victim, attacker, damage, damagetype);
		if(!npc.Anger)
			npc.m_bCamo = false;
	}
}
...
static void ClotDeath(int entity)
{
	...
	Construction_NPCDeath("iron", 35, npc);
}
