...
void Interstellar_Weaver_MapStart_Mid()
{
	NPCData data;
...
{	
	...
	PrecacheModel(INTERSTELLAR_WEAVER_MODEL);
}
...
{
	Interstellar_Weaver_Mid npc = view_as<Interstellar_Weaver_Mid>(iNPC);
	...
	if(IsValidEntity(follow_id))
	{
		...
		if(I_see==follow_id)
		{
			Interstellar_Weaver_Middle_Movement(npc, Follow_Loc, false);	//we can see it, travel normally!
		}
		else
		{
			Interstellar_Weaver_Middle_Movement(npc, Follow_Loc, true);	//we can't see the thing we are following, noclip
		}
...
{
	Interstellar_Weaver_Mid npc = view_as<Interstellar_Weaver_Mid>(victim);
	...
	damage=0.0;	//interstellar weaver doesn't really take any damage, his "health bar" is just the combined health of all the towers
