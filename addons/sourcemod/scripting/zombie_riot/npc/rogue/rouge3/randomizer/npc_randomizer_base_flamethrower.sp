...
void RandomizerBaseFlamethrower_OnMapStart_NPC()
{
	...
	PrecacheParticleSystem("m_brazier_flame");
	
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Randomizer Base Flamethrower");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_randomizer_base_flamethrower");
...
{
	return RandomizerBaseFlamethrower(vecPos, vecAng, team);
}

methodmap RandomizerBaseFlamethrower < CClotBody
{
	...
	public RandomizerBaseFlamethrower(float vecPos[3], float vecAng[3], int ally)
	{
		RandomizerBaseFlamethrower npc = view_as<RandomizerBaseFlamethrower>(CClotBody(vecPos, vecAng, "models/player/pyro.mdl", "1.0", "3000", ally));
		...
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(RandomizerBaseFlamethrower_OnTakeDamage);
...
public void RandomizerBaseFlamethrower_ClotThink(int iNPC)
{
	RandomizerBaseFlamethrower npc = view_as<RandomizerBaseFlamethrower>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		RandomizerBaseFlamethrowerSelfDefense(npc); 
	}
	else
	{
...
public Action RandomizerBaseFlamethrower_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	RandomizerBaseFlamethrower npc = view_as<RandomizerBaseFlamethrower>(victim);
...
public void RandomizerBaseFlamethrower_NPCDeath(int entity)
{
	RandomizerBaseFlamethrower npc = view_as<RandomizerBaseFlamethrower>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void RandomizerBaseFlamethrowerSelfDefense(RandomizerBaseFlamethrower npc)
{
	...
	if(flDistanceToTarget < (NORMAL_ENEMY_MELEE_RANGE_FLOAT_SQUARED * 5.0))
	{
		...
		WandProjectile_ApplyFunctionToEntity(projectile, RandomizerBaseFlamethrower_Rocket_Particle_StartTouch);	
	}
	if(SpinSound)
		npc.PlayMinigunSound(false);
}
...
public void RandomizerBaseFlamethrower_Rocket_Particle_StartTouch(int entity, int target)
{
