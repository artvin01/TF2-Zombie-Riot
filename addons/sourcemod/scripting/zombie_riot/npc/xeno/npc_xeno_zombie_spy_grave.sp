...
public void XenoSpy_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Xeno Spy Facestabber");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_xeno_zombie_spy_grave");
...
{
	return XenoSpy(vecPos, vecAng, team);
}
methodmap XenoSpy < CClotBody
{
	...
	public XenoSpy(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = XenoSpy_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = XenoSpy_OnTakeDamage;
		func_NPCThink[npc.index] = XenoSpy_ClotThink;		
...
public Action XenoSpy_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
