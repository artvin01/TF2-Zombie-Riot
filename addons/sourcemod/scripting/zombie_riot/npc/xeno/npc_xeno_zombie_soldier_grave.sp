...
public void XenoSoldier_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Xeno Soldier Rocketeer");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_xeno_zombie_soldier_grave");
...
{
	return XenoSoldier(vecPos, vecAng, team);
}
methodmap XenoSoldier < CClotBody
{
	...
	public XenoSoldier(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = XenoSoldier_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = XenoSoldier_OnTakeDamage;
		func_NPCThink[npc.index] = XenoSoldier_ClotThink;	
			...
			if(flDistanceToTarget < 90000)
			{
				...
				if(IsValidEnemy(npc.index, Enemy_I_See))
				{
					...
					if(npc.m_flNextMeleeAttack < GetGameTime(npc.index))
					{
						...
						npc.FireRocket(vecTarget, 40.0, projectile_speed);
...
public void XenoSoldier_NPCDeath(int entity)
{
	XenoSoldier npc = view_as<XenoSoldier>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
