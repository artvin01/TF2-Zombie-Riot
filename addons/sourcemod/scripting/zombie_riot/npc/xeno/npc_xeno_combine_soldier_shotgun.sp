...
public void XenoCombineSoldierShotgun_OnMapStart_NPC()
{
...
methodmap XenoCombineSoldierShotgun < CClotBody
{
	...
	public XenoCombineSoldierShotgun(int client, float vecPos[3], float vecAng[3])
	{
		XenoCombineSoldierShotgun npc = view_as<XenoCombineSoldierShotgun>(CClotBody(vecPos, vecAng, "models/combine_soldier.mdl", "1.15", "800"));
...
public void XenoCombineSoldierShotgun_ClotThink(int iNPC)
{
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
			...
			if(npc.m_flNextRangedAttack < GetGameTime() && flDistanceToTarget < 10000 && npc.m_flReloadDelay < GetGameTime())
			{
				npc.m_fbGunout = true;
				
			//	float vPredictedPos[3]; vPredictedPos = PredictSubjectPosition(npc, PrimaryThreatIndex);
				npc.m_flNextRangedAttack = GetGameTime() + 0.9;
...
//					PrintToChatAll("%f", y);
					...
					if(EscapeModeForNpc)
					{
						FireBullet(npc.index, npc.m_iWearable1, WorldSpaceCenter(npc.index), vecDir, 15.0, 100.0, DMG_BULLET, "bullet_tracer02_blue");
					}
					else
					{
						FireBullet(npc.index, npc.m_iWearable1, WorldSpaceCenter(npc.index), vecDir, 10.0, 100.0, DMG_BULLET, "bullet_tracer02_blue");
					}
				}
				npc.PlayRangedSound();				
			}
...
public Action XenoCombineSoldierShotgun_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	XenoCombineSoldierShotgun npc = view_as<XenoCombineSoldierShotgun>(victim);
...
public void XenoCombineSoldierShotgun_NPCDeath(int entity)
{
	XenoCombineSoldierShotgun npc = view_as<XenoCombineSoldierShotgun>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
