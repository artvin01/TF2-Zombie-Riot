...
public void XenoPoisonZombie_OnMapStart_NPC()
{
...
methodmap XenoPoisonZombie < CClotBody
{
	...
	public XenoPoisonZombie(int client, float vecPos[3], float vecAng[3])
	{
		XenoPoisonZombie npc = view_as<XenoPoisonZombie>(CClotBody(vecPos, vecAng, "models/zombie/poison.mdl", "1.15", "1000"));
		...
		SDKHook(npc.index, SDKHook_OnTakeDamage, XenoPoisonZombie_ClotDamaged);
...
//TODO 
//Rewrite
public void XenoPoisonZombie_ClotThink(int iNPC)
{
	XenoPoisonZombie npc = view_as<XenoPoisonZombie>(iNPC);
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
			...
			//	npc.FaceTowards(vecTarget, 20000.0);
				...
				if (!npc.m_flAttackHappenswillhappen)
				{
					...
					npc.PlayMeleeSound();
					npc.m_flAttackHappens = GetGameTime()+0.8;
...
public Action XenoPoisonZombie_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	XenoPoisonZombie npc = view_as<XenoPoisonZombie>(victim);
...
public Action XenoPoisonZombie_Revert_Poison_Zombie_Resistance(Handle timer, int ref)
{
...
public Action XenoPoisonZombie_Revert_Poison_Zombie_Resistance_Enable(Handle timer, int ref)
{
	int zombie = EntRefToEntIndex(ref);
	if(IsValidEntity(zombie))
	{
		XenoPoisonZombie npc = view_as<XenoPoisonZombie>(zombie);
		npc.bXenoInfectedSpecialHurt = false;
	}
	return Plugin_Handled;
}

public void XenoPoisonZombie_NPCDeath(int entity)
{
	XenoPoisonZombie npc = view_as<XenoPoisonZombie>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
	
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, XenoPoisonZombie_ClotDamaged);
