...
public void XenoFortifiedPoisonZombie_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Xeno Fortified Poison Zombie");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_xeno_poisonzombie_fortified");
...
{
	return XenoFortifiedPoisonZombie(client, vecPos, vecAng, ally);
}
methodmap XenoFortifiedPoisonZombie < CClotBody
{
	...
	public XenoFortifiedPoisonZombie(int client, float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = XenoFortifiedPoisonZombie_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = XenoFortifiedPoisonZombie_OnTakeDamage;
		func_NPCThink[npc.index] = XenoFortifiedPoisonZombie_ClotThink;	
...
//TODO 
//Rewrite
public void XenoFortifiedPoisonZombie_ClotThink(int iNPC)
{
	XenoFortifiedPoisonZombie npc = view_as<XenoFortifiedPoisonZombie>(iNPC);
...
public Action XenoFortifiedPoisonZombie_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	XenoFortifiedPoisonZombie npc = view_as<XenoFortifiedPoisonZombie>(victim);
...
public Action XenoFortifiedPoisonZombie_Revert_Poison_Zombie_Resistance(Handle timer, int ref)
{
...
public Action XenoFortifiedPoisonZombie_Revert_Poison_Zombie_Resistance_Enable(Handle timer, int ref)
{
	int zombie = EntRefToEntIndex(ref);
	if(IsValidEntity(zombie))
	{
		XenoFortifiedPoisonZombie npc = view_as<XenoFortifiedPoisonZombie>(zombie);
		npc.bXenoInfectedSpecialHurt = false;
	}
	return Plugin_Handled;
}

public void XenoFortifiedPoisonZombie_NPCDeath(int entity)
{
	XenoFortifiedPoisonZombie npc = view_as<XenoFortifiedPoisonZombie>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
