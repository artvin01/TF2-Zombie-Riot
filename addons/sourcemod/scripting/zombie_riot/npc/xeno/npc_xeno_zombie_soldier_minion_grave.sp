...
public void XenoSoldierMinion_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Xeno Soldier Minion");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_xeno_zombie_soldier_minion_grave");
...
{
	return XenoSoldierMinion(vecPos, vecAng, team);
}
methodmap XenoSoldierMinion < CClotBody
{
	...
	public XenoSoldierMinion(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = XenoSoldierMinion_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = XenoSoldierMinion_OnTakeDamage;
		func_NPCThink[npc.index] = XenoSoldierMinion_ClotThink;		
...
public void XenoSoldierMinion_NPCDeath(int entity)
{
	XenoSoldierMinion npc = view_as<XenoSoldierMinion>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
