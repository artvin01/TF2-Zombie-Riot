...
public void XenoSpyCloaked_OnMapStart_NPC()
{
...
methodmap XenoSpyCloaked < CClotBody
{
	...
	public XenoSpyCloaked(int client, float vecPos[3], float vecAng[3])
	{
		XenoSpyCloaked npc = view_as<XenoSpyCloaked>(CClotBody(vecPos, vecAng, "models/player/spy.mdl", "1.0", "16500"));
...
public void XenoSpyCloaked_ClotThink(int iNPC)
{
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
			...
			if(npc.m_flNextRangedAttack < GetGameTime() && flDistanceToTarget > 22500 && flDistanceToTarget < 62500 && npc.m_flReloadDelay < GetGameTime())
			{
				int target;
			...
			//	npc.FaceTowards(vecTarget, 1000.0);
				...
				if(npc.m_flNextMeleeAttack < GetGameTime() && flDistanceToTarget < 10000)
				{
...
public Action XenoSpyCloaked_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	XenoSpyCloaked npc = view_as<XenoSpyCloaked>(victim);
...
public void XenoSpyCloaked_NPCDeath(int entity)
{
	XenoSpyCloaked npc = view_as<XenoSpyCloaked>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
