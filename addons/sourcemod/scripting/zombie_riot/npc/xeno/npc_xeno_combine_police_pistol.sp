...
methodmap XenoCombinePolicePistol < CClotBody
{
	...
	public XenoCombinePolicePistol(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		...
		SDKHook(npc.index, SDKHook_OnTakeDamage, XenoCombinePolicePistol_ClotDamaged);
...
public Action XenoCombinePolicePistol_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	XenoCombinePolicePistol npc = view_as<XenoCombinePolicePistol>(victim);
...
public Action XenoCombinePolicePistol_Revert_Poison_Zombie_Resistance(Handle timer, int ref)
{
...
public Action XenoCombinePolicePistol_Revert_Poison_Zombie_Resistance_Enable(Handle timer, int ref)
{
	int zombie = EntRefToEntIndex(ref);
	if(IsValidEntity(zombie))
	{
...
public void XenoCombinePolicePistol_NPCDeath(int entity)
{
	XenoCombinePolicePistol npc = view_as<XenoCombinePolicePistol>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
	
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, XenoCombinePolicePistol_ClotDamaged);
