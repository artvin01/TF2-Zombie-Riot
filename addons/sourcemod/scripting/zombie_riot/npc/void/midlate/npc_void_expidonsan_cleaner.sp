...
void VoudExpidonsanCleaner_OnMapStart_NPC()
{
...
methodmap VoudExpidonsanCleaner < CClotBody
{
	...
	public VoudExpidonsanCleaner(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(VoudExpidonsanCleaner_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(VoudExpidonsanCleaner_OnTakeDamage);
		...
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop_partner/player/items/pyro/as_pyro_cleansuit/as_pyro_cleansuit.mdl");
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/pyro/hazeguard/hazeguard.mdl");
...
public void VoudExpidonsanCleaner_ClotThink(int iNPC)
{
	VoudExpidonsanCleaner npc = view_as<VoudExpidonsanCleaner>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		SetGoalVectorIndex = VoudExpidonsanCleanerSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
...
int VoudExpidonsanCleanerSelfDefense(VoudExpidonsanCleaner npc, float gameTime, int target, float distance)
{
...
public Action VoudExpidonsanCleaner_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	VoudExpidonsanCleaner npc = view_as<VoudExpidonsanCleaner>(victim);
...
public void VoudExpidonsanCleaner_NPCDeath(int entity)
{
	VoudExpidonsanCleaner npc = view_as<VoudExpidonsanCleaner>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
public void ExpidonsaCleaner_Rocket_Particle_StartTouch(int entity, int target)
{
