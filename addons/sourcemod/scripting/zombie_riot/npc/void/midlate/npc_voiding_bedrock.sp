...
void VoidingBedrock_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Voiding Bedrock");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_voiding_bedrock");
...
methodmap VoidingBedrock < CClotBody
{
	...
	public VoidingBedrock(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(VoidingBedrock_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(VoidingBedrock_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(VoidingBedrock_ClotThink);
		...
		npc.m_iWearable1 = npc.EquipItem("head", "models/workshop/player/items/soldier/hw2013_rocket_ranger/hw2013_rocket_ranger.mdl");
		
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/soldier/hw2013_jupiter_jumpers/hw2013_jupiter_jumpers.mdl");
...
public void VoidingBedrock_ClotThink(int iNPC)
{
	VoidingBedrock npc = view_as<VoidingBedrock>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		VoidingBedrockSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
...
public Action VoidingBedrock_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	VoidingBedrock npc = view_as<VoidingBedrock>(victim);
...
public void VoidingBedrock_NPCDeath(int entity)
{
	VoidingBedrock npc = view_as<VoidingBedrock>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void VoidingBedrockSelfDefense(VoidingBedrock npc, float gameTime, int target, float distance)
{
