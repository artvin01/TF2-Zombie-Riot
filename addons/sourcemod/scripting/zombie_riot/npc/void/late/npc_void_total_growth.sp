...
void VoidTotalGrowth_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Total Growth");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_void_total_growth");
...
methodmap VoidTotalGrowth < CClotBody
{
	...
	public VoidTotalGrowth(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(VoidTotalGrowth_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(VoidTotalGrowth_OnTakeDamage);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/player/items/soldier/chief_rocketeer.mdl");
...
public void VoidTotalGrowth_ClotThink(int iNPC)
{
	VoidTotalGrowth npc = view_as<VoidTotalGrowth>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		VoidTotalGrowthSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
...
public Action VoidTotalGrowth_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	VoidTotalGrowth npc = view_as<VoidTotalGrowth>(victim);
...
public void VoidTotalGrowth_NPCDeath(int entity)
{
	VoidTotalGrowth npc = view_as<VoidTotalGrowth>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void VoidTotalGrowthSelfDefense(VoidTotalGrowth npc, float gameTime, int target, float distance)
{
