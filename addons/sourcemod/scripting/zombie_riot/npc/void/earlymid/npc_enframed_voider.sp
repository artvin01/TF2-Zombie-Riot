...
void VoidEnFramedVoider_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Enframed Voider");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_enframed_voider");
...
methodmap VoidEnFramedVoider < CClotBody
{
	...
	{
		VoidEnFramedVoider npc = view_as<VoidEnFramedVoider>(CClotBody(vecPos, vecAng, "models/player/soldier.mdl", "1.0", "700", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(VoidEnFramedVoider_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(VoidEnFramedVoider_OnTakeDamage);
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/weapons/c_models/c_paintrain/c_paintrain.mdl");
...
public void VoidEnFramedVoider_ClotThink(int iNPC)
{
	VoidEnFramedVoider npc = view_as<VoidEnFramedVoider>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		VoidEnFramedVoiderSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
		npc.m_flGetClosestTargetTime = 0.0;
...
public Action VoidEnFramedVoider_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	VoidEnFramedVoider npc = view_as<VoidEnFramedVoider>(victim);
...
public void VoidEnFramedVoider_NPCDeath(int entity)
{
	VoidEnFramedVoider npc = view_as<VoidEnFramedVoider>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void VoidEnFramedVoiderSelfDefense(VoidEnFramedVoider npc, float gameTime, int target, float distance)
{
