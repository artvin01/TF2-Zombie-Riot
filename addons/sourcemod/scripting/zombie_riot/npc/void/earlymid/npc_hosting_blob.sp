...
void VoidHostingBlob_OnMapStart_NPC()
{
...
methodmap VoidHostingBlob < CClotBody
{
	...
	public VoidHostingBlob(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(VoidHostingBlob_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(VoidHostingBlob_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(VoidHostingBlob_ClotThink);
		...
		npc.m_iWearable1 = npc.EquipItem("head", "models/player/items/pyro/pyro_brainhead_s1.mdl");
...
public void VoidHostingBlob_ClotThink(int iNPC)
{
	VoidHostingBlob npc = view_as<VoidHostingBlob>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		VoidHostingBlobSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
...
public Action VoidHostingBlob_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	VoidHostingBlob npc = view_as<VoidHostingBlob>(victim);
...
public void VoidHostingBlob_NPCDeath(int entity)
{
	VoidHostingBlob npc = view_as<VoidHostingBlob>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void VoidHostingBlobSelfDefense(VoidHostingBlob npc, float gameTime, int target, float distance)
{
