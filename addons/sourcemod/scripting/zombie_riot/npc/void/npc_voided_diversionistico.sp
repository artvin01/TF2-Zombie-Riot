...
methodmap VoidedDiversionistico < CClotBody
{
	...
	public VoidedDiversionistico(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		...
		func_NPCDeath[npc.index] = VoidedDiversionistico_NPCDeath;
		...
		npc.m_iWearable5 = npc.EquipItem("head", "models/player/items/spy/spy_zombie.mdl");
			...
			if(LastSpawnDiversio < GetGameTime())
			{
				...
				for(int client_check=1; client_check<=MaxClients; client_check++)
				{
					if(IsClientInGame(client_check) && !IsFakeClient(client_check))
					{
						...
						ShowGameText(client_check, "voice_player", 1, "%t", "Voided Diversionistico Spawn");
					}
				}
			}
...
public void VoidedDiversionistico_ClotThink(int iNPC)
{
	VoidedDiversionistico npc = view_as<VoidedDiversionistico>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		switch(AntiCheeseReply)
		{
			case 0:
			{
				VoidedDiversionisticoSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
			}
			case 1:
			{
				npc.m_flAttackHappens = 0.0;
				VoidedDiversionisticoSelfDefenseRanged(npc,GetGameTime(npc.index), npc.m_iTarget); 
			}
		}
	}
	else
...
public Action VoidedDiversionistico_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	VoidedDiversionistico npc = view_as<VoidedDiversionistico>(victim);
...
public void VoidedDiversionistico_NPCDeath(int entity)
{
	VoidedDiversionistico npc = view_as<VoidedDiversionistico>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void VoidedDiversionisticoSelfDefenseRanged(VoidedDiversionistico npc, float gameTime, int target)
{
...
void VoidedDiversionisticoSelfDefense(VoidedDiversionistico npc, float gameTime, int target, float distance)
{
