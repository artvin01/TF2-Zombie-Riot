...
void VoidHardCrust_OnMapStart_NPC()
{
...
methodmap VoidHardCrust < CClotBody
{
	...
	public VoidHardCrust(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		...
		{
			npc.m_iOverlordComboAttack = 150;
		}
		...
		func_NPCDeath[npc.index] = view_as<Function>(VoidHardCrust_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(VoidHardCrust_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(VoidHardCrust_ClotThink);
...
public void VoidHardCrust_ClotThink(int iNPC)
{
	VoidHardCrust npc = view_as<VoidHardCrust>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		VoidHardCrustSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
...
public Action VoidHardCrust_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	VoidHardCrust npc = view_as<VoidHardCrust>(victim);
...
public void VoidHardCrust_NPCDeath(int entity)
{
	VoidHardCrust npc = view_as<VoidHardCrust>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void VoidHardCrustSelfDefense(VoidHardCrust npc, float gameTime, int target, float distance)
{
