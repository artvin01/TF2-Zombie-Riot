...
methodmap BunkerHeadlessHorse < CClotBody
{
	...
	public BunkerHeadlessHorse(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		...
		IllScareYouToDeath[npc.index] = 25.0 + GetGameTime();
...
//	PrintToChatAll("%.f",GetEntPropFloat(view_as<int>(iNPC), Prop_Data, "m_speed"));
	...
	if(npc.m_bDoSpawnGesture)
	{
		...
		npc.PlayHeIsAwake();
		WakeTheFUCKUp[npc.index] = true && GetGameTime() + 6.0;
	...
	if(IllScareYouToDeath[npc.index] <= GetGameTime() && !TimeToScare[npc.index])
	{
		npc.AddGesture("ACT_MP_GESTURE_VC_HANDMOUTH_ITEM1");
		npc.m_flSpeed = 0.0;
		IllScareYouToDeath[npc.index] = GetGameTime() + 25.0;
		TimeToScare[npc.index] = true;
		Resetspeed[npc.index] = GetGameTime() + 1.2;
		npc.PlaySpookEm();
	...
	if(Resetspeed[npc.index] <= GetGameTime() && TimeToScare[npc.index])
	{
		npc.m_flSpeed = 300.0;
		TimeToScare[npc.index] = false;
	}
		...
		if(flDistanceToTarget < 10000 || npc.m_flAttackHappenswillhappen) //Target close enough to hit
		{
			...
			if(npc.m_flNextMeleeAttack < GetGameTime())
			{
				if (!npc.m_flAttackHappenswillhappen)//Play attack anims
				{
					...
					npc.m_flAttackHappens_bullshit = GetGameTime()+0.75;
				...
				if (npc.m_flAttackHappens < GetGameTime() && npc.m_flAttackHappens_bullshit >= GetGameTime() && npc.m_flAttackHappenswillhappen)
				{
					...
					delete swingTrace;
					npc.m_flNextMeleeAttack = GetGameTime() + 0.75;
