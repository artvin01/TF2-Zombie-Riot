...
methodmap PabloGonzalos < CClotBody
{
	...
	public PabloGonzalos(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		...
		if(FinalGunTimer[npc.index] <= GetGameTime() && !FinalGunReady[npc.index])
		{
			...
			if(npc.m_flNextRangedAttack < GetGameTime() && flDistanceToTarget < 1240000 && npc.m_flReloadDelay < GetGameTime())
			{
				...
				else
				{
					...
					{
						FireBullet(npc.index, npc.m_iWearable1, WorldSpaceCenter(npc.index), vecDir, 300.0, 9000.0, DMG_BULLET, "bullet_tracer01_red");
					}
					npc.PlayRangedSound();
				}
			}
