...
#define BONES_SLUGGER_HP			"1500"
#define BONES_SLUGGER_HP_BUFFED	"4000"

#define BONES_SLUGGER_SKIN		"2"
#define BONES_SLUGGER_SKIN_BUFFED	"0"

#define BONES_SLUGGER_SCALE		 "1.0"
#define BONES_SLUGGER_SCALE_BUFFED "1.2"
...
static float BONES_SLUGGER_SPEED_BUFFED_RAMPAGE = 440.0;
static float SLUGGER_NATURAL_BUFF_CHANCE = 0.1;	//Percentage chance for non-buffed skeletons of this type to be naturally buffed instead.
static float SLUGGER_NATURAL_BUFF_LEVEL_MODIFIER = 0.1;	//Max percentage increase for natural buff chance based on the average level of all players in the lobby, relative to natural_buff_level.
static float SLUGGER_NATURAL_BUFF_LEVEL = 100.0;	//The average level at which level_modifier reaches its max.

static float BONES_SLUGGER_PLAYERDAMAGE = 60.0;
static float BONES_SLUGGER_PLAYERDAMAGE_BUFFED = 90.0;
static float BONES_SLUGGER_PLAYERDAMAGE_BUFFED_RAMPAGE = 90.0;

static float BONES_SLUGGER_BUILDINGDAMAGE = 120.0;
static float BONES_SLUGGER_BUILDINGDAMAGE_BUFFED = 200.0;
static float BONES_SLUGGER_BUILDINGDAMAGE_BUFFED_RAMPAGE = 300.0;

static float BONES_SLUGGER_ATTACKINTERVAL = 1.2;
...
public void SluggerBones_OnMapStart_NPC()
{
	...
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_slugger");
	...
	NPCData data_buffed;
	strcopy(data_buffed.Name, sizeof(data_buffed.Name), "Spinal Slugger");
	strcopy(data_buffed.Plugin, sizeof(data_buffed.Plugin), "npc_slugger_buffed");
...
methodmap SluggerBones < CClotBody
{
	public void PlayIdleSound() {
		...
		PrintToServer("CSluggerBones::PlayIdleSound()");
		#endif
	}
	...
	public void PlayHurtSound() {
		...
		#if defined DEBUG_SOUND
		PrintToServer("CSluggerBones::PlayHurtSound()");
		#endif
	}
	...
	public void PlayDeathSound() {
		...
		#if defined DEBUG_SOUND
		PrintToServer("CSluggerBones::PlayDeathSound()");
		#endif
	}
	...
	public void PlayGibSound() {
		...
		#if defined DEBUG_SOUND
		PrintToServer("CSluggerBones::PlayGibSound()");
		#endif
	}
	...
	public void PlayHeIsAwake() {
		...
		#if defined DEBUG_SOUND
		PrintToServer("CSluggerBones::PlayHeIsAwakeSound()");
		#endif
	}
	...
	public SluggerBones(int client, float vecPos[3], float vecAng[3], int ally, bool buffed)
	{
		if (!buffed)
		{
			float chance = SLUGGER_NATURAL_BUFF_CHANCE;
			if (SLUGGER_NATURAL_BUFF_LEVEL_MODIFIER > 0.0)
			{
				float total;
				float players;
				...
				float mult = average / SLUGGER_NATURAL_BUFF_LEVEL;
				if (mult > 1.0)
					mult = 1.0;
					
				chance += (mult * SLUGGER_NATURAL_BUFF_LEVEL_MODIFIER);
			}
		...
		func_NPCDeath[npc.index] = view_as<Function>(SluggerBones_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(SluggerBones_OnTakeDamage);
		...
		Slugger_GiveCosmetics(npc, buffed);
		...
		//IDLE
		npc.m_flSpeed = (buffed ? BONES_SLUGGER_SPEED_BUFFED : BONES_SLUGGER_SPEED);
...
stock void Slugger_GiveCosmetics(CClotBody npc, bool buffed)
{
	...
	if (buffed)
	{
		...
		DispatchKeyValue(npc.index, "skin", BONES_SLUGGER_SKIN_BUFFED);
	}
	else
	{
		...
		DispatchKeyValue(npc.index, "skin", BONES_SLUGGER_SKIN);
	}
}
...
public void SluggerBones_SetBuffed(int index, bool buffed)
{
	...
	if (!b_BonesBuffed[index] && buffed)
	{
		...
		int HP = StringToInt(BONES_SLUGGER_HP_BUFFED);
	...
	else if (b_BonesBuffed[index] && !buffed)
	{
		...
		int HP = StringToInt(BONES_SLUGGER_HP);
		SetEntProp(index, Prop_Data, "m_iMaxHealth", HP);
		...
		SluggerBones_RemoveRampageParticle(index);

		if (b_SluggerRampage[index])
		{
			b_SluggerRampage[index] = false;
			view_as<SluggerBones>(index).PlayRampageEnd();
			npc.SetPlaybackRate(1.0);
		}
	}
}

public void SluggerBones_ApplyRampageParticle(int index)
{
	TE_SetupParticleEffect(BONES_SLUGGER_RAMPAGEPARTICLE, PATTACH_ABSORIGIN_FOLLOW, index);
	TE_WriteNum("m_bControlPoint1", index);	
	TE_SendToAll();
}

public void SluggerBones_RemoveRampageParticle(int index)
{
	TE_Start("EffectDispatch");
	TE_WriteNum("entindex", index);
	TE_WriteNum("m_nHitBox", GetParticleEffectIndex(BONES_SLUGGER_RAMPAGEPARTICLE));
...
//TODO 
//Rewrite
public void SluggerBones_ClotThink(int iNPC)
{
	SluggerBones npc = view_as<SluggerBones>(iNPC);
	npc.Update();
//	PrintToChatAll("%.f",GetEntPropFloat(view_as<int>(iNPC), Prop_Data, "m_speed"));
	...
	if (b_BonesBuffed[npc.index])
	{
		...
		if (b_SluggerRampage[npc.index] && percentage > BONES_SLUGGER_RAMPAGE_THRESHOLD)
		{
			SluggerBones_RemoveRampageParticle(npc.index);
			npc.m_flSpeed = BONES_SLUGGER_SPEED_BUFFED;
			b_SluggerRampage[npc.index] = false;
			npc.PlayRampageEnd();
			npc.SetPlaybackRate(1.0);
		}
		else if (!b_SluggerRampage[npc.index] && percentage <= BONES_SLUGGER_RAMPAGE_THRESHOLD)
		{
			SluggerBones_ApplyRampageParticle(npc.index);
			npc.m_flSpeed = BONES_SLUGGER_SPEED_BUFFED_RAMPAGE;
			b_SluggerRampage[npc.index] = true;
...
public Action SluggerBones_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;

	SluggerBones npc = view_as<SluggerBones>(victim);
...
public void SluggerBones_NPCDeath(int entity)
{
