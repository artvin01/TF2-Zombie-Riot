...
void InfectedHeavy_Precache()
{
	...
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_zs_zombie_heavy");
...
{
	return InfectedHeavy(vecPos, vecAng, team);
}

methodmap InfectedHeavy < CClotBody
{
	...
	public InfectedHeavy(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = InfectedHeavy_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = InfectedHeavy_OnTakeDamage;
		func_NPCThink[npc.index] = InfectedHeavy_ClotThink;
...
void InfectedHeavy_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	if(attacker > 0)
	{
		InfectedHeavy npc = view_as<InfectedHeavy>(victim);
