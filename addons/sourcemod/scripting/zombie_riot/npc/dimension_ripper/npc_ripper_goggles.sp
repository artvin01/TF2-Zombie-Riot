...
void RipperGoggles_OnMapStart()
{
	...
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ripper_goggles");
...
{
	return RipperGoggles(client, vecPos, vecAng, ally);
}

methodmap RipperGoggles < StalkerShared
{
	...
	public RipperGoggles(int client, float vecPos[3], float vecAng[3], int ally)
	{
		RipperGoggles npc = view_as<RipperGoggles>(CClotBody(vecPos, vecAng, "models/bots/sniper/bot_sniper.mdl", "1.0", "66666666", ally));
		...
		func_NPCDeath[npc.index] = RipperGoggles_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = RipperGoggles_OnTakeDamage;
		func_NPCThink[npc.index] = RipperGoggles_ClotThink;
...
public void RipperGoggles_ClotThink(int iNPC)
{
	RipperGoggles npc = view_as<RipperGoggles>(iNPC);
				...
				case 0, 1, 2:
				{
					if(sniper)
					{
						...
						if(state)
						{
							...
							npc.PlayRangedSound();
							npc.FireArrow(vecTarget, 400.0, 3500.0);
...
public Action RipperGoggles_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	RipperGoggles npc = view_as<RipperGoggles>(victim);
...
void RipperGoggles_NPCDeath(int entity)
{
	RipperGoggles npc = view_as<RipperGoggles>(entity);
