...
static void Internal_ClotThink(int iNPC)
{
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
		...
		{	//necksnap
			...
			CPrintToChatAll("{blue}카멜슈타인{default}: {crimson}널 박살내주마.");
		...
		{
			CPrintToChatAll("{blue}카멜슈타인{default}: 이러고 있을 시간 없다.");
		...
		if(fl_kahml_nano_reset[npc.index] <= GameTime && b_kahml_inNANOMACHINESSON[npc.index])
		{
			...
			CPrintToChatAll("{blue}카멜슈타인{default}: 잘 살아남았군. 잘 했다.");
...
static void Internal_NPCDeath(int entity)
{
	...
	CPrintToChatAll("{blue}카멜슈타인{default}: 점점 지겹군. 잘 있어라.");
