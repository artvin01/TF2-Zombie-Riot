...
{
	return Schwertkrieg(vecPos, vecAng, team, data);
}
...
static void Internal_ClotThink(int iNPC)
{
	...
	if(!IsValidEntity(RaidBossActive) && !g_b_schwert_died && i_RaidGrantExtra[npc.index] == 1)
	{
		RaidBossActive=EntIndexToEntRef(npc.index);
	}
	else
	{
		if(EntRefToEntIndex(RaidBossActive)==npc.index && g_b_schwert_died && i_RaidGrantExtra[npc.index] == 1)
		{
			RaidBossActive = INVALID_ENT_REFERENCE;
		}
	}
