...
static void Internal_ClotThink(int iNPC)
{
	...
	if(ZR_Waves_GetRound()+1 >=60 && EntRefToEntIndex(RaidBossActive)==npc.index && i_RaidGrantExtra[npc.index] == 1)	//donnerkrieg handles the timer if its the same index
	{
	...
	if(!IsValidEntity(RaidBossActive) && !g_b_donner_died && ZR_Waves_GetRound()+1 >=60 && i_RaidGrantExtra[npc.index] == 1)
	{
		RaidBossActive=EntIndexToEntRef(npc.index);
	}
	else
	{
		if(ZR_Waves_GetRound()+1 >=60 && EntRefToEntIndex(RaidBossActive)==npc.index && g_b_donner_died && i_RaidGrantExtra[npc.index] == 1)
		{
			RaidBossActive = INVALID_ENT_REFERENCE;
		}
	}
