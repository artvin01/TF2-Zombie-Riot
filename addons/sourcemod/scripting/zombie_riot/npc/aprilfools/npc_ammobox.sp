...
static const char NPCModel[] = "models/items/ammocrate_smg1.mdl";
...
void AmmoBox_OnMapStart()
{
	...
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ammobox");
	strcopy(data.Icon, sizeof(data.Icon), "ammobox");
...
methodmap AmmoBox < CClotBody
{
	...
	public AmmoBox(float vecPos[3], int ally, const char[] data)
	{
		...
		func_NPCDeath[npc.index] = AmmoBox_NPCDeath;
		func_NPCThink[npc.index] = AmmoBox_ClotThink;
		...
		SDKHook(npc.index, SDKHook_Touch, AmmoTouchDamageTouch);
		
		return npc;
	}
}

public void AmmoTouchDamageTouch(int entity, int other)
{
...
public void AmmoBox_ClotThink(int iNPC)
{
	AmmoBox npc = view_as<AmmoBox>(iNPC);
...
void AmmoBox_NPCDeath(int entity)
{
	AmmoBox npc = view_as<AmmoBox>(entity);
	if(!npc.m_bGib)
	{
//		npc.PlayDeathSound();	
	}
...
public Action Ammo_Spawner_Delay(Handle timer, DataPack pack)
{
