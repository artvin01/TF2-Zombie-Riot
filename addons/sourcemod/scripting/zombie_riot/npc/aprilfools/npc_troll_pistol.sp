...
public void TrollPistol_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Troll Cop");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_troll_pistol");
...
{
	return TrollPistol(vecPos, vecAng, team);
}
methodmap TrollPistol < CClotBody
{
	...
	public TrollPistol(float vecPos[3], float vecAng[3], int ally)
	{
		TrollPistol npc = view_as<TrollPistol>(CClotBody(vecPos, vecAng, "models/zombie_riot/trollface/trollface.mdl", "1.15", "550", ally, false));
		
		i_NpcWeight[npc.index] = 1;
		
		int iActivity = npc.LookupActivity("ACT_RUN_PISTOL");
...
public void TrollPistol_ClotThink(int iNPC)
{
	TrollPistol npc = view_as<TrollPistol>(iNPC);
...
public Action TrollPistol_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	TrollPistol npc = view_as<TrollPistol>(victim);
...
public void TrollPistol_NPCDeath(int entity)
{
	TrollPistol npc = view_as<TrollPistol>(entity);
