...
methodmap SpiritRunner < CClotBody
{
	...
	public SpiritRunner(float vecPos[3], float vecAng[3], int ally)
	{
		SpiritRunner npc = view_as<SpiritRunner>(CClotBody(vecPos, vecAng, "models/zombie_riot/cof/sawrunner_2.mdl", "1.35", GetSpiritRunnerHealth(), ally, false, true, true));
		...
		func_NPCDeath[npc.index] = SpiritRunner_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = SpiritRunner_OnTakeDamage;
		func_NPCThink[npc.index] = SpiritRunner_ClotThink;
		...
		SetEntityRenderColor(npc.index, 0, 0, 0, 75);
...
public void SpiritRunner_ClotThink(int iNPC)
{
	SpiritRunner npc = view_as<SpiritRunner>(iNPC);
...
public Action SpiritRunner_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	SpiritRunner npc = view_as<SpiritRunner>(victim);
...
public void SpiritRunner_NPCDeath(int entity)
{
	SpiritRunner npc = view_as<SpiritRunner>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
		
	Music_Stop_All_Spiritrunner(entity);
...
//		GetEntPropString(client, Prop_Data, "m_ModelName", model, sizeof(model));
		...
		CreateTimer(2.0, Timer_RemoveEntitySpiritrunner, EntIndexToEntRef(entity_death), TIMER_FLAG_NO_MAPCHANGE);
		CreateTimer(0.7, Timer_RemoveEntitySpiritrunner_Tantrum, EntIndexToEntRef(entity_death), TIMER_FLAG_NO_MAPCHANGE);
...
public Action Timer_RemoveEntitySpiritrunner(Handle timer, any entid)
{
...
public Action Timer_RemoveEntitySpiritrunner_Tantrum(Handle timer, any entid)
{
	int entity = EntRefToEntIndex(entid);
	if(IsValidEntity(entity) && entity>MaxClients)
	{
...
void Music_Stop_All_Spiritrunner(int entity)
{
