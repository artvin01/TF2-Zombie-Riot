...
public void TrollAr2_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Troll Rifler");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_troll_ar2");
...
{
	return TrollAr2(vecPos, vecAng, team);
}
methodmap TrollAr2 < CClotBody
{
	...
	public TrollAr2(float vecPos[3], float vecAng[3], int ally)
	{
		TrollAr2 npc = view_as<TrollAr2>(CClotBody(vecPos, vecAng, "models/zombie_riot/trollface/trollface.mdl", "1.15", "1250", ally));
		...
		func_NPCDeath[npc.index] = TrollAr2_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = TrollAr2_OnTakeDamage;
		func_NPCThink[npc.index] = TrollAr2_ClotThink;
...
public void TrollAr2_ClotThink(int iNPC)
{
	TrollAr2 npc = view_as<TrollAr2>(iNPC);
...
public Action TrollAr2_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	TrollAr2 npc = view_as<TrollAr2>(victim);
...
public void TrollAr2_NPCDeath(int entity)
{
	TrollAr2 npc = view_as<TrollAr2>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
