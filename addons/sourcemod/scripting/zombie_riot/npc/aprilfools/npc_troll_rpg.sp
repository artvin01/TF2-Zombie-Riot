...
public void TrollRPG_OnMapStart_NPC()
{
	PrecacheModel("models/zombie_riot/trollface/trollface.mdl");
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Troll RPG Carrier");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_troll_rpg");
	strcopy(data.Icon, sizeof(data.Icon), "troll");
...
{
	return TrollRPG(vecPos, vecAng, team);
}
methodmap TrollRPG < CClotBody
{
	...
	public TrollRPG(float vecPos[3], float vecAng[3], int ally)
	{
		TrollRPG npc = view_as<TrollRPG>(CClotBody(vecPos, vecAng, "models/zombie_riot/trollface/trollface.mdl", "1.15", "550", ally, false));
...
public void TrollRPG_ClotThink(int iNPC)
{
	TrollRPG npc = view_as<TrollRPG>(iNPC);
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
			...
			if (npc.m_fbGunout == false && npc.m_flReloadDelay < GetGameTime(npc.index))
			{
				if (!npc.m_bmovedelay)
				{
					int iActivity_melee = npc.LookupActivity("ACT_RUN_ON_FIRE");
			...
			{
				int iActivity_melee = npc.LookupActivity("ACT_IDLE_ANGRY_RPG");
				if(iActivity_melee > 0) npc.StartActivity(iActivity_melee);
				npc.m_bmovedelay = false;
			...
			if(npc.m_flNextRangedAttack < GetGameTime(npc.index) && flDistanceToTarget > 62500 && flDistanceToTarget < 122500 && npc.m_flReloadDelay < GetGameTime(npc.index))
			{
				...
				else
				{
					...
					npc.FireRocket(vecTarget, 100.0, 1100.0);
...
public Action TrollRPG_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	TrollRPG npc = view_as<TrollRPG>(victim);
...
public void TrollRPG_NPCDeath(int entity)
{
	TrollRPG npc = view_as<TrollRPG>(entity);
