...
void SuctionMedic_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Suction Medic");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_suction_medic");
...
{
	return SuctionMedic(vecPos, vecAng, ally);
}
methodmap SuctionMedic < CClotBody
{
	...
	public SuctionMedic(float vecPos[3], float vecAng[3], int ally)
	{
		SuctionMedic npc = view_as<SuctionMedic>(CClotBody(vecPos, vecAng, "models/player/medic.mdl", "1.0", "30000", ally));
		...
		func_NPCDeath[npc.index] = SuctionMedic_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = SuctionMedic_OnTakeDamage;
		func_NPCThink[npc.index] = SuctionMedic_ClotThink;
...
//TODO 
//Rewrite
public void SuctionMedic_ClotThink(int iNPC)
{
	SuctionMedic npc = view_as<SuctionMedic>(iNPC);
	...
	if(!npc.m_bFUCKYOU)
	{
		...
		if(IsValidEnemy(npc.index, PrimaryThreatIndex))
		{
			...
			if(flDistanceToTarget < 250000)
			{
				...
				{
					npc.StartHealing();
					npc.m_iWearable4 = ConnectWithBeam(npc.m_iWearable3, PrimaryThreatIndex, 200, 0, 0, 3.0, 3.0, 1.35, LASERBEAM);
					npc.Healing = true;
				...
				if(npc.m_flNextRangedAttack < GetGameTime(npc.index))
				{
					...
					SDKHooks_TakeDamage(npc.m_iTarget, npc.index, npc.index, 5.0, DMG_CLUB, -1, _);
				}
...
public Action SuctionMedic_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &m_iWearable3, float damageForce[3], float damagePosition[3], int damagecustom)
{
	SuctionMedic npc = view_as<SuctionMedic>(victim);
...
public void SuctionMedic_NPCDeath(int entity)
{
	SuctionMedic npc = view_as<SuctionMedic>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
