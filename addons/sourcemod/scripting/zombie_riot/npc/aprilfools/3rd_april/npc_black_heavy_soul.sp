...
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "BlackHeavySoul");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_black_heavy_soul");
	strcopy(data.Icon, sizeof(data.Icon), "BlackHeavySoul_raid");
...
methodmap BlackHeavySoul < CClotBody
{
	...
	public void PlayIdleAlertSound() 
	{
		...
		EmitSoundToAll(g_IdleAlertedSounds[GetRandomInt(0, sizeof(g_IdleAlertedSounds) - 1)], this.index, SNDCHAN_STATIC, RAIDBOSS_ZOMBIE_SOUNDLEVEL, _, BOSS_ZOMBIE_VOLUME);y
		this.m_flNextIdleSound = GetGameTime(this.index) + GetRandomFloat(6.0, 9.0);
	...
	{
		BlackHeavySoul npc = view_as<BlackHeavySoul>(CClotBody(vecPos, vecAng, "models/player/heavy.mdl", "1.35", "40000", ally, false, true, true,true)); //giant!
