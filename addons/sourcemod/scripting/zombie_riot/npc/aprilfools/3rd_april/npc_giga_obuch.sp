...
void GigaObuch_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Giga Obuch");
...
{
	return GigaObuch(vecPos, vecAng, team);
}
methodmap GigaObuch < CClotBody
{
	...
	public GigaObuch(float vecPos[3], float vecAng[3], int ally)
	{
		GigaObuch npc = view_as<GigaObuch>(CClotBody(vecPos, vecAng, COMBINE_CUSTOM_MODEL, "1.75", "15000", ally, false, true));
		SetVariantInt(1);
		...
		func_NPCDeath[npc.index] = GigaObuch_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = GigaObuch_OnTakeDamage;
		func_NPCThink[npc.index] = GigaObuch_ClotThink;
...
public void GigaObuch_ClotThink(int iNPC)
{
	GigaObuch npc = view_as<GigaObuch>(iNPC);
		...
		if(npc.m_flAttackHappens < gameTime)
		{
			...
			if(IsValidEnemy(npc.index, npc.m_iTarget))
			{
				...
				if(npc.DoSwingTrace(swingTrace, npc.m_iTarget, _, _, _, _)) //Big range, but dont ignore buildings if somehow this doesnt count as a raid to be sure.
				{
					...
					npc.m_iOverlordComboAttack++; //Used to count below for attack speed increase
					...
					//Lazy way of doing it but Aperture Combatant does it like this, sooooo
...
public Action GigaObuch_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	GigaObuch npc = view_as<GigaObuch>(victim);
...
public void GigaObuch_NPCDeath(int entity)
{
	GigaObuch npc = view_as<GigaObuch>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
