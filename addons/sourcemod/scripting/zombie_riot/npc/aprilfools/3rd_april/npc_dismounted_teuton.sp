...
void DismountedTeuton_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Dismounted Teuton");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_dismounted_teuton");
...
{
	return DismountedTeuton(vecPos, vecAng, team);
}
methodmap DismountedTeuton < CClotBody
{
	...
	public DismountedTeuton(float vecPos[3], float vecAng[3], int ally)
	{
		DismountedTeuton npc = view_as<DismountedTeuton>(CClotBody(vecPos, vecAng, COMBINE_CUSTOM_MODEL, "1.15", "5000", ally));
		SetVariantInt(1);
		...
		func_NPCDeath[npc.index] = DismountedTeuton_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = DismountedTeuton_OnTakeDamage;
		func_NPCThink[npc.index] = DismountedTeuton_ClotThink;
...
public void DismountedTeuton_ClotThink(int iNPC)
{
	DismountedTeuton npc = view_as<DismountedTeuton>(iNPC);
		...
		switch(npc.m_iState)
		{
			...
			{			
				...
				if(IsValidEntity(Enemy_I_See) && IsValidEnemy(npc.index, Enemy_I_See))
				{
					...
					npc.AddGesture("ACT_TEUTON_ATTACK_NEW");
...
public Action DismountedTeuton_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	DismountedTeuton npc = view_as<DismountedTeuton>(victim);
...
public void DismountedTeuton_NPCDeath(int entity)
{
	DismountedTeuton npc = view_as<DismountedTeuton>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
