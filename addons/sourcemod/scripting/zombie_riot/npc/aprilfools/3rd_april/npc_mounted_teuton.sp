...
void MountedTeuton_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Mounted Teuton");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_mounted_teuton");
...
{
	return MountedTeuton(vecPos, vecAng, team);
}
methodmap MountedTeuton < CClotBody
{
	...
	public MountedTeuton(float vecPos[3], float vecAng[3], int ally)
	{
		MountedTeuton npc = view_as<MountedTeuton>(CClotBody(vecPos, vecAng, COMBINE_CUSTOM_MODEL, "1.15", "5000", ally));
		SetVariantInt(1);
		...
		func_NPCDeath[npc.index] = MountedTeuton_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = MountedTeuton_OnTakeDamage;
		func_NPCThink[npc.index] = MountedTeuton_ClotThink;
...
public void MountedTeuton_ClotThink(int iNPC)
{
	MountedTeuton npc = view_as<MountedTeuton>(iNPC);
...
public Action MountedTeuton_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	MountedTeuton npc = view_as<MountedTeuton>(victim);
...
public void MountedTeuton_NPCDeath(int entity)
{
	MountedTeuton npc = view_as<MountedTeuton>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
	...
	for(int i; i < 1; i++)
	{
		int other = NPC_CreateByName("npc_dismounted_teuton", -1, pos, ang, team);
		if(other > MaxClients)
		{
			if(team != TFTeam_Red)
			Zombies_Currently_Still_Ongoing++;
	
			SetEntProp(other, Prop_Data, "m_iHealth", (ReturnEntityMaxHealth(npc.index)));
			SetEntProp(other, Prop_Data, "m_iMaxHealth", (ReturnEntityMaxHealth(npc.index)));
