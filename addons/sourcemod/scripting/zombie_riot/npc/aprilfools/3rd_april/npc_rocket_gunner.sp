...
void RocketGunner_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Rocket Gunner");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_rocket_gunner");
...
{
	return RocketGunner(vecPos, vecAng, team);
}

methodmap RocketGunner < CClotBody
{
	...
	public RocketGunner(float vecPos[3], float vecAng[3], int ally)
	{
		RocketGunner npc = view_as<RocketGunner>(CClotBody(vecPos, vecAng, "models/player/heavy.mdl", "1.0", "10000", ally));
		...
		func_NPCDeath[npc.index] = RocketGunner_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = RocketGunner_OnTakeDamage;
		func_NPCThink[npc.index] = RocketGunner_ClotThink;
...
public Action RocketGunner_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	RocketGunner npc = view_as<RocketGunner>(victim);
...
public void RocketGunner_NPCDeath(int entity)
{
	RocketGunner npc = view_as<RocketGunner>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void RocketGunnerSelfDefense(RocketGunner npc)
{
	int target;
	...
	if(flDistanceToTarget < (NORMAL_ENEMY_MELEE_RANGE_FLOAT_SQUARED * 20.0))
	{
		...
		int entity = npc.FireRocket(vecTarget, 35.0, 1000.0, "models/weapons/w_bullet.mdl", 2.0);	
		int trail = Trail_Attach(entity, ARROW_TRAIL_RED, 175, 0.25, 2.0, 2.0, 5);
