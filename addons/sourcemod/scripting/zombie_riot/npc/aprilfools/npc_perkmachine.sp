...
static const char NPCModel[] = "models/props_farm/welding_machine01.mdl";
...
void PerkMachiner_OnMapStart()
{
	...
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_perkmachine");
	strcopy(data.Icon, sizeof(data.Icon), "perk");
...
methodmap PerkMachiner < CClotBody
{
	...
	public PerkMachiner(float vecPos[3], int ally, const char[] data)
	{
		...
		func_NPCDeath[npc.index] = PerkMachiner_NPCDeath;
		func_NPCThink[npc.index] = PerkMachiner_ClotThink;
		...
		SDKHook(npc.index, SDKHook_Touch, PerkTouchDamageTouch);
		
		return npc;
	}
}

public void PerkTouchDamageTouch(int entity, int other)
{
...
public void PerkMachiner_ClotThink(int iNPC)
{
	PerkMachiner npc = view_as<PerkMachiner>(iNPC);
...
void PerkMachiner_NPCDeath(int entity)
{
	PerkMachiner npc = view_as<PerkMachiner>(entity);
	if(!npc.m_bGib)
	{
//		npc.PlayDeathSound();	
	}
...
public Action Perk_Spawner_Delay(Handle timer, DataPack pack)
{
