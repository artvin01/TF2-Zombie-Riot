	...
	if(distance < (NORMAL_ENEMY_MELEE_RANGE_FLOAT_SQUARED * 5.0))
	{
		...
		if(IsValidEnemy(npc.index, Enemy_I_See))
		{
			...
			if(npc.Anger)
			{
				...
				projectile = npc.FireParticleRocket(vecTarget, 22.0, ProjectileSpeed, 150.0, "superrare_burning2", true);
			...
			{
				projectile = npc.FireParticleRocket(vecTarget, 22.0, ProjectileSpeed, 150.0, "superrare_burning1", true);
			}
...
public void AnarchyAbomination_Rocket_Particle_StartTouch(int entity, int target)
{
	if(target > 0 && target < MAXENTITIES)	//did we hit something???
	{
		...
		NPC_Ignite(target, owner,12.0, -1, 10.0);
