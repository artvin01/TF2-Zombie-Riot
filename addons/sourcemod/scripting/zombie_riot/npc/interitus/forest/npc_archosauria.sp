...
methodmap Archosauria < CClotBody
{
	...
	public Archosauria(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = view_as<Function>(Archosauria_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(Archosauria_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(Archosauria_ClotThink);
		...
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/sniper/sum21_crocodile_dandy/sum21_crocodile_dandy.mdl");
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/sniper/sum22_rocko/sum22_rocko.mdl");
		npc.m_iWearable5 = npc.EquipItem("head", "models/workshop/player/items/sniper/sf14_skinless_slashers/sf14_skinless_slashers.mdl");
...
public void Archosauria_ClotThink(int iNPC)
{
	Archosauria npc = view_as<Archosauria>(iNPC);
...
{
	Archosauria npc = view_as<Archosauria>(victim);
...
public void Archosauria_NPCDeath(int entity)
{
	Archosauria npc = view_as<Archosauria>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
int ArchosauriaSelfDefense(Archosauria npc, float gameTime)
{
