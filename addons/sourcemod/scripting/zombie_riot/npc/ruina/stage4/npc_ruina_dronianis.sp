...
void Dronianis_OnMapStart_NPC()
{
	
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Dronianis");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_dronianis");
...
methodmap Dronianis < RuinaBaseNpc
{
...
{
	Dronianis npc = view_as<Dronianis>(victim);
...
{
	Dronianis npc = view_as<Dronianis>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
