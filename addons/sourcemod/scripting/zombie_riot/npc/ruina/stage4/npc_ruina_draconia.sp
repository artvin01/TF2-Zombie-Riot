...
void Draconia_OnMapStart_NPC()
{
	

	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Draconia");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_draconia");
...
methodmap Draconia < RuinaBaseNpc
{
	...
	public Draconia(float vecPos[3], float vecAng[3], int ally)
	{
		...
		*/
		...
		fl_ruina_battery_max[npc.index] = 350.0;
...
static void ClotThink(int iNPC)
{
	...
	Ruina_Add_Battery(npc.index, 1.75);
...
static Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	Draconia npc = view_as<Draconia>(victim);
...
{
	Draconia npc = view_as<Draconia>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
