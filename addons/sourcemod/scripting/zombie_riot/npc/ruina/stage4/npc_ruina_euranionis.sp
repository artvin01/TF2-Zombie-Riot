...
void Euranionis_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Euranionis");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_euranionis");
...
methodmap Euranionis < RuinaBaseNpc
{
...
static void ClotThink(int iNPC)
{
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
		...
		Euranionis_SelfDefense(npc, GameTime, Anchor_Id);
	}
	else
	{
		npc.StopPathing();
...
{
	Euranionis npc = view_as<Euranionis>(victim);
...
{
	Euranionis npc = view_as<Euranionis>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
static void Euranionis_SelfDefense(Euranionis npc, float gameTime, int Anchor_Id)	//ty artvin
{
