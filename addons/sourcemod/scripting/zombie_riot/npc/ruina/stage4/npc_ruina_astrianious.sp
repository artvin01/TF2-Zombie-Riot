...
void Astrianious_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Astrianious");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_astrianious");
...
methodmap Astrianious < RuinaBaseNpc
{
	...
	{
		Astrianious npc = view_as<Astrianious>(CClotBody(vecPos, vecAng, "models/player/engineer.mdl", "1.35", "1250", ally, _,true));
...
static void ClotThink(int iNPC)
{
	...
	Astrianious_SelfDefense(npc, GameTime);	//note: Masters can use this method, but slaves should still use primarythreatindex rather then finding via distance.
...
{
	Astrianious npc = view_as<Astrianious>(victim);
...
{
	Astrianious npc = view_as<Astrianious>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
static void Astrianious_SelfDefense(Astrianious npc, float gameTime)
{
