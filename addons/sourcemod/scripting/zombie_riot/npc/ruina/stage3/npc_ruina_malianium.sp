...
void Malianium_OnMapStart_NPC()
{

	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Malianium");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_malianium");
...
methodmap Malianium < RuinaBaseNpc
{
...
static void ClotThink(int iNPC)
{
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
		...
		Malianium_SelfDefense(npc, GameTime, Anchor_Id);
	}
	else
	{
		npc.StopPathing();
...
static void Malianium_Effects_Attack(Malianium npc, float Target_Vec[3], int GetClosestEnemyToAttack, float flDistanceToTarget)
{
	int amt = 2;
	float Npc_Loc[3];
...
static void Malianium_SelfDefense(Malianium npc, float gameTime, int Anchor_Id)	//ty artvin
{
	...
	{	
		if(gameTime > npc.m_flNextRangedAttack)
		{
			...
			Malianium_Effects_Attack(npc, vecTarget, GetClosestEnemyToAttack, flDistanceToTarget);
...
static Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	Malianium npc = view_as<Malianium>(victim);
...
{
	Malianium npc = view_as<Malianium>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
