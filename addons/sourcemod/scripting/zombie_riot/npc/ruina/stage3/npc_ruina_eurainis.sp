...
void Eurainis_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Eurainis");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_eurainis");
...
methodmap Eurainis < RuinaBaseNpc
{
...
static void ClotThink(int iNPC)
{
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
		...
		Eurainis_SelfDefense(npc, GameTime, Anchor_Id);
	}
	else
	{
		npc.StopPathing();
...
{
	Eurainis npc = view_as<Eurainis>(victim);
...
{
	Eurainis npc = view_as<Eurainis>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
static void Eurainis_Spawn_Minnions(Eurainis npc)
{
	...
	maxhealth = RoundToNearest(maxhealth * 1.8);
...
static void Eurainis_Spawn_Self(Eurainis npc)
{
	int maxhealth = ReturnEntityMaxHealth(npc.index);
	...
	spawn_index = NPC_CreateByName("npc_ruina_eurainis", npc.index, pos, ang, GetTeam(npc.index));
...
static void Eurainis_SelfDefense(Eurainis npc, float gameTime, int Anchor_Id)	//ty artvin
{
