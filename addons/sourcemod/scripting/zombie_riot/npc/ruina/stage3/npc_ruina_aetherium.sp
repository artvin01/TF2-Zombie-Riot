...
void Aetherium_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Aetherium");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_aetherium");
...
methodmap Aetherium < RuinaBaseNpc
{
...
static void ClotThink(int iNPC)
{
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
		...
		Aetherium_SelfDefense(npc, GameTime, Anchor_Id);
	}
	else
	{
		npc.StopPathing();
...
{
	Aetherium npc = view_as<Aetherium>(victim);
...
{
	Aetherium npc = view_as<Aetherium>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
static void Func_On_Proj_Touch(int projectile, int other)
{
	...
	{
		Aetherium npc = view_as<Aetherium>(owner);
		i_laz_entity[npc.index] = INVALID_ENT_REFERENCE;
	}
...
static void Aetherium_SelfDefense(Aetherium npc, float gameTime, int Anchor_Id)	//ty artvin
{
