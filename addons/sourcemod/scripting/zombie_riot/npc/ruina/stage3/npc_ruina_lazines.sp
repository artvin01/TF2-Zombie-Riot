...
void Lazines_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Lazines");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_lazines");
	data.Category = Type_Ruina;
	data.Func = ClotSummon;
	data.Precache = ClotPrecache;
	strcopy(data.Icon, sizeof(data.Icon), "laz"); 						//leaderboard_class_(insert the name)
...
methodmap Lazines < RuinaBaseNpc
{	
		...
		if(npc.m_flNextRangedAttack < GameTime)	//Initialize the attack.
		{
			if(flDistanceToTarget<(1000.0*1000.0))
			{
				...
				{		
					...
					Projectile.damage = 80.0;
					Projectile.bonus_dmg = 150.0;
...
static void Func_On_Proj_Touch(int projectile, int other)
{
	...
	{
		Lazines npc = view_as<Lazines>(owner);
		npc.m_flAttackHappens = 0.0;
		npc.PlayRangedReloadSound();
		i_laz_entity[npc.index] = INVALID_ENT_REFERENCE;
	}
...
static Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	Lazines npc = view_as<Lazines>(victim);
...
{
	Lazines npc = view_as<Lazines>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
