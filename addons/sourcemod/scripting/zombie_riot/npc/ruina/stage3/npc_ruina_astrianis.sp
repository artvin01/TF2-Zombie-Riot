...
void Astrianis_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Astrianis");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_astrianis");
...
methodmap Astrianis < RuinaBaseNpc
{
	...
	{
		Astrianis npc = view_as<Astrianis>(CClotBody(vecPos, vecAng, "models/player/engineer.mdl", "1.35", "1250", ally, _,true));
...
static void ClotThink(int iNPC)
{
	...
	Astrianis_SelfDefense(npc, GameTime);	//note: Masters can use this method, but slaves should still use primarythreatindex rather then finding via distance.
...
{
	Astrianis npc = view_as<Astrianis>(victim);
...
{
	Astrianis npc = view_as<Astrianis>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
