...
void Draeonis_OnMapStart_NPC()
{
	

	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Draeonis");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_draeonis");
...
methodmap Draeonis < RuinaBaseNpc
{
...
static void ClotThink(int iNPC)
{
	...
	Ruina_Add_Battery(npc.index, 1.2);
...
static Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	Draeonis npc = view_as<Draeonis>(victim);
...
{
	Draeonis npc = view_as<Draeonis>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
