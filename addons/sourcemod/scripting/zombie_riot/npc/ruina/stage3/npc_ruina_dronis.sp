...
void Dronis_OnMapStart_NPC()
{
	
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Dronis");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_dronis");
...
{
	return Dronis(vecPos, vecAng, team);
}

methodmap Dronis < RuinaBaseNpc
{
	...
	public Dronis(float vecPos[3], float vecAng[3], int ally)
	{
		Dronis npc = view_as<Dronis>(CClotBody(vecPos, vecAng, "models/player/spy.mdl", "1.0", "1250", ally));
...
{
	Dronis npc = view_as<Dronis>(victim);
...
{
	Dronis npc = view_as<Dronis>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
