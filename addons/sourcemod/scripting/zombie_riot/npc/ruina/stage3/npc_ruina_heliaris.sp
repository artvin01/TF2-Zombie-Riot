...
void Heliaris_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Heliaris");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_ruina_heliaris");
...
methodmap Heliaris < RuinaBaseNpc
{
	...
	public Heliaris(float vecPos[3], float vecAng[3], int ally)
	{
		...
		npc.m_iWearable7 = npc.EquipItem("head", Items[5]);
		...
		fl_ruina_battery_max[npc.index] = 1200.0;
...
{
	Heliaris npc = view_as<Heliaris>(iNPC);
	...
	{			
		Helia_Healing_Logic(npc.index, 1200, 500.0, GameTime, 1.0);
	}
...
{
	Heliaris npc = view_as<Heliaris>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
