...
void Zapper_OnMapStart_NPC()
{
	...
	strcopy(data.Icon, sizeof(data.Icon), "victoria_zapper");
...
methodmap Zapper < CClotBody
{
	...
	public Zapper(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = Zapper_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = Zapper_OnTakeDamage;
		func_NPCThink[npc.index] = Zapper_ClotThink;
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/pyro/hwn2022_propaniac/hwn2022_propaniac.mdl");
		SetVariantString("1.0");
		AcceptEntityInput(npc.m_iWearable2, "SetModelScale");
		SetEntProp(npc.m_iWearable2, Prop_Send, "m_nSkin", 1);
		SetEntityRenderMode(npc.m_iWearable1, RENDER_TRANSCOLOR);
		SetEntityRenderColor(npc.m_iWearable1, 255, 200, 255, 255);
		
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/pyro/sum23_flamewarrior_style1/sum23_flamewarrior_style1.mdl");
		SetVariantString("1.0");
		AcceptEntityInput(npc.m_iWearable3, "SetModelScale");
		SetEntProp(npc.m_iWearable3, Prop_Send, "m_nSkin", 1);
		
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/pyro/dec23_masked_loyalty_style3/dec23_masked_loyalty_style3.mdl");
		SetVariantString("1.0");
		AcceptEntityInput(npc.m_iWearable4, "SetModelScale");
		SetEntProp(npc.m_iWearable4, Prop_Send, "m_nSkin", 1);

		npc.m_iWearable5 = npc.EquipItem("head", "models/workshop/player/items/pyro/dec23_torchers_trench_coat/dec23_torchers_trench_coat.mdl");
		SetVariantString("1.0");
...
//TODO 
//Rewrite
public void Zapper_ClotThink(int iNPC)
{
	Zapper npc = view_as<Zapper>(iNPC);
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
			...
			ZapperSelfdefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
		}
	else
	{
		npc.m_flGetClosestTargetTime = 0.0;
...
void ZapperSelfdefense(Zapper npc, float gameTime, int target, float distance)
{
...
public Action Zapper_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Zapper npc = view_as<Zapper>(victim);
...
public void Zapper_NPCDeath(int entity)
{
	Zapper npc = view_as<Zapper>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
