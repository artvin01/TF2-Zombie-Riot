...
methodmap KazimierzKnightArcher < CClotBody
{
	...
	public KazimierzKnightArcher(int client, float vecPos[3], float vecAng[3], bool ally, const char[] data)
	{
		...
		SDKHook(npc.index, SDKHook_OnTakeDamage, KazimierzKnightArcher_ClotDamaged);
...
public Action KazimierzKnightArcher_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
...
public void KazimierzKnightArcher_NPCDeath(int entity)
{
	...
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, KazimierzKnightArcher_ClotDamaged);
