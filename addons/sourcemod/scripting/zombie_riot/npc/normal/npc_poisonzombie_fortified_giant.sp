...
public void FortifiedGiantPoisonZombie_OnMapStart_NPC()
{
...
{
	return FortifiedGiantPoisonZombie(vecPos, vecAng, team);
}
...
methodmap FortifiedGiantPoisonZombie < CClotBody
{
	...
	public FortifiedGiantPoisonZombie(float vecPos[3], float vecAng[3], int ally)
	{
...
public Action FortifiedGiantPoisonZombie_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	FortifiedGiantPoisonZombie npc = view_as<FortifiedGiantPoisonZombie>(victim);
...
public void FortifiedGiantPoisonZombie_NPCDeath(int entity)
{
	FortifiedGiantPoisonZombie npc = view_as<FortifiedGiantPoisonZombie>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
