...
public void CombineSoldierShotgun_OnMapStart_NPC()
{
...
public Action CombineSoldierShotgun_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	CombineSoldierShotgun npc = view_as<CombineSoldierShotgun>(victim);
...
public void CombineSoldierShotgun_NPCDeath(int entity)
{
	CombineSoldierShotgun npc = view_as<CombineSoldierShotgun>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
