...
void Spy_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Spy Facestabber");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_zombie_spy_grave");
...
methodmap Spy < CClotBody
{
	...
	public Spy(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = Spy_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = Spy_OnTakeDamage;
		func_NPCThink[npc.index] = Spy_ClotThink;		
...
public Action Spy_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Spy npc = view_as<Spy>(victim);
...
public void Spy_NPCDeath(int entity)
{
	Spy npc = view_as<Spy>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
