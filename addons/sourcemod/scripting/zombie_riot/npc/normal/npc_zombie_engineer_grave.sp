...
void Engineer_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Engineer Deconstructor");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_zombie_engineer_grave");
...
methodmap Engineer < CClotBody
{
	...
	public Engineer(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = Engineer_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = Engineer_OnTakeDamage;
		func_NPCThink[npc.index] = Engineer_ClotThink;		
...
public Action Engineer_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Engineer npc = view_as<Engineer>(victim);
...
public void Engineer_NPCDeath(int entity)
{
	Engineer npc = view_as<Engineer>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
