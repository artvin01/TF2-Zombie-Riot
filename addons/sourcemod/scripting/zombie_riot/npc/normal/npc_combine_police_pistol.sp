...
public void Combine_Police_Pistol_OnMapStart_NPC()
{
...
methodmap Combine_Police_Pistol < CClotBody
{
	...
	public Combine_Police_Pistol(int client, float vecPos[3], float vecAng[3])
	{
		Combine_Police_Pistol npc = view_as<Combine_Police_Pistol>(CClotBody(vecPos, vecAng, "models/police.mdl", "1.15", "550", false, false));
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
			...
			if(npc.m_flNextRangedAttack < GetGameTime() && flDistanceToTarget > 62500 && flDistanceToTarget < 122500 && npc.m_flReloadDelay < GetGameTime())
			{
				...
				else
				{
					...
					{
						FireBullet(npc.index, npc.m_iWearable1, WorldSpaceCenter(npc.index), vecDir, 2.0, 9000.0, DMG_BULLET, "bullet_tracer01_red");
					}
...
public Action Combine_Police_Pistol_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	Combine_Police_Pistol npc = view_as<Combine_Police_Pistol>(victim);
...
public void CombinePolicePistol_NPCDeath(int entity)
{
