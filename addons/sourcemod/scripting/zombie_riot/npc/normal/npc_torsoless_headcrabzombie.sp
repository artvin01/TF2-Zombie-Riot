...
public void TorsolessHeadcrabZombie_OnMapStart_NPC()
{
...
methodmap TorsolessHeadcrabZombie < CClotBody
{
	...
	public TorsolessHeadcrabZombie(int client, float vecPos[3], float vecAng[3])
	{
		TorsolessHeadcrabZombie npc = view_as<TorsolessHeadcrabZombie>(CClotBody(vecPos, vecAng, "models/zombie/classic_torso.mdl", "1.15", "250", false, false));
...
//TODO 
//Rewrite
public void TorsolessHeadcrabZombie_ClotThink(int iNPC)
{
	TorsolessHeadcrabZombie npc = view_as<TorsolessHeadcrabZombie>(iNPC);
...
public Action TorsolessHeadcrabZombie_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	TorsolessHeadcrabZombie npc = view_as<TorsolessHeadcrabZombie>(victim);
...
public void TorsolessHeadcrabZombie_NPCDeath(int entity)
{
	TorsolessHeadcrabZombie npc = view_as<TorsolessHeadcrabZombie>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
