...
void CombineDDT_OnMapStart_NPC()
{
...
methodmap CombineDDT < CClotBody
{
	...
	public CombineDDT(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = CombineDDT_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = CombineDDT_OnTakeDamage;
		func_NPCThink[npc.index] = CombineDDT_ClotThink;
...
public Action CombineDDT_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	CombineDDT npc = view_as<CombineDDT>(victim);
	/*
...
public void CombineDDT_NPCDeath(int entity)
{
	CombineDDT npc = view_as<CombineDDT>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
