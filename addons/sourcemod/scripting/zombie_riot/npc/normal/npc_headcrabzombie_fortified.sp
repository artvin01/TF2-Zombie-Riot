...
public void Fortified_HeadcrabZombie_OnMapStart_NPC()
{
...
methodmap FortifiedHeadcrabZombie < CClotBody
{
	...
	public FortifiedHeadcrabZombie(int client, float vecPos[3], float vecAng[3])
	{
		FortifiedHeadcrabZombie npc = view_as<FortifiedHeadcrabZombie>(CClotBody(vecPos, vecAng, "models/zombie/classic.mdl", "1.15", "500", false, false));
...
//TODO 
//Rewrite
public void FortifiedHeadcrabZombie_ClotThink(int iNPC)
{
	FortifiedHeadcrabZombie npc = view_as<FortifiedHeadcrabZombie>(iNPC);
				...
				if (npc.m_flAttackHappens < GetGameTime() && npc.m_flAttackHappens_bullshit >= GetGameTime() && npc.m_flAttackHappenswillhappen)
				{
					...
					if(npc.DoSwingTrace(swingTrace, closest))
					{
						...
						if(target > 0) 
						{
							...
							{
								if(target <= MaxClients)
									SDKHooks_TakeDamage(target, npc.index, npc.index, 55.0, DMG_SLASH|DMG_CLUB);
								else
									SDKHooks_TakeDamage(target, npc.index, npc.index, 75.0, DMG_SLASH|DMG_CLUB);					
							}
