...
{
	return XenoAcclaimedSwordsman(vecPos, vecAng, team);
}
...
methodmap XenoAcclaimedSwordsman < CClotBody
{
	...
	public XenoAcclaimedSwordsman(float vecPos[3], float vecAng[3], int ally)
	{
	...
	if(npc.m_flNextRangedSpecialAttack)
	{
		...
		if(npc.m_flRangedSpecialDelay < gameTime)
		{
			if(IsValidEnemy(npc.index, PrimaryThreatIndex))
			{
				...
				FireBullet(npc.index, npc.index, WorldSpaceVec, vecDir, 300.0, 400.0, DMG_CLUB, "bullet_tracer02_blue", _,_,"anim_attachment_LH");
			}
