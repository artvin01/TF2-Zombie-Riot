...
public void XenoAcclaimedSwordsman_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Xeno Acclaimed Swordsman");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_xeno_acclaimed_swordsman");
...
{
	return XenoAcclaimedSwordsman(client, vecPos, vecAng, ally);
}

methodmap XenoAcclaimedSwordsman < CClotBody
{
	...
	public XenoAcclaimedSwordsman(int client, float vecPos[3], float vecAng[3], int ally)
	{
		XenoAcclaimedSwordsman npc = view_as<XenoAcclaimedSwordsman>(CClotBody(vecPos, vecAng, COMBINE_CUSTOM_MODEL, "1.25", "200000", ally));
		SetVariantInt(1);
		...
		func_NPCDeath[npc.index] = XenoAcclaimedSwordsman_NPCDeath;
	...
	if(npc.m_flNextRangedSpecialAttack)
	{
		...
		if(npc.m_flRangedSpecialDelay < gameTime)
		{
			if(IsValidEnemy(npc.index, PrimaryThreatIndex))
			{
				...
				FireBullet(npc.index, npc.index, WorldSpaceVec, vecDir, 250.0, 200.0, DMG_BULLET, "bullet_tracer02_blue", _,_,"anim_attachment_LH");
			}
...
public Action XenoAcclaimedSwordsman_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	XenoAcclaimedSwordsman npc = view_as<XenoAcclaimedSwordsman>(victim);
...
public void XenoAcclaimedSwordsman_NPCDeath(int entity)
{
	XenoAcclaimedSwordsman npc = view_as<XenoAcclaimedSwordsman>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
