...
public void XenoPatientFew_OnMapStart_NPC()
{
...
methodmap XenoPatientFew < CClotBody
{
	...
	public void PlayMeleeMissSound() {
		EmitSoundToAll(g_MeleeMissSounds[GetRandomInt(0, sizeof(g_MeleeMissSounds) - 1)], this.index, SNDCHAN_STATIC, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME, 70);
	...
	public XenoPatientFew(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = XenoPatientFew_NPCDeath;
		func_NPCThink[npc.index] = XenoPatientFew_ClotThink;
		func_NPCOnTakeDamage[npc.index] = XenoPatientFew_OnTakeDamage;
		...
		switch(npc.m_iState)
		{
			...
			{			
				...
				if(IsValidEntity(Enemy_I_See) && IsValidEnemy(npc.index, Enemy_I_See))
				{
					...
					npc.AddGesture("ACT_MELEE_ATTACK1", _,_,_,0.8);
					CreateEarthquake(vecSelf, 1.2, 150.0, 8.0, 150.0);

					npc.PlayMeleeSound();
					
					npc.m_flAttackHappens = gameTime + 1.2;
...
public Action XenoPatientFew_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;

	XenoPatientFew npc = view_as<XenoPatientFew>(victim);
//	if(!NpcStats_IsEnemySilenced(victim)) no silence
	{
...
public void XenoPatientFew_NPCDeath(int entity)
{
	XenoPatientFew npc = view_as<XenoPatientFew>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
