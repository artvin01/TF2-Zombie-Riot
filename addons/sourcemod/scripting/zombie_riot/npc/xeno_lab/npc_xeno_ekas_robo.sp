...
methodmap XenoOuroborosEkas < CClotBody
{
	...
	public XenoOuroborosEkas(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = XenoOuroborosEkas_NPCDeath;
		func_NPCThink[npc.index] = XenoOuroborosEkas_ClotThink;
		func_NPCOnTakeDamage[npc.index] = XenoOuroborosEkas_OnTakeDamage;
...
public Action XenoOuroborosEkas_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;

	XenoOuroborosEkas npc = view_as<XenoOuroborosEkas>(victim);
...
public void XenoOuroborosEkas_NPCDeath(int entity)
{
	XenoOuroborosEkas npc = view_as<XenoOuroborosEkas>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
