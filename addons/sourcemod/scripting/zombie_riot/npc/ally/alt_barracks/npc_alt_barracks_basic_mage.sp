...
methodmap Barrack_Alt_Basic_Mage < BarrackBody
{
	...
	public Barrack_Alt_Basic_Mage(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		Barrack_Alt_Basic_Mage npc = view_as<Barrack_Alt_Basic_Mage>(BarrackBody(client, vecPos, vecAng, "100", "models/player/medic.mdl", STEPTYPE_NORMAL));
		
		i_NpcInternalId[npc.index] = ALT_BARRACK_BASIC_MAGE;
		i_NpcWeight[npc.index] = 1;
		
		SDKHook(npc.index, SDKHook_Think, Barrack_Alt_Basic_Mage_ClotThink);
			...
			if(flDistanceToTarget < 100000.0)
			{
				...
				if(IsValidEnemy(npc.index, Enemy_I_See))
				{
					...
					if(npc.m_flNextMeleeAttack < GameTime)
					{
						...
						npc.FireParticleRocket(vecTarget, 300.0 * npc.BonusDamageBonus , speed+100.0 , 100.0 , "raygun_projectile_blue_crit", _, false, true, flPos, _ , GetClientOfUserId(npc.OwnerUserId));
...
void Barrack_Alt_Basic_Mage_NPCDeath(int entity)
{
	...
	SDKUnhook(npc.index, SDKHook_Think, Barrack_Alt_Basic_Mage_ClotThink);
}