...
void HighInquisitorDario_OnMapStart_NPC()
{
...
methodmap HighInquisitorDario < CClotBody
{
...
//TODO 
//Rewrite
public void HighInquisitorDario_ClotThink(int iNPC)
{
	HighInquisitorDario npc = view_as<HighInquisitorDario>(iNPC);
...
public Action HighInquisitorDario_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	HighInquisitorDario npc = view_as<HighInquisitorDario>(victim);
...
public void HighInquisitorDario_NPCDeath(int entity)
{
	HighInquisitorDario npc = view_as<HighInquisitorDario>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
