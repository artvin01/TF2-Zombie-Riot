...
void ErasusDebug_OnMapStart_NPC()
{
	...
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_erasus_debug");
...
{
	return ErasusDebug(vecPos, vecAng, team);
}

methodmap ErasusDebug < CClotBody
{
	...
	property float m_TimeSinceLastThought
	{
	...
	public ErasusDebug(float vecPos[3], float vecAng[3], int ally)
	{
		ErasusDebug npc = view_as<ErasusDebug>(CClotBody(vecPos, vecAng, "models/player/medic.mdl", "1.0", "10000", ally));
		...
		func_NPCDeath[npc.index] = ErasusDebug_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = ErasusDebug_OnTakeDamage;
		func_NPCThink[npc.index] = ErasusDebug_ClotThink;
...
public void ErasusDebug_ClotThink(int iNPC)
{
	ErasusDebug npc = view_as<ErasusDebug>(iNPC);
	...
	PrintToServer("Difference: %f", npc.m_TimeSinceLastThought - GetGameTime());
	npc.m_TimeSinceLastThought = GetGameTime();
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		ErasusDebugSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
...
public Action ErasusDebug_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	ErasusDebug npc = view_as<ErasusDebug>(victim);
...
public void ErasusDebug_NPCDeath(int entity)
{
	ErasusDebug npc = view_as<ErasusDebug>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void ErasusDebugSelfDefense(ErasusDebug npc, float gameTime, int target, float distance)
{
