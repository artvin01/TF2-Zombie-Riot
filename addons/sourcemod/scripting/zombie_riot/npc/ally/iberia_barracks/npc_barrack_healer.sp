...
public void Barrack_Iberia_Healer_ClotThink(int iNPC)
{
	...
	if(BarrackBody_ThinkStart(npc.index, GameTime))
	{
					...
					if(npc.m_flAttackHappens < GameTime && npc.m_flAttackHappens_bullshit >= GameTime && npc.m_flAttackHappenswillhappen)
					{
						...
						if(npc.DoSwingTrace(swingTrace, npc.m_iTarget))
						{
							...
							if(target > 0) 
							{
								...
								ExpidonsaGroupHeal(npc.index, 150.0, 3, Barracks_UnitExtraDamageCalc(npc.index, GetClientOfUserId(npc.OwnerUserId),1000.0, 0), 1.0, true);
		...
		BarrackBody_ThinkMove(npc.index, 220.0, "ACT_MP_RUN_MELEE_ALLCLASS", "ACT_MP_RUN_MELEE_ALLCLASS");
	}
}
...
void Barrack_Iberia_Healer_NPCDeath(int entity)
{
	...
	ExpidonsaGroupHeal(npc.index, 300.0, 3, Barracks_UnitExtraDamageCalc(npc.index, GetClientOfUserId(npc.OwnerUserId),2000.0, 0), 1.0, true);
