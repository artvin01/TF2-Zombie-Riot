...
methodmap BarrackGoggles < BarrackBody
{
	public BarrackGoggles(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		BarrackGoggles npc = view_as<BarrackGoggles>(BarrackBody(client, vecPos, vecAng, "3000", "models/player/sniper.mdl", _, "0.75", _, "models/pickups/pickup_powerup_regen.mdl"));
		
		i_NpcInternalId[npc.index] = BARRACK_GOGGLES;
		...
		SDKHook(npc.index, SDKHook_Think, BarrackGoggles_ClotThink);
...
public void BarrackGoggles_ClotThink(int iNPC)
{
	BarrackGoggles npc = view_as<BarrackGoggles>(iNPC);
...
void BarrackGoggles_NPCDeath(int entity)
{
	BarrackGoggles npc = view_as<BarrackGoggles>(entity);
	BarrackBody_NPCDeath(npc.index);
	SDKUnhook(npc.index, SDKHook_Think, BarrackGoggles_ClotThink);
}