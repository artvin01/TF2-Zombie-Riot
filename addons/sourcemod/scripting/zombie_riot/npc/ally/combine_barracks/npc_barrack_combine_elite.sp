...
void Barracks_Combine_Elite_Precache()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Barracks Combine Elite");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_barrack_combine_elite");
...
public void Barrack_Combine_Elite_ClotThink(int iNPC)
{
	Barrack_Combine_Elite npc = view_as<Barrack_Combine_Elite>(iNPC);
				...
				if(IsValidEnemy(npc.index, Enemy_I_See))
				{
					...
					if(npc.m_flNextRangedAttack < GameTime)
					{
						if(!npc.m_fbRangedSpecialOn)
						{
							...
							npc.FireRocket(vPredictedPos, Barracks_UnitExtraDamageCalc(npc.index, GetClientOfUserId(npc.OwnerUserId), 1250.0, 1), 400.0, "models/effects/combineball.mdl",0.5);
							npc.m_flNextRangedSpecialAttack = GetGameTime(npc.index) + 9.0;
							npc.PlayRangedAttackSecondarySound();
							npc.m_fbRangedSpecialOn = true;
							npc.m_flRangedSpecialDelay = GetGameTime(npc.index) + 7.5;
						}
...
void Barrack_Combine_Elite_NPCDeath(int entity)
{
	Barrack_Combine_Elite npc = view_as<Barrack_Combine_Elite>(entity);
