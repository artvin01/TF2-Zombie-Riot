...
public void SeabornCombineSoldierAr2_OnMapStart_NPC()
{
	...
	for (int i = 0; i < (sizeof(g_GrenadeSound));   i++) { PrecacheSound(g_GrenadeSound[i]);   }
...
{
	return SeabornCombineSoldierAr2(client, vecPos, vecAng, ally);
}
methodmap SeabornCombineSoldierAr2 < CClotBody
{
	...
	public void PlayGrenadeSound() {
		EmitSoundToAll(g_GrenadeSound[GetRandomInt(0, sizeof(g_GrenadeSound) - 1)], this.index, _, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME);
		
		
	}
	
	public SeabornCombineSoldierAr2(int client, float vecPos[3], float vecAng[3], int ally)
	{
	...
	public void FireGrenade(float vecTarget[3])
	{
		...
		{
			this.AddGesture("ACT_COMBINE_THROW_GRENADE");
			...
			f_CustomGrenadeDamage[entity] = 150.0;	
