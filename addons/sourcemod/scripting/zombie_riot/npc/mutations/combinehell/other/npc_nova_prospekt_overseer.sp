...
void NovaProspektOverseer_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Nova Prospekt Overseer");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_nova_prospekt_overseer");
...
{
	return NovaProspektOverseer(vecPos, vecAng, team);
}

methodmap NovaProspektOverseer < CClotBody
{
	...
	public NovaProspektOverseer(float vecPos[3], float vecAng[3], int ally)
	{
		NovaProspektOverseer npc = view_as<NovaProspektOverseer>(CClotBody(vecPos, vecAng, COMBINE_CUSTOM_MODEL, "1.15", "1500", ally));	
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/combine_soldier_prisonguard.mdl");
...
public void NovaProspektOverseer_ClotThink(int iNPC)
{
	...
	{
		if(npc.m_flMeleeArmor < 0.2)
		{
			npc.m_flMeleeArmor = 0.2;
		}
		if(npc.m_flRangedArmor < 0.2)
		{
...
public Action NovaProspektOverseer_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	NovaProspektOverseer npc = view_as<NovaProspektOverseer>(victim);
...
public void NovaProspektOverseer_NPCDeath(int entity)
{
	NovaProspektOverseer npc = view_as<NovaProspektOverseer>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
