...
public void VoidCombinePoliceSmg_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Voided Metro Raider");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_voided_combine_police_smg");
...
methodmap VoidCombinePoliceSmg < CClotBody
{
	...
	public VoidCombinePoliceSmg(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = VoidCombinePoliceSmg_NPCDeath;
...
//TODO 
//Rewrite
public void VoidCombinePoliceSmg_ClotThink(int iNPC)
{
	VoidCombinePoliceSmg npc = view_as<VoidCombinePoliceSmg>(iNPC);
...
public Action VoidCombinePoliceSmg_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	VoidCombinePoliceSmg npc = view_as<VoidCombinePoliceSmg>(victim);
...
public void VoidCombinePoliceSmg_NPCDeath(int entity)
{
	VoidCombinePoliceSmg npc = view_as<VoidCombinePoliceSmg>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
