...
public void Void_Combine_Police_Pistol_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Voided Metro Cop");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_voided_combine_police_pistol");
...
{
	return Void_Combine_Police_Pistol(client, vecPos, vecAng, ally);
}
methodmap Void_Combine_Police_Pistol < CClotBody
{
	...
	public Void_Combine_Police_Pistol(int client, float vecPos[3], float vecAng[3], int ally)
	{
		Void_Combine_Police_Pistol npc = view_as<Void_Combine_Police_Pistol>(CClotBody(vecPos, vecAng, "models/police.mdl", "1.15", "550", ally, false));
		...
		func_NPCDeath[npc.index] = Void_Combine_Police_Pistol_NPCDeath;
		func_NPCThink[npc.index] = Void_Combine_Police_Pistol_ClotThink;
		...
		SetEntityRenderColor(npc.index, 200, 0, 200, 225);
...
//TODO 
//Rewrite
public void Void_Combine_Police_Pistol_ClotThink(int iNPC)
{
	Void_Combine_Police_Pistol npc = view_as<Void_Combine_Police_Pistol>(iNPC);
						...
						if(npc.DoSwingTrace(swingTrace, PrimaryThreatIndex))
							{
								...
								if(target > 0) 
								{
									...
									else
									{
										...
										Elemental_AddVoidDamage(target, npc.index, 10, true, true);
										ApplyStatusEffect(npc.index, target, "Cripple", DurationGive);
									}
									// Hit particle
...
public Action Void_Combine_Police_Pistol_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	Void_Combine_Police_Pistol npc = view_as<Void_Combine_Police_Pistol>(victim);
...
public void Void_Combine_Police_Pistol_NPCDeath(int entity)
{
	Void_Combine_Police_Pistol npc = view_as<Void_Combine_Police_Pistol>(entity);
