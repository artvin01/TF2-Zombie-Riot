...
void VoidCombineElite_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Voided W.F. Elite");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_voided_combine_soldier_elite");
...
methodmap VoidCombineElite < CClotBody
{
	...
	public VoidCombineElite(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = VoidCombineElite_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = VoidCombineElite_OnTakeDamage;
		func_NPCThink[npc.index] = VoidCombineElite_ClotThink;
...
static void EnergyBall_AttackAbility(VoidCombineElite npc, int target, float gameTime)
{
...
//TODO 
//Rewrite
public void VoidCombineElite_ClotThink(int iNPC)
{
	VoidCombineElite npc = view_as<VoidCombineElite>(iNPC);
...
public Action VoidCombineElite_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	VoidCombineElite npc = view_as<VoidCombineElite>(victim);
...
public void VoidCombineElite_NPCDeath(int entity)
{
	VoidCombineElite npc = view_as<VoidCombineElite>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
