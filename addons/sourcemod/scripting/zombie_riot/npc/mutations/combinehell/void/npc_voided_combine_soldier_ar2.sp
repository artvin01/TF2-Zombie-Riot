...
public void VoidCombineSoldierAr2_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Voided W.F. Rifler");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_voided_combine_soldier_ar2");
...
{
	return VoidCombineSoldierAr2(client, vecPos, vecAng, ally);
}
methodmap VoidCombineSoldierAr2 < CClotBody
{
	...
	public VoidCombineSoldierAr2(int client, float vecPos[3], float vecAng[3], int ally)
	{
		VoidCombineSoldierAr2 npc = view_as<VoidCombineSoldierAr2>(CClotBody(vecPos, vecAng, "models/combine_soldier.mdl", "1.15", "1250", ally));
		...
		func_NPCDeath[npc.index] = VoidCombineSoldierAr2_NPCDeath;
...
static void Reload_Ability(VoidCombineSoldierAr2 npc)
{
	...
	if(npc.m_bFUCKYOU && npc.m_flAbilityOrAttack0 <= gameTime)
	{
		...
		{
			npc.m_flAbilityOrAttack0 = gameTime + 0.7;
		}
		switch(npc.m_iAttacksTillReload)
		{
			case 10:
			{
			...
			case 30:
			{
				npc.m_iAttacksTillReload = 40;
			}
			case 40:
			{
				npc.m_iAttacksTillReload = 50;
			}
			case 50:
			{
				npc.m_iAttacksTillReload = 60;
			}
		}
	}
}
...
//TODO 
//Rewrite
public void VoidCombineSoldierAr2_ClotThink(int iNPC)
{
	VoidCombineSoldierAr2 npc = view_as<VoidCombineSoldierAr2>(iNPC);
	float gameTime = GetGameTime(npc.index);
...
public Action VoidCombineSoldierAr2_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	VoidCombineSoldierAr2 npc = view_as<VoidCombineSoldierAr2>(victim);
...
public void VoidCombineSoldierAr2_NPCDeath(int entity)
{
	VoidCombineSoldierAr2 npc = view_as<VoidCombineSoldierAr2>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
