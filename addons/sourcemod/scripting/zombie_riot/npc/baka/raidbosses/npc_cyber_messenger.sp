...
static const char g_RangedSound[] = "weapons/tacky_grenadier_shoot.wav";
static const char g_MeleeAttackSounds[] = "weapons/cbar_miss1.wav";
static const char g_MessengerThrowIce[] = "weapons/icicle_freeze_victim_01.wav";

static bool b_khamlWeaponRage[MAXENTITIES];
static float f_MessengerSpeedUp[MAXENTITIES];
static int PurpleParticle[MAXENTITIES];

void CyberMessenger_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "The Messenger");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_cyber_messenger");
...
static void ClotPrecache()
{
	...
	PrecacheSound(g_RangedSound);
	PrecacheSound(g_MessengerThrowIce);
...
{
	return CyberMessenger(vecPos, vecAng, team, data);
}

methodmap CyberMessenger < CClotBody
{
	...
	public CyberMessenger(float vecPos[3], float vecAng[3], int ally, const char[] data)
	{
		CyberMessenger npc = view_as<CyberMessenger>(CClotBody(vecPos, vecAng, "models/player/demo.mdl", "1.35", "40000", ally, false, true, true,true)); //giant!
		...
		func_NPCDeath[npc.index] = CyberMessenger_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = CyberMessenger_OnTakeDamage;
		func_NPCThink[npc.index] = CyberMessenger_ClotThink;
		func_NPCFuncWin[npc.index] = CyberMessenger_Win;

		SDKHook(npc.index, SDKHook_OnTakeDamagePost, CyberMessenger_OnTakeDamagePost);
...
static void CyberMessenger_ClotThink(int iNPC)
{
	CyberMessenger npc = view_as<CyberMessenger>(iNPC);
	float gametimer = GetGameTime(npc.index);
	if(npc.m_flNextDelayTime > gametimer)
		return;
	npc.m_flNextDelayTime = gametimer + DEFAULT_UPDATE_DELAY_FLOAT;
	npc.Update();
	...
	if(npc.m_flNextThinkTime > gametimer)
		return;

	npc.m_flNextThinkTime = gametimer + 0.1;

	if(npc.i_GunMode == 0 && CyberMessanger_Elemental_Attack_Projectiles(npc))
	{
	...
	if(CyberMessanger_Elemental_Attack_TempPowerup(npc))
	{
		npc.m_flMeleeArmor = 0.65;
		npc.m_flRangedArmor = 0.5;	
		return;
	...
	if(npc.m_flGetClosestTargetTime < gametimer)
	{
		npc.m_iTarget = GetClosestTarget(npc.index);
		npc.m_flGetClosestTargetTime = gametimer + GetRandomRetargetTime();
	}
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		SetGoalVectorIndex = CyberMessengerSelfDefense(npc,gametimer, npc.m_iTarget, flDistanceToTarget); 
		...
		switch(SetGoalVectorIndex)
		{
			case 0:
			{
				...
				if(flDistanceToTarget < npc.GetLeadRadius()) 
				{
					...
					CyberMessanger_Elemental_Attack_FingerPoint(npc);
				}
				else 
				{
	...
	if(npc.m_flDoingAnimation < gametimer)
		CyberMessengerAnimationChange(npc);
}

static bool CyberMessanger_Elemental_Attack_Projectiles(CyberMessenger npc)
{
...
static bool CyberMessanger_Elemental_Attack_TempPowerup(CyberMessenger npc)
{
...
static bool CyberMessanger_Elemental_Attack_FingerPoint(CyberMessenger npc)
{
	...
	if(npc.m_flJumpStartTimeInternal)
	{
		if(npc.m_flJumpStartTimeInternal < GetGameTime(npc.index) && npc.m_flSwitchCooldown > GetGameTime(npc.index))
		{
			...
			if(Succeed)
			{
				...
				npc.m_flJumpStartTimeInternal = 0.0;
				CyberMessengerResetAndDelayAttack(npc.index);
			}
		}
		return true;
	}
...
static Action CyberMessenger_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	CyberMessenger npc = view_as<CyberMessenger>(victim);
...
static void CyberMessenger_NPCDeath(int entity)
{
	CyberMessenger npc = view_as<CyberMessenger>(entity);
...
static void CyberMessengerAnimationChange(CyberMessenger npc)
{
	...
	switch(npc.i_GunMode)
	{
		case 1: //primary
		{
			if (npc.IsOnGround())
			{
				...
				{
				// ResetCyberMessengerWeapon(npc, 1);
		...
		case 0: //Melee
		{
			if (npc.IsOnGround())
			{
				...
				{
				//	ResetCyberMessengerWeapon(npc, 0);
					npc.m_bisWalking = true;
...
static int CyberMessengerSelfDefense(CyberMessenger npc, float gameTime, int target, float distance)
{
	if(npc.i_GunMode == 1)
	{
			...
			if(distance < (NORMAL_ENEMY_MELEE_RANGE_FLOAT_SQUARED * 7.5))
			{
				...
				if(IsValidEnemy(npc.index, Enemy_I_See))
				{
					...
					if(IsValidEntity(projectile))
					{
						...
						GetAbsOrigin(projectile, vecTarget);
						particle = ParticleEffectAt(vecTarget, "spell_fireball_small_blue", 3.5);
						SetParent(projectile, particle);
						PurpleParticle[projectile]=EntIndexToEntRef(particle);
						
						WandProjectile_ApplyFunctionToEntity(projectile, CyberMessenger_Rocket_Particle_StartTouch);
					}
				...
				for (int counter = 1; counter <= HowManyEnemeisAoeMelee; counter++)
				{
					if (i_EntitiesHitAoeSwing_NpcSwing[counter] > 0)
					{
						if(IsValidEntity(i_EntitiesHitAoeSwing_NpcSwing[counter]))
						{
							...
							float Elemental_Damage = 22.0 * RaidModeScaling;
							Elemental_Damage *= 0.1;
							NPC_Ignite(targetTrace, npc.index,2.5, -1, Elemental_Damage);
							Elemental_Damage = 150.0;
							if(NpcStats_IsEnemySilenced(npc.index))
								Elemental_Damage = 140.0;

							ApplyStatusEffect(npc.index, targetTrace, "Near Zero", 3.5);
							Elemental_AddChaosDamage(targetTrace, npc.index, RoundToCeil(Elemental_Damage), true, true);
...
static void CyberMessengerResetAndDelayAttack(int entity)
{
	CyberMessenger npc = view_as<CyberMessenger>(entity);
	npc.m_flAttackHappens = 0.0;
...
static void CyberMessenger_Rocket_Particle_StartTouch(int entity, int target)
{
	if(target > 0 && target < MAXENTITIES)
	{
		...
		Elemental_AddChaosDamage(target, owner, 100, true, true);
		
		int particle = EntRefToEntIndex(i_WandParticle[entity]);
		if(IsValidEntity(particle))
			RemoveEntity(particle);
		particle = EntRefToEntIndex(PurpleParticle[entity]);
		if(IsValidEntity(particle))
			RemoveEntity(particle);
	}
	else
...
static void CyberMessengerInitiateGroupAttack(CyberMessenger npc)
{
	...
	for(int i; i < sizeof(enemy); i++)
	{
		if(enemy[i])
		{
			...
			if(IsValidEntity(projectile))
			{
				GetAbsOrigin(projectile, vecHit);
				int particle = ParticleEffectAt(vecHit, "spell_fireball_small_blue", 0.0);
				SetParent(projectile, particle);
				PurpleParticle[projectile]=EntIndexToEntRef(particle);
				WandProjectile_ApplyFunctionToEntity(projectile, CyberMessenger_Rocket_Particle_StartTouch);		
...
static void CyberMessenger_OnTakeDamagePost(int victim, int attacker, int inflictor, float damage, int damagetype) 
{
