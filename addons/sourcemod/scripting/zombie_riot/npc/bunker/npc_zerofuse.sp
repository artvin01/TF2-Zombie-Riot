	...
	if(!b_IsAlliedNpc[npc.index])//Don't allow the ally version to fuck over the round
	{
		...
		if(fl_MinionSummon[npc.index] <= gameTime)
		{
			...
			for(int i; i<1; i++)
			{
				...
				int spawn_index = Npc_Create(SPY_MAIN_BOSS, -1, pos, ang, GetEntProp(npc.index, Prop_Send, "m_iTeamNum"));
			...
			for(int j; j<1; j++)
			{
				...
				int spawn_index = Npc_Create(MEDIVAL_SAMURAI, -1, pos, ang, GetEntProp(npc.index, Prop_Send, "m_iTeamNum"));
				if(spawn_index > MaxClients)
				{
					Zombies_Currently_Still_Ongoing += 1;
					SetEntProp(spawn_index, Prop_Data, "m_iHealth", maxhealth);
