...
void CombineBunkerOverlord_OnMapStart_NPC()
{
...
methodmap CombineBunkerOverlord < CClotBody
{
	...
	public CombineBunkerOverlord(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		CombineBunkerOverlord npc = view_as<CombineBunkerOverlord>(CClotBody(vecPos, vecAng, "models/zombie_riot/bosses/overlord_1.mdl", "1.4", "85000", ally));
		
		i_NpcInternalId[npc.index] = COMBINEBUNKER_OVERLORD;
		...
		SDKHook(npc.index, SDKHook_OnTakeDamage, CombineBunkerOverlord_ClotDamaged);
		SDKHook(npc.index, SDKHook_Think, CombineBunkerOverlord_ClotThink);
...
//TODO 
//Rewrite
public void CombineBunkerOverlord_ClotThink(int iNPC)
{
	CombineBunkerOverlord npc = view_as<CombineBunkerOverlord>(iNPC);
				...
				if(npc.m_flNextMeleeAttack < GetGameTime(npc.index))
				{
					if (!npc.m_flAttackHappenswillhappen)
					{
						if(npc.m_flAngerDelay < GetGameTime(npc.index))
						{
							...
							npc.PlayMeleeSound();
							npc.m_flAttackHappens = GetGameTime(npc.index)+0.24;
					...
					if (npc.m_flAttackHappens < GetGameTime(npc.index) && npc.m_flAttackHappens_bullshit >= GetGameTime(npc.index) && npc.m_flAttackHappenswillhappen)
					{
						...
						delete swingTrace;
						npc.m_flNextMeleeAttack = GetGameTime(npc.index) + 0.24;
						npc.m_flAttackHappenswillhappen = false;
					}
...
public Action CombineBunkerOverlord_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	CombineBunkerOverlord npc = view_as<CombineBunkerOverlord>(victim);
...
public void CombineBunkerOverlord_NPCDeath(int entity)
{
	CombineBunkerOverlord npc = view_as<CombineBunkerOverlord>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
	
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, CombineBunkerOverlord_ClotDamaged);
	SDKUnhook(npc.index, SDKHook_Think, CombineBunkerOverlord_ClotThink);
