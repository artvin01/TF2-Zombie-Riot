...
//JUST A REMINDER THIS .SP WILL GET A BETTER WAY OF DOING THE GUNS I JUST COPY AND PASTED IT CONSTANTLY FOR NOW
...
static float DefaultDamage = 100.0;//Default Damage
static float CashGain_OnHit = 20.0;//How much cash can he gain on hit
...
methodmap TheGambler < CClotBody
{
	...
	{
		TheGambler npc = view_as<TheGambler>(CClotBody(vecPos, vecAng, GAMBLERMODEL, "1.0", "18000", ally));
		...
		SDKHook(npc.index, SDKHook_OnTakeDamage, TheGambler_ClotDamaged);
...
public void TheGambler_ClotThink(int iNPC)
{
	...
	if(AllowExperimentalGearDamage[npc.index])
	{
		...
		CPrintToChatAll("{cyan}[Bunker]{white}: {green}Gambler Now deals additional {orange}%.0f Damage.", ExperimentalGearDamage[npc.index]);
	}
	if(ExperimentalGear_On[npc.index] && TempOpener4[npc.index])
	{
		...
		{
			CPrintToChatAll("{cyan}[Bunker]{white}: {green}Gambler Now takes {yellow}%i%% less Damage.", 100 - RoundFloat(100 * RandomizedDamageTaken));
		}
		else
		{
			CPrintToChatAll("{cyan}[Bunker]{white}: {green}Gambler Now takes {red}%i%% more Damage.", -(100 - RoundFloat(100 * RandomizedDamageTaken)));
		}
	...
	if(SlotMachine_Usage[npc.index] && TempOpener5[npc.index])
	{
		...
		if(SlotMachineMaxCashGain <= 1.00)
		{
			...
			CPrintToChatAll("{cyan}[Bunker]{default}: {green}Gambler lost {yellow}%i%% {green}of his cash.", 100 - RoundFloat(100 * SlotMachineMaxCashGain));
		}
		if(SlotMachineMaxCashGain >= 1.00)
		{
			EmitSoundToAll(SLOTMACHINESUCCESS, _, _, _, _, 1.0);
			CPrintToChatAll("{cyan}[Bunker]{default}: {green}Gambler won {red}%i%% {green}of his cash.", -(100 - RoundFloat(100 * SlotMachineMaxCashGain)));
		}
	}
	if(Gambler_AbilityManagement_Timer[npc.index] <= GetGameTime(npc.index) && !Gambler_AbilityManagement[npc.index] && !Jawbreaker_On[npc.index] && !Archers_Anhilation[npc.index] && !Exodia_TheFuckYouGun[npc.index] && !Pellet_Gun[npc.index] && !WeakMachineGun_Smg[npc.index] && !TheHolyOrb[npc.index] && GamblersCash[npc.index] >= 700.0)
	{
		switch(GetRandomInt(1, 6))
		{
			...
			default:
			{
				...
				CPrintToChatAll("{cyan}[Bunker]{default}: {green}Gambler gained a broken gun and threw it away.");
			}
		}
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex, true))
	{
		...
		{//Exodia
			...
			else
			{
				...
				//npc.m_bPathing = false;
				npc.FaceTowards(vecTarget, 10000.0);
				npc.m_flNextRangedAttack = GetGameTime(npc.index) + 3.4;
				//npc.m_iAttacksTillReload -= 1;
...
public Action TheGambler_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
...
public void TheGambler_NPCDeath(int entity)
{
	...
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, TheGambler_ClotDamaged);
	SDKUnhook(npc.index, SDKHook_Think, TheGambler_ClotThink);		
