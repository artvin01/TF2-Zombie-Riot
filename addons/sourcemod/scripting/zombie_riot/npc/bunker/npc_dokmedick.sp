...
static float OverDose_Usage[MAXENTITIES];
static bool OverDose_Animation[MAXENTITIES] = {false, ...};
static bool OverDose_Activate[MAXENTITIES] = {false, ...};
static bool Overdose_TeleportUsage[MAXENTITIES] = {false, ...};
static bool TempOpener[MAXENTITIES] = {false, ...};
static bool OverDoseActive[MAXENTITIES] = {false, ...};
...
static bool BackToMeleeAnimation[MAXENTITIES] = {false, ...};

static bool AbilityManagement[MAXENTITIES] = {false, ...};
static float AbilityManagement_Timer[MAXENTITIES];

static bool KetamineUsage[MAXENTITIES] = {false, ...};
static float KetamineTimer[MAXENTITIES];

static bool SteroidsUsage[MAXENTITIES] = {false, ...};
static float SteroidsTimer[MAXENTITIES];

static bool Chemical_Warfare[MAXENTITIES] = {false, ...};
static float Chemical_Warfare_Timer[MAXENTITIES];

static float KetamineSteroidsComboTimer[MAXENTITIES];
static bool KetamineSteroidsCombo[MAXENTITIES] = {false, ...};

static float DokMed_MaxSpeed = 340.0;//Main Speed
static float AbilityManagement_TimerReuseability = 17.0;//How long it should be reuseable again
...
static float Chemical_Warfare_Wearoff = 5.0;//how long it should stay

static float SteroidsDamage = 300.0;//Melee Damage for players 
static float SteroidsBuildingNpcDamage = 1950.0;//Melee Damage for npc's and building
static float DefaultDamage = 100.0;//Melee Damage for players
static float DefaultBuildingNpcDamage = 1550.0;//Melee Damage for npc's and building
static float Chemical_Warfare_Damage = 160.0;//Chemical Damage for players
...
methodmap Doktor_Medick < CClotBody
{
	...
	public Doktor_Medick(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		...
		AbilityManagement_Timer[npc.index] = GetGameTime(npc.index) + 15.0;
		OverDose_Usage[npc.index] = GetGameTime(npc.index) + 10.0;
		KetamineTimer[npc.index] = GetGameTime(npc.index) + 10.0;
...
public void Doktor_Medick_ClotThink(int iNPC)
{
	...
	if(OverDose_Usage[npc.index] <= GetGameTime(npc.index) && !OverDoseActive[npc.index] && !TempOpener[npc.index] && !MoonLight_used[npc.index])
	{
		OverDose_Animation[npc.index] = true;
		npc.m_flSpeed = 0.0;
		float vEnd[3];
		GetEntPropVector(npc.index, Prop_Send, "m_vecOrigin", vEnd);
		OverDose_Usage[npc.index] = GetGameTime(npc.index) + Overdose_Timer;
	...
	if(OverDose_Usage[npc.index] <= GetGameTime(npc.index) && !OverDoseActive[npc.index] && TempOpener[npc.index])
	{
		//OverDose_Usage[npc.index] = GetGameTime(npc.index) + Overdose_Reuseable;
		if(!Chemical_Warfare[npc.index])
		{
			int iActivity = npc.LookupActivity("ACT_MP_RUN_MELEE");
			if(iActivity > 0) npc.StartActivity(iActivity);
		}
		...
		Overdose_TeleportUsage[npc.index] = true;
		OverDoseActive[npc.index] = true;
		OverDose_Animation[npc.index] = false;
		TempOpener[npc.index] = false;
	}
	if(AbilityManagement_Timer[npc.index] <= GetGameTime(npc.index) && !AbilityManagement[npc.index] && !KetamineUsage[npc.index] && !SteroidsUsage[npc.index] &&!OverDoseActive[npc.index] && !Overdose_TeleportUsage[npc.index] && !OverDose_Animation[npc.index] && !MoonLight_used[npc.index])
	{
		float vEnd[3];
		GetEntPropVector(npc.index, Prop_Send, "m_vecOrigin", vEnd);
		switch(GetRandomInt(1,4))
		{
			case 1:
			{
				//AbilityManagement[npc.index] = true;
				KetamineUsage[npc.index] = true;
				KetamineTimer[npc.index] = GetGameTime(npc.index) + KetamineWearoff;
				if(b_IsAlliedNpc[npc.index])
				{
					//EmitSoundToAll(OVERDOSE_ACTIVATE, 0, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, vEnd);
			...
			{
				//AbilityManagement[npc.index] = true;
				SteroidsUsage[npc.index] = true;
				SteroidsTimer[npc.index] = GetGameTime(npc.index) + SteroidsWearoff;
				if(b_IsAlliedNpc[npc.index])
				{
					//EmitSoundToAll(OVERDOSE_ACTIVATE, 0, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, vEnd);
			...
			{
				//AbilityManagement[npc.index] = true;
				KetamineSteroidsCombo[npc.index] = true;
				KetamineSteroidsComboTimer[npc.index] = GetGameTime(npc.index) + KetamineSteroids_Timer;
				if(b_IsAlliedNpc[npc.index])
				{
					//EmitSoundToAll(OVERDOSE_ACTIVATE, 0, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, vEnd);
			...
			case 4:
			{
				Chemical_Warfare[npc.index] = true;
				Chemical_Warfare_Timer[npc.index] = GetGameTime(npc.index) + Chemical_Warfare_Wearoff;
				BackToMeleeAnimation[npc.index] = true;
				f_BackToMeleeAnimation[npc.index] = GetGameTime(npc.index) + Chemical_Warfare_Wearoff;
				if(b_IsAlliedNpc[npc.index])
				{
					//EmitSoundToAll(OVERDOSE_ACTIVATE, 0, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, vEnd);
		...
		AbilityManagement[npc.index] = true;
		//AbilityManagement_Timer[npc.index] = 5.0;
	}
	if(KetamineUsage[npc.index] && KetamineTimer[npc.index] <= GetGameTime(npc.index) && AbilityManagement[npc.index] && !SteroidsUsage[npc.index])
	{//ketamine
		KetamineUsage[npc.index] = false;
		AbilityManagement[npc.index] = false;
		//KetamineTimer[npc.index] = 5.0;
		AbilityManagement_Timer[npc.index] = GetGameTime(npc.index) + AbilityManagement_TimerReuseability;
	}
	if(SteroidsUsage[npc.index] && SteroidsTimer[npc.index] <= GetGameTime(npc.index) && AbilityManagement[npc.index] && !KetamineUsage[npc.index])
	{//Steroids
		SteroidsUsage[npc.index] = false;
		AbilityManagement[npc.index] = false;
		//KetamineTimer[npc.index] = 5.0;
		AbilityManagement_Timer[npc.index] = GetGameTime(npc.index) + AbilityManagement_TimerReuseability;
	}
	if(KetamineSteroidsCombo[npc.index] && KetamineSteroidsComboTimer[npc.index] <= GetGameTime(npc.index) && AbilityManagement[npc.index] && !SteroidsUsage[npc.index] && !KetamineUsage[npc.index])
	{//ketamineSteroids
		KetamineSteroidsCombo[npc.index] = false;
		AbilityManagement[npc.index] = false;
		AbilityManagement_Timer[npc.index] = GetGameTime(npc.index) + AbilityManagement_TimerReuseability;
		//KetamineSteroidsComboTimer[npc.index] = GetGameTime(npc.index) + 999.0;
	}
	if(Chemical_Warfare[npc.index] && Chemical_Warfare_Timer[npc.index] <= GetGameTime(npc.index) && AbilityManagement[npc.index] && !SteroidsUsage[npc.index] && !KetamineUsage[npc.index])
	{//ketamineSteroids
		Chemical_Warfare[npc.index] = false;
		//Chemical_Warfare_Timer[npc.index] = GetGameTime(npc.index) + Chemical_Warfare_Wearoff;
		AbilityManagement[npc.index] = false;
		AbilityManagement_Timer[npc.index] = GetGameTime(npc.index) + AbilityManagement_TimerReuseability;
	}
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
		...
		//if(OverDoseActive[npc.index] && npc.m_flNextRangedSpecialAttack < GetGameTime(npc.index))
		if(OverDoseActive[npc.index])
		{
			float vPredictedPos[3]; vPredictedPos = PredictSubjectPosition(npc, PrimaryThreatIndex, 0.3);
			static float flVel[3];
			OverDose_Usage[npc.index] = GetGameTime(npc.index) + Overdose_Reuseable;
			
			if(Overdose_TeleportUsage[npc.index])
			{
				int color[4];
				color[0] = 0;
				color[1] = 255;
				...
				if(Tele_Check < 100000000 || Tele_Check < 10000000 || Tele_Check < 1000000 || Tele_Check < 100000 || Tele_Check < 10000 || Tele_Check > 100000000 || Tele_Check > 10000000 || Tele_Check > 1000000 || Tele_Check > 100000 || Tele_Check > 10000)
				{
					...
					OverDose_Activate[npc.index] = true;
					Overdose_TeleportUsage[npc.index] = false;
					npc.m_flSpeed = DokMed_MaxSpeed;
				}
			}
			...
			if(IsValidEnemy(npc.index, npc.m_iTarget) && npc.m_iTarget == Enemy_I_See && OverDose_Activate[npc.index] && !Overdose_TeleportUsage[npc.index])
			{
				//float vAngles[3];
				//float vOrigin[3];
				//float vEnd[3];
				...
				OverDoseActive[npc.index] = false;
			}
		}
		if(npc.m_flNextRangedAttack < GetGameTime(npc.index) && flDistanceToTarget < 142500 && npc.m_flReloadDelay < GetGameTime(npc.index) && Chemical_Warfare[npc.index])
		{
		...
		if(flDistanceToTarget < 10000 && !Chemical_Warfare[npc.index] || npc.m_flAttackHappenswillhappen && !Chemical_Warfare[npc.index])
		{
		...
		//	npc.FaceTowards(vecTarget, 1000.0);
			...
			if(npc.m_flNextMeleeAttack < GetGameTime(npc.index))
			{
				...
				if (!npc.m_flAttackHappenswillhappen)
				{
					...
					npc.PlayMeleeSound();
					if(KetamineUsage[npc.index] || KetamineSteroidsCombo[npc.index])
					{
				...
				if (npc.m_flAttackHappens < GetGameTime(npc.index) && npc.m_flAttackHappens_bullshit >= GetGameTime(npc.index) && npc.m_flAttackHappenswillhappen)
				{
					...
					if(npc.DoSwingTrace(swingTrace, PrimaryThreatIndex))
					{
						...
						if(target > 0) 
						{
							...
							{
								if(SteroidsUsage[npc.index] || KetamineSteroidsCombo[npc.index])
								{
...
public void MoonLight_Invoke(int ref, int enemy, float timer, float charge)
{
	...
	if(IsValidEntity(entity))
	{
		...
		float time=MoonLight_Duration[npc.index];
		MoonLight_Duration[npc.index]*=66.0;
		MoonLight_RemainingDuration[npc.index] = 0.0;
		
		MoonLight_Scale2_timer[npc.index]=GetGameTime(npc.index)+(timer/3);	//makes it so the 3 beam rings spawn in 3 seperate times.
