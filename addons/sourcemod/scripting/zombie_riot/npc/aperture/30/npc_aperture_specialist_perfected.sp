...
void ApertureSpecialistPerfected_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Perfected Aperture Specialist");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_aperture_specialist_perfected");
...
{
	return ApertureSpecialistPerfected(vecPos, vecAng, team);
}
methodmap ApertureSpecialistPerfected < CClotBody
{
	...
	public ApertureSpecialistPerfected(float vecPos[3], float vecAng[3], int ally)
	{
		ApertureSpecialistPerfected npc = view_as<ApertureSpecialistPerfected>(CClotBody(vecPos, vecAng, "models/bots/spy/bot_spy.mdl", "1.0", "6500", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(ApertureSpecialistPerfected_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(ApertureSpecialistPerfected_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(ApertureSpecialistPerfected_ClotThink);
...
public void ApertureSpecialistPerfected_ClotThink(int iNPC)
{
	ApertureSpecialistPerfected npc = view_as<ApertureSpecialistPerfected>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		int ActionDo = ApertureSpecialistPerfectedSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
		switch(ActionDo)
		{
			case 0:
			{
...
public Action ApertureSpecialistPerfected_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	ApertureSpecialistPerfected npc = view_as<ApertureSpecialistPerfected>(victim);
...
public void ApertureSpecialistPerfected_NPCDeath(int entity)
{
	ApertureSpecialistPerfected npc = view_as<ApertureSpecialistPerfected>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
int ApertureSpecialistPerfectedSelfDefense(ApertureSpecialistPerfected npc, float gameTime, int target, float distance)
{
