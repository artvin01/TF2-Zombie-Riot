...
void ApertureSupporterV2_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Aperture Supporter V2");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_aperture_supporter_v2");
...
{
	return ApertureSupporterV2(vecPos, vecAng, ally);
}
methodmap ApertureSupporterV2 < CClotBody
{
	...
	public ApertureSupporterV2(float vecPos[3], float vecAng[3], int ally)
	{
		ApertureSupporterV2 npc = view_as<ApertureSupporterV2>(CClotBody(vecPos, vecAng, "models/player/medic.mdl", "1.0", "30000", ally));
		...
		func_NPCDeath[npc.index] = ApertureSupporterV2_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = ApertureSupporterV2_OnTakeDamage;
		func_NPCThink[npc.index] = ApertureSupporterV2_ClotThink;
...
//TODO 
//Rewrite
public void ApertureSupporterV2_ClotThink(int iNPC)
{
	ApertureSupporterV2 npc = view_as<ApertureSupporterV2>(iNPC);
	...
	if(!npc.m_bFUCKYOU)
	{
		...
		if(IsValidAlly(npc.index, PrimaryThreatIndex))
		{
			...
			if(flDistanceToTarget < 250000)
			{
				...
				{
					npc.StartHealing();
					npc.m_iWearable4 = ConnectWithBeam(npc.m_iWearable3, PrimaryThreatIndex, 50, 200, 50, 3.0, 3.0, 1.35, LASERBEAM);
					npc.Healing = true;
				...
				if(npc.m_flNextRangedAttack < GetGameTime(npc.index))
				{
					...
					ExpidonsaGroupHeal(npc.index, 400.0, 99, 5000.0, 4.0, false,Expidonsa_DontHealSameIndex,Aperture_GrantDicksucking);
				}
...
public Action ApertureSupporterV2_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &m_iWearable3, float damageForce[3], float damagePosition[3], int damagecustom)
{
	ApertureSupporterV2 npc = view_as<ApertureSupporterV2>(victim);
...
public void ApertureSupporterV2_NPCDeath(int entity)
{
	ApertureSupporterV2 npc = view_as<ApertureSupporterV2>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
