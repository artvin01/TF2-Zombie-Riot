...
static const char g_MeleeAttackSounds[][] = {
	"weapons/shooting_star_shoot_charged.wav",
};


void ApertureSniperPerfected_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Perfected Aperture Sniper");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_aperture_sniper_perfected");
...
{
	return ApertureSniperPerfected(vecPos, vecAng, team);
}

methodmap ApertureSniperPerfected < CClotBody
{
	...
	{
		ApertureSniperPerfected npc = view_as<ApertureSniperPerfected>(CClotBody(vecPos, vecAng, "models/player/sniper.mdl", "1.0", "4000", ally));
		...
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(ApertureSniperPerfected_OnTakeDamage);
		...
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/sniper/invasion_final_frontiersman/invasion_final_frontiersman.mdl");
...
public void ApertureSniperPerfected_ClotThink(int iNPC)
{
	ApertureSniperPerfected npc = view_as<ApertureSniperPerfected>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTargetWalkTo))
	{
		...
		int ExtraBehavior = ApertureSniperPerfectedSelfDefense(npc,GetGameTime(npc.index)); 
...
public Action ApertureSniperPerfected_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	ApertureSniperPerfected npc = view_as<ApertureSniperPerfected>(victim);
...
public void ApertureSniperPerfected_NPCDeath(int entity)
{
	ApertureSniperPerfected npc = view_as<ApertureSniperPerfected>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
int ApertureSniperPerfectedSelfDefense(ApertureSniperPerfected npc, float gameTime)
{
	...
	if(npc.m_flAttackHappens)
	{
		TE_SetupBeamPoints(origin, ThrowPos[npc.index], Shared_BEAM_Laser, 0, 0, 0, 0.11, 5.0, 5.0, 0, 0.0, {102,0,102,125}, 3);
		TE_SendToAll(0.0);
	}
