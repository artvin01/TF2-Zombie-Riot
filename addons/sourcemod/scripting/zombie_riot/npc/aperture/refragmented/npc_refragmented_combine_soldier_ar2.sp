...
public void RefragmentedCombineSoldierAr2_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Refragmented W.F. Rifler");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_refragmented_combine_soldier_ar2");
...
{
	return RefragmentedCombineSoldierAr2(vecPos, vecAng, team);
}
methodmap RefragmentedCombineSoldierAr2 < CClotBody
{
	...
	public RefragmentedCombineSoldierAr2(float vecPos[3], float vecAng[3], int ally)
	{
		RefragmentedCombineSoldierAr2 npc = view_as<RefragmentedCombineSoldierAr2>(CClotBody(vecPos, vecAng, "models/combine_soldier.mdl", "1.15", "1250", ally));
		...
		func_NPCDeath[npc.index] = RefragmentedCombineSoldierAr2_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = RefragmentedCombineSoldierAr2_OnTakeDamage;
		func_NPCThink[npc.index] = RefragmentedCombineSoldierAr2_ClotThink;
...
public void RefragmentedCombineSoldierAr2_ClotThink(int iNPC)
{
	RefragmentedCombineSoldierAr2 npc = view_as<RefragmentedCombineSoldierAr2>(iNPC);
...
public Action RefragmentedCombineSoldierAr2_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	RefragmentedCombineSoldierAr2 npc = view_as<RefragmentedCombineSoldierAr2>(victim);
...
public void RefragmentedCombineSoldierAr2_NPCDeath(int entity)
{
	RefragmentedCombineSoldierAr2 npc = view_as<RefragmentedCombineSoldierAr2>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
	...
	for(int i; i < i_MaxcountNpcTotal; i++)
	{
		...
		if(entity2 != INVALID_ENT_REFERENCE && i_NpcInternalId[entity2] == RefragmentedCombinePoliceSmg_ID() && IsEntityAlive(entity2) && GetTeam(entity2) == team)
		{
