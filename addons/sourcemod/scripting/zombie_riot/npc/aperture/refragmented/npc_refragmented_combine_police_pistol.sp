...
public void Refragmented_Combine_Police_Pistol_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Refragmented Metro Cop");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_refragmented_combine_police_pistol");
...
{
	return Refragmented_Combine_Police_Pistol(vecPos, vecAng, team);
}
methodmap Refragmented_Combine_Police_Pistol < CClotBody
{
	...
	public Refragmented_Combine_Police_Pistol(float vecPos[3], float vecAng[3], int ally)
	{
		Refragmented_Combine_Police_Pistol npc = view_as<Refragmented_Combine_Police_Pistol>(CClotBody(vecPos, vecAng, "models/police.mdl", "1.15", "550", ally, false));
...
public void Refragmented_Combine_Police_Pistol_ClotThink(int iNPC)
{
	Refragmented_Combine_Police_Pistol npc = view_as<Refragmented_Combine_Police_Pistol>(iNPC);
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
			...
			if(npc.m_flNextRangedAttack < GetGameTime(npc.index) && flDistanceToTarget > 62500 && flDistanceToTarget < 122500 && npc.m_flReloadDelay < GetGameTime(npc.index))
			{
				...
				else
				{
					...
					{
						HealEntityGlobal(npc.index, npc.index, 1000.0, 1.50, 0.0, HEAL_SELFHEAL);
...
public Action RefragmentedCombinePolicePistol_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	Refragmented_Combine_Police_Pistol npc = view_as<Refragmented_Combine_Police_Pistol>(victim);
...
public void RefragmentedCombinePolicePistol_NPCDeath(int entity)
{
	Refragmented_Combine_Police_Pistol npc = view_as<Refragmented_Combine_Police_Pistol>(entity);
