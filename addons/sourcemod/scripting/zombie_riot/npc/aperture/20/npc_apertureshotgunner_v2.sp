...
void ApertureShotgunnerV2_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Aperture Shotgunner V2");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_aperture_shotgunner_v2");
...
{
	return ApertureShotgunnerV2(vecPos, vecAng, ally);
}
methodmap ApertureShotgunnerV2 < CClotBody
{
	...
	public ApertureShotgunnerV2(float vecPos[3], float vecAng[3], int ally)
	{
		ApertureShotgunnerV2 npc = view_as<ApertureShotgunnerV2>(CClotBody(vecPos, vecAng, "models/player/engineer.mdl", "1.0", "700", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(ApertureShotgunnerV2_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(ApertureShotgunnerV2_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(ApertureShotgunnerV2_ClotThink);
		...
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/engineer/all_work_and_no_plaid/all_work_and_no_plaid.mdl");
...
public void ApertureShotgunnerV2_ClotThink(int iNPC)
{
	ApertureShotgunnerV2 npc = view_as<ApertureShotgunnerV2>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		ApertureShotgunnerV2SelfDefense(npc, GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
		npc.m_flGetClosestTargetTime = 0.0;
...
public Action ApertureShotgunnerV2_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	ApertureShotgunnerV2 npc = view_as<ApertureShotgunnerV2>(victim);
...
public void ApertureShotgunnerV2_NPCDeath(int entity)
{
	ApertureShotgunnerV2 npc = view_as<ApertureShotgunnerV2>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void ApertureShotgunnerV2SelfDefense(ApertureShotgunnerV2 npc, float gameTime, int target, float distance)
{
