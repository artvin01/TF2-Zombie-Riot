...
void ApertureSniper_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Aperture Sniper");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_aperture_sniper");
...
{
	return ApertureSniper(vecPos, vecAng, team);
}

methodmap ApertureSniper < CClotBody
{
	...
	public ApertureSniper(float vecPos[3], float vecAng[3], int ally)
	{
		ApertureSniper npc = view_as<ApertureSniper>(CClotBody(vecPos, vecAng, "models/player/sniper.mdl", "1.0", "4000", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(ApertureSniper_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(ApertureSniper_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(ApertureSniper_ClotThink);
		...
		npc.m_iWearable3 = npc.EquipItem("head", "models/workshop/player/items/sniper/sum19_wagga_wagga_wear/sum19_wagga_wagga_wear.mdl");
...
public void ApertureSniper_ClotThink(int iNPC)
{
	ApertureSniper npc = view_as<ApertureSniper>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTargetWalkTo))
	{
		...
		int ExtraBehavior = ApertureSniperSelfDefense(npc,GetGameTime(npc.index)); 
...
public Action ApertureSniper_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	ApertureSniper npc = view_as<ApertureSniper>(victim);
...
public void ApertureSniper_NPCDeath(int entity)
{
	ApertureSniper npc = view_as<ApertureSniper>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
int ApertureSniperSelfDefense(ApertureSniper npc, float gameTime)
{
