...
{
	return Test(vecPos, vecAng, ally);
}
methodmap Test < CClotBody
{
	...
	public Test(float vecPos[3], float vecAng[3], int ally)
	{
		Test npc = view_as<Test>(CClotBody(vecPos, vecAng, "models/player/soldier.mdl", "1.0", "700", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(Test_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(Test_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(Test_ClotThink);
...
public void Test_ClotThink(int iNPC)
{
	Test npc = view_as<Test>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		TestSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
		npc.m_flGetClosestTargetTime = 0.0;
...
public Action Test_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Test npc = view_as<Test>(victim);
...
public void Test_NPCDeath(int entity)
{
	Test npc = view_as<Test>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void TestSelfDefense(Test npc, float gameTime, int target, float distance)
{
			...
			if(IsValidEnemy(npc.index, Enemy_I_See))
			{
				...
				if(npc.DoSwingTrace(swingTrace, target, { 9999.0, 9999.0, 9999.0 }))
				{
					...
					if(ShouldNpcDealBonusDamage(target))
						damage *= 10.0;
					int RocketGet = npc.FireRocket(vecTarget, damage, speed);
