...
void ApertureHuntsman_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Aperture Huntsman");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_aperture_huntsman");
...
{
	return ApertureHuntsman(vecPos, vecAng, team);
}
methodmap ApertureHuntsman < CClotBody
{	
	...
	public ApertureHuntsman(float vecPos[3], float vecAng[3], int ally)
	{
		ApertureHuntsman npc = view_as<ApertureHuntsman>(CClotBody(vecPos, vecAng, "models/player/sniper.mdl", "1.0", "700", ally));
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/sniper/spr17_hawk_eyed_hunter/spr17_hawk_eyed_hunter.mdl");
		...
		func_NPCDeath[npc.index] = ApertureHuntsman_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = ApertureHuntsman_OnTakeDamage;
		func_NPCThink[npc.index] = ApertureHuntsman_ClotThink;
...
public void ApertureHuntsman_ClotThink(int iNPC)
{
	ApertureHuntsman npc = view_as<ApertureHuntsman>(iNPC);
...
public Action ApertureHuntsman_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	ApertureHuntsman npc = view_as<ApertureHuntsman>(victim);
...
public void ApertureHuntsman_NPCDeath(int entity)
{
	ApertureHuntsman npc = view_as<ApertureHuntsman>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
public void ApertureHuntsman_Particle_StartTouch(int entity, int target)
{
