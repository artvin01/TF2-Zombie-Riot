...
void Aperture_collector_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Aperture Supppresor");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_suppressor");
...
{
	return Aperture_collector(vecPos, vecAng, team);
}

methodmap Aperture_collector < CClotBody
{
	...
	public Aperture_collector(float vecPos[3], float vecAng[3], int ally)
	{
		Aperture_collector npc = view_as<Aperture_collector>(CClotBody(vecPos, vecAng, "models/bots/heavy/bot_heavy.mdl", "1.2", "10000", ally, false, true));
		...
		func_NPCDeath[npc.index] = Aperture_collector_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = Aperture_collector_OnTakeDamage;
		func_NPCThink[npc.index] = Aperture_collector_ClotThink;
		func_NPCDeathForward[npc.index] = Aperture_collector_AllyDeath;
...
public void Aperture_collector_ClotThink(int iNPC)
{

	Aperture_collector npc = view_as<Aperture_collector>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		Aperture_collectorSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
...
public Action Aperture_collector_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Aperture_collector npc = view_as<Aperture_collector>(victim);
...
public void Aperture_collector_NPCDeath(int entity)
{
	Aperture_collector npc = view_as<Aperture_collector>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void Aperture_collectorSelfDefense(Aperture_collector npc, float gameTime, int target, float distance)
{
...
public void Aperture_collector_AllyDeath(int self, int ally)
{
	Aperture_collector npc = view_as<Aperture_collector>(self);
