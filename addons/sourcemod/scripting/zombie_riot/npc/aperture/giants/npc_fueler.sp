...
void Aperture_Fueler_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Fueler");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_fueler");
...
{
	return Aperture_Fueler(vecPos, vecAng, ally);
}
methodmap Aperture_Fueler < CClotBody
{
	...
	public Aperture_Fueler(float vecPos[3], float vecAng[3], int ally)
	{
		Aperture_Fueler npc = view_as<Aperture_Fueler>(CClotBody(vecPos, vecAng, "models/bots/pyro_boss/bot_pyro_boss.mdl", "1.35", "20000", ally));
		...
		func_NPCDeath[npc.index] = view_as<Function>(Aperture_Fueler_NPCDeath);
		func_NPCOnTakeDamage[npc.index] = view_as<Function>(Aperture_Fueler_OnTakeDamage);
		func_NPCThink[npc.index] = view_as<Function>(Aperture_Fueler_ClotThink);
...
public void Aperture_Fueler_ClotThink(int iNPC)
{
	Aperture_Fueler npc = view_as<Aperture_Fueler>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		Aperture_FuelerSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
		npc.m_flGetClosestTargetTime = 0.0;
...
public Action Aperture_Fueler_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Aperture_Fueler npc = view_as<Aperture_Fueler>(victim);
...
public void Aperture_Fueler_NPCDeath(int entity)
{
	Aperture_Fueler npc = view_as<Aperture_Fueler>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void Aperture_FuelerSelfDefense(Aperture_Fueler npc, float gameTime, int target, float distance)
{
		...
		if(npc.m_flAttackHappens < gameTime)
		{
			...
			if(npc.DoSwingTrace(swingTrace, npc.m_iTarget))
			{
				...
				if(IsValidEnemy(npc.index, target))
				{
					...
					Explode_Logic_Custom(50.0, -1, npc.index, -1, vecMe, 150.0, _, 0.75, true, _, false, _, Aperture_Fueler_ExplodePost);
...
public void Aperture_Fueler_ExplodePost(int attacker, int victim, float damage, int weapon)
{
