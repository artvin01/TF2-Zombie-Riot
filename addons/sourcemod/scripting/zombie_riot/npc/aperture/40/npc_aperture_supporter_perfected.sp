...
void ApertureSupporterPerfected_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Perfected Aperture Supporter");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_aperture_supporter_perfected");
...
{
	return ApertureSupporterPerfected(vecPos, vecAng, ally);
}
methodmap ApertureSupporterPerfected < CClotBody
{
	...
	public ApertureSupporterPerfected(float vecPos[3], float vecAng[3], int ally)
	{
		ApertureSupporterPerfected npc = view_as<ApertureSupporterPerfected>(CClotBody(vecPos, vecAng, "models/player/medic.mdl", "1.0", "30000", ally));
		...
		func_NPCDeath[npc.index] = ApertureSupporterPerfected_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = ApertureSupporterPerfected_OnTakeDamage;
		func_NPCThink[npc.index] = ApertureSupporterPerfected_ClotThink;
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/medic/hwn2022_lavish_labwear/hwn2022_lavish_labwear.mdl");
...
//TODO 
//Rewrite
public void ApertureSupporterPerfected_ClotThink(int iNPC)
{
	ApertureSupporterPerfected npc = view_as<ApertureSupporterPerfected>(iNPC);
	...
	if(!npc.m_bFUCKYOU)
	{
		...
		if(IsValidAlly(npc.index, PrimaryThreatIndex))
		{
			...
			if(flDistanceToTarget < 250000)
			{
				...
				{
					npc.StartHealing();
					npc.m_iWearable4 = ConnectWithBeam(npc.m_iWearable3, PrimaryThreatIndex, 255, 255, 100, 3.0, 3.0, 1.35, LASERBEAM);
					npc.Healing = true;
				...
				if(npc.m_flNextRangedAttack < GetGameTime(npc.index))
				{
					...
					ExpidonsaGroupHeal(npc.index, 600.0, 99, 7500.0, 15.0, false,Expidonsa_DontHealSameIndex,Aperture_GrantGrantness);
				}
...
public Action ApertureSupporterPerfected_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &m_iWearable3, float damageForce[3], float damagePosition[3], int damagecustom)
{
	ApertureSupporterPerfected npc = view_as<ApertureSupporterPerfected>(victim);
...
public void ApertureSupporterPerfected_NPCDeath(int entity)
{
	ApertureSupporterPerfected npc = view_as<ApertureSupporterPerfected>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void Aperture_GrantGrantness(int entity, int victim)
{
	int flMaxHealth = ReturnEntityMaxHealth(victim);
	ApplyStatusEffect(entity, victim, "Healing Adaptiveness All",	10.0);
	ApplyStatusEffect(entity, victim, "War Cry",		  	10.0);	
	ApplyStatusEffect(entity, victim, "Defensive Backup", 	10.0);	
	ApplyStatusEffect(entity, victim, "Ancient Melodies", 	10.0);
