...
void MedivalScout_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Medieval Scout");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_medival_scout");
...
methodmap MedivalScout < CClotBody
{
	...
	public MedivalScout(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = MedivalScout_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = MedivalScout_OnTakeDamage;
		func_NPCThink[npc.index] = MedivalScout_ClotThink;
...
public Action MedivalScout_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
