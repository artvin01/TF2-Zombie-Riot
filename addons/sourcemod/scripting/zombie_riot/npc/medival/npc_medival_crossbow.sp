...
void MedivalCrossbowMan_OnMapStart_NPC()
{
...
{
	return MedivalCrossbowMan(vecPos, vecAng, team);
}
methodmap MedivalCrossbowMan < CClotBody
{
	...
	public MedivalCrossbowMan(float vecPos[3], float vecAng[3], int ally)
	{
...
public void HandleAnimEventMedival_CrossbowMan(int entity, int event)
{
	if(event == 1001)
	{
...
public Action MedivalCrossbowMan_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	MedivalCrossbowMan npc = view_as<MedivalCrossbowMan>(victim);
...
public void MedivalCrossbowMan_NPCDeath(int entity)
{
	MedivalCrossbowMan npc = view_as<MedivalCrossbowMan>(entity);
	if(!npc.m_bGib)
	{
