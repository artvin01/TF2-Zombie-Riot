...
methodmap MedivalPaladin < CClotBody
{
	...
	public MedivalPaladin(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		...
		SDKHook(npc.index, SDKHook_OnTakeDamage, MedivalPaladin_ClotDamaged);
		...
		npc.m_iWearable5 = npc.EquipItem("weapon_bone", "models/workshop/player/items/soldier/hw2013_rocket_ranger/hw2013_rocket_ranger.mdl");
...
public Action MedivalPaladin_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	MedivalPaladin npc = view_as<MedivalPaladin>(victim);
...
void MedivalPaladin_NPCDeath(int entity)
{
	MedivalPaladin npc = view_as<MedivalPaladin>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
	
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, MedivalPaladin_ClotDamaged);
