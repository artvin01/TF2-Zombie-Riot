...
void MedivalLongbowmen_OnMapStart_NPC()
{
...
methodmap MedivalLongbowmen < CClotBody
{
	...
	public MedivalLongbowmen(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		...
		SDKHook(npc.index, SDKHook_OnTakeDamage, MedivalLongbowmen_ClotDamaged);
...
public void HandleAnimEventMedivalLongbowmen(int entity, int event)
{
	if(event == 1001)
	{
		MedivalLongbowmen npc = view_as<MedivalLongbowmen>(entity);
...
public Action MedivalLongbowmen_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	MedivalLongbowmen npc = view_as<MedivalLongbowmen>(victim);
...
public void MedivalLongbowmen_NPCDeath(int entity)
{
	MedivalLongbowmen npc = view_as<MedivalLongbowmen>(entity);
	if(!npc.m_bGib)
	{
	...
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, MedivalLongbowmen_ClotDamaged);
