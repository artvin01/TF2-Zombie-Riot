...
public void MedivalCavalary_OnMapStart_NPC()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Cavalary");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_medival_cavalary");
...
methodmap MedivalCavalary < CClotBody
{
	...
	public MedivalCavalary(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = MedivalCavalary_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = MedivalCavalary_OnTakeDamage;
		func_NPCThink[npc.index] = MedivalCavalary_ClotThink;
...
public Action MedivalCavalary_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	MedivalCavalary npc = view_as<MedivalCavalary>(victim);
...
void MedivalCavalary_NPCDeath(int entity)
{
	MedivalCavalary npc = view_as<MedivalCavalary>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
