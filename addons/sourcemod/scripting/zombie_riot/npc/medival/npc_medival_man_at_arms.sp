...
void MedivalManAtArms_OnMapStart_NPC()
{
	...
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_medival_man_at_arms");
...
methodmap MedivalManAtArms < CClotBody
{
	...
	public MedivalManAtArms(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCThink[npc.index] = MedivalManAtArms_ClotThink;
		npc.m_iState = 0;
		npc.m_flSpeed = 240.0;
...
public Action MedivalManAtArms_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
