...
void MedivalHalb_OnMapStart_NPC()
{
...
methodmap MedivalHalb < CClotBody
{
	...
	public MedivalHalb(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		...
		SDKHook(npc.index, SDKHook_OnTakeDamage, MedivalHalb_ClotDamaged);
		SDKHook(npc.index, SDKHook_Think, MedivalHalb_ClotThink);
...
//		SetEntityRenderColor(npc.index, 200, 255, 200, 255);
		...
		npc.m_iWearable2 = npc.EquipItem("weapon_bone", "models/workshop/player/items/soldier/hwn2015_hellmet/hwn2015_hellmet.mdl");
						...
						if(npc.DoSwingTrace(swingTrace, PrimaryThreatIndex))
							{
								...
								if(target > 0) 
								{
									...
									{
										SDKHooks_TakeDamage(target, npc.index, npc.index, damage * 3.5, DMG_CLUB, -1, _, vecHit);
									}
									else
									{
...
public Action MedivalHalb_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;
		
	MedivalHalb npc = view_as<MedivalHalb>(victim);
...
public void MedivalHalb_NPCDeath(int entity)
{
	MedivalHalb npc = view_as<MedivalHalb>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
	
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, MedivalHalb_ClotDamaged);
	SDKUnhook(npc.index, SDKHook_Think, MedivalHalb_ClotThink);
