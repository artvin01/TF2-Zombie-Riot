...
void MedivalArbalest_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Arbalest");
...
public void HandleAnimEventMedival_Arbalest(int entity, int event)
{
	if(event == 1001)
	{
		MedivalArbalest npc = view_as<MedivalArbalest>(entity);
...
public Action MedivalArbalest_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	MedivalArbalest npc = view_as<MedivalArbalest>(victim);
...
public void MedivalArbalest_NPCDeath(int entity)
{
	MedivalArbalest npc = view_as<MedivalArbalest>(entity);
	if(!npc.m_bGib)
	{
