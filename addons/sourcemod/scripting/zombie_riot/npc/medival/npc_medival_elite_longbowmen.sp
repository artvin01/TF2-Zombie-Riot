...
void MedivalEliteLongbowmen_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Medieval Elite Longbowmen");
...
public void HandleAnimEventMedivalEliteLongbowmen(int entity, int event)
{
	if(event == 1001)
	{
		MedivalEliteLongbowmen npc = view_as<MedivalEliteLongbowmen>(entity);
...
public Action MedivalEliteLongbowmen_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	MedivalEliteLongbowmen npc = view_as<MedivalEliteLongbowmen>(victim);
...
public void MedivalEliteLongbowmen_NPCDeath(int entity)
{
	MedivalEliteLongbowmen npc = view_as<MedivalEliteLongbowmen>(entity);
	if(!npc.m_bGib)
	{
