...
void MedivalEagleScout_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Eagle Scout");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_medival_eagle_scout");
...
{
	return MedivalEagleScout(vecPos, vecAng, team);
}
...
methodmap MedivalEagleScout < CClotBody
{
	...
	public MedivalEagleScout(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = MedivalEagleScout_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = MedivalEagleScout_OnTakeDamage;
		func_NPCThink[npc.index] = MedivalEagleScout_ClotThink;
