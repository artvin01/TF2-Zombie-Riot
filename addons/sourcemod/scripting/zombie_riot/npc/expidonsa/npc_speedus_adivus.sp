...
void SpeedusAdivus_OnMapStart_NPC()
{
...
methodmap SpeedusAdivus < CClotBody
{
	...
	public SpeedusAdivus(float vecPos[3], float vecAng[3], int ally)
	{
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/scout/bonk_mask/bonk_mask.mdl");
		SetVariantString("1.0");
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/workshop/player/items/all_class/spr17_wingman/spr17_wingman_scout.mdl");
		SetVariantString("1.0");
...
public Action SpeedusAdivus_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	SpeedusAdivus npc = view_as<SpeedusAdivus>(victim);
...
public void SpeedusAdivus_NPCDeath(int entity)
{
	SpeedusAdivus npc = view_as<SpeedusAdivus>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
