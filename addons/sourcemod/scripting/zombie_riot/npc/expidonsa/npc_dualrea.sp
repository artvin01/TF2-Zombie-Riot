...
public void DualRea_ClotThink(int iNPC)
{
...
public Action DualRea_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	DualRea npc = view_as<DualRea>(victim);
...
public void DualRea_NPCDeath(int entity)
{
	DualRea npc = view_as<DualRea>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void DualReaSelfDefense(DualRea npc, float gameTime, int target, float distance)
{
