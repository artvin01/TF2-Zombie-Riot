...
methodmap Helena < CClotBody
{
	public void PlayIdleAlertSound() {
		...
		EmitSoundToAll(g_DefaultMedic_IdleAlertedSounds[GetRandomInt(0, sizeof(g_DefaultMedic_IdleAlertedSounds) - 1)], this.index, SNDCHAN_VOICE, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME);
	...
	public Helena(float vecPos[3], float vecAng[3], int ally)
	{
		...
		npc.m_iWearable1 = npc.EquipItem("head", "models/player/items/all_class/all_halo.mdl");
		...
		npc.m_iWearable5 = npc.EquipItem("head", "models/workshop/player/items/sniper/fall2013_kyoto_rider/fall2013_kyoto_rider.mdl");
		SetVariantString("1.0");
...
public Action Helena_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &m_iWearable3, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Helena npc = view_as<Helena>(victim);
...
public void Helena_NPCDeath(int entity)
{
	Helena npc = view_as<Helena>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
