...
void GiantTankus_OnMapStart_NPC()
{
...
methodmap GiantTankus < CClotBody
{
	...
	public GiantTankus(float vecPos[3], float vecAng[3], int ally)
	{
		...
		npc.m_iWearable2 = npc.EquipItem("head", "models/workshop/player/items/all_class/bak_teufort_knight/bak_teufort_knight_heavy.mdl");
		SetVariantString("1.0");
...
public void GiantTankus_ClotThink(int iNPC)
{
	...
	if(IsValidEnemy(npc.index, PrimaryThreatIndex))
	{
		...
		GiantTankusSelfdefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
		npc.StartPathing();
	}
	else
	{
...
void GiantTankusSelfdefense(GiantTankus npc, float gameTime, int target, float distance)
{
...
public Action GiantTankus_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	GiantTankus npc = view_as<GiantTankus>(victim);
...
public void GiantTankus_NPCDeath(int entity)
{
	GiantTankus npc = view_as<GiantTankus>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
