...
{
	return Siccerino(vecPos, vecAng, team);
}
methodmap Siccerino < CClotBody
{
	...
	public Siccerino(float vecPos[3], float vecAng[3], int ally)
	{
...
public void Siccerino_ClotThink(int iNPC)
{
	Siccerino npc = view_as<Siccerino>(iNPC);
	...
	if(IsValidEnemy(npc.index, npc.m_iTarget))
	{
		...
		SiccerinoSelfDefense(npc,GetGameTime(npc.index), npc.m_iTarget, flDistanceToTarget); 
	}
	else
	{
...
public Action Siccerino_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	Siccerino npc = view_as<Siccerino>(victim);
...
public void Siccerino_NPCDeath(int entity)
{
	Siccerino npc = view_as<Siccerino>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
...
void SiccerinoSelfDefense(Siccerino npc, float gameTime, int target, float distance)
{
