			...
			else
			{
				...
				if(Armor_Charge[victim] < (-Elemental_TriggerDamage(victim, Element_Plasma)))
				{
					...
					if(HasSpecificBuff(victim, "Plasmatic Rampage")) // lastman for plasmic surprise
					{
						...
						PlasmicElemental_HealNearby(attacker, float(ReturnEntityMaxHealth(victim)), position, 200.0, 2.0, 0, 3);
						float HudY = -1.0;
						float HudX = -1.0;
					...
					else
					{
						...
						if(theteam == 2)
							...
							theteam = 2;
						PlasmicElemental_HealNearby(attacker, float(ReturnEntityMaxHealth(victim)), position, 200.0, 2.0, 2, theteam);
					}
