...
public void Event_GB_OnHatTouch(int entity, int target)
{
	...
	if (target > 0)	
	{
		...
		SDKHooks_TakeDamage(target, owner, owner, f_WandDamage[entity] * Target_Sucked_Long_Return(target), DMG_PLASMA, weapon, CalculateDamageForceOld(vecForward, 10000.0), Entity_Position);	// 2048 is DMG_NOGIB?
