		...
		if(Ability_Check_Cooldown(client, slot) < 0.0)
		{
			...
			switch(GetRandomInt(1,8))
			{
				case 1:
				{
					...
					Address address = TF2Attrib_GetByDefIndex(weapon, 394);
					if(address != Address_Null)
					fl_IncreaseAttackSpeed[client] = TF2Attrib_GetValue(address);
					TF2Attrib_SetByDefIndex(weapon, 394, fl_IncreaseAttackSpeed[client] * IncreaseAttackSpeed);
...
public Action LessAttackSpeed(Handle cut_timer, int client)
{
	if(IsValidClient(client))
	{
		...
		if(address != Address_Null)
		fl_IncreaseAttackSpeed[client] = TF2Attrib_GetValue(address);
		TF2Attrib_SetByDefIndex(weapon, 394, fl_IncreaseAttackSpeed[client] * DecreaseAttackSpeed_Pap);
...
public Action MoreAttackSpeed(Handle cut_timer, int client)
{
	if(IsValidClient(client))
	{
		...
		if(address != Address_Null)
		fl_IncreaseAttackSpeed[client] = TF2Attrib_GetValue(address);
		TF2Attrib_SetByDefIndex(weapon, 394, fl_IncreaseAttackSpeed[client] * IncreaseAttackSpeed_Pap);
...
public Action Reset_ToNormalAttackSpeed(Handle cut_timer, int client)
{
	if(IsValidClient(client))
	{
		...
		{
			TF2Attrib_SetByDefIndex(weapon, 394, fl_IncreaseAttackSpeed[client]);
		}
