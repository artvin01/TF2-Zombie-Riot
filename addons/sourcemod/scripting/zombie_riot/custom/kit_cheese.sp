...
This kit introduces the Cheesy Elemental status. Yes, cheese as an elemental status.
Upon reaching 100%, the target recieves massive damage based on the attacker's 
pap level and applier weapon, and is debuffed with Cheesed II for 8s (bosses for 4s, raids get I for 4s instead).
Cheesy Elemental buildup is reduced by 50% on bosses and raids 
...
but has a much better health on kill stat and builds the Cheesy Elemental status much faster.
1st pap allows the user to heal off Cheesed targets with the melee, healing grows stronger with more paps.
Lethal Injection (M2 Melee Ability) temporarily increases attackspeed, 
damage and GREATLY increases melee Cheesy Elemental status buildup.
Later paps buff this ability to last a bit longer and to inflict true dmg bleed.
Mocha Inoculation (R Melee Ability)  greatly increases a random stat of the melee temporarily
(Damage, Resistance, cheese buildup or attackspeed) and slowly heals the user overtime.
Later paps buff this ability's buffs and healing to last longer and be a bit stronger.

Primary - Shoots "cheesy balls" in quick succession, like the clockwork assault rifle from Terraria.
These projectiles build the Cheesy Elemental status much slower than the melee.
Cheddar Burst (M2 Primary Ability) - Shoots a short-ranged laser similar to the Laserstick, minus the knockback.
This laser inflicts Cheesed I temporarily to enemies hit by it.
Later paps buff this ability to have a slightly larger range, less cooldown,
and allow it to inflict Cheesed II instead of I for an overall longer duration.
...
void Cheese_BeamEffect(float position[3], float startrad = 1.0, float endrad = 125.0, float lifetime = 0.25, float width = 6.5, bool elemental = false, int client = -1)
{
    if(elemental)
    {
        TE_SetupBeamRingPoint(position, startrad, endrad, LaserIndex, LaserIndex, 0, 1, lifetime, width, 0.0, { 255, 120, 0, 65 }, 1, 0);
	    TE_SendToClient(client);
    }
    else
    {
        TE_SetupBeamRingPoint(position, startrad, endrad, LaserIndex, LaserIndex, 0, 1, lifetime, width, 0.0, { 255, 130, 0, 255 }, 1, 0);
	    TE_SendToAll();
    }
}
...
public float Cheese_OnTakeDamage_Melee(int attacker, int victim, float &damage, int damagetype, int weapon)
{
	...
	{   
        ...
        {
            if(HasSpecificBuff(victim, "Cheesed I") || HasSpecificBuff(victim, "Cheesed II"))
            {
        ...
        Elemental_AddCheeseDamage(victim, attacker, RoundToNearest(cheesedmg * 1.5), weapon);
    }
...
{
    Elemental_AddCheeseDamage(victim, attacker, RoundToNearest(damage * 0.225), weapon);
}
...
public void Weapon_Kit_CheeseInject_R(int client, int weapon, bool &result, int slot)
{
	if(weapon >= MaxClients)
	{
		if (Ability_Check_Cooldown(client, slot) < 0.0 && Cheese_PapLevel[client] >= 3)
		{
            ...
            SetDefaultHudPosition(client, 255, 130, 0, 3.0);
...
static void TBB_Tick(int client)
{
	...
	if (TR_DidHit(trace))
	{
		...
		for (int building = 0; building < MAX_TARGETS_HIT; building++)
		{
			if (Cheese_BuildingHit[building])
			{
				if(IsValidEntity(Cheese_BuildingHit[building]))
				{
                    ...
                    if(Cheese_PapLevel[client] >= 5)
                        ApplyStatusEffect(client, Cheese_BuildingHit[building], "Cheesed II", duration);
                    else
                        ApplyStatusEffect(client, Cheese_BuildingHit[building], "Cheesed I", duration);
		...
		int colorLayer3[4];
		SetColorRGBA(colorLayer3, colorLayer4[0] * 7 + 255 / 8, colorLayer4[1] * 7 + 135 / 8, colorLayer4[2] * 7 + 30 / 8, 255);
		int colorLayer2[4];
		SetColorRGBA(colorLayer2, colorLayer4[0] * 6 + 510 / 8, colorLayer4[1] * 6 + 270 / 8, colorLayer4[2] * 6 + 60 / 8, 255);
		int colorLayer1[4];
		SetColorRGBA(colorLayer1, colorLayer4[0] * 5 + 765 / 8, colorLayer4[1] * 5 + 405 / 8, colorLayer4[2] * 5 + 90 / 8, 255);
