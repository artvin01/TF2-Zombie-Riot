...
public void Weapon_Lightning_Wand(int client, int weapon, bool crit)
{
	...
	if(mana_cost <= Current_Mana[client])
	{
		...
		EmitSoundToAll(SOUND_WAND_SHOT_LIGHTNING, client, SNDCHAN_WEAPON, 65, _, 0.45, 100);
...
static void Wand_Launch(int client, int iRot, float speed, float time, float damage, int weapon)
{
	...
	{
		case 2:
			particle = ParticleEffectAt(position, "unusual_zap_yellow", 5.0);

		default:
			particle = ParticleEffectAt(position, "unusual_zap_yellow", 5.0);
	}
	//eyeboss_projectile is really cool
	...
	SDKHook(iCarrier, SDKHook_StartTouch, Event_Wand_Lightning_OnHatTouch);
}

public Action Event_Wand_Lightning_OnHatTouch(int entity, int other)
{
