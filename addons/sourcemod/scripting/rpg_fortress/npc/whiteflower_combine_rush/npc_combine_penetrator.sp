...
void OnMapStartCombinePenetrator()
{
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "W.F. Penetrator");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_whiteflower_penetrator");
...
methodmap CombinePenetrator < CombineSoldier
{
	public CombinePenetrator(int client, float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = CombinePenetrator_NPCDeath;
...
public void CombinePenetrator_ClotThink(int iNPC)
{
	CombinePenetrator npc = view_as<CombinePenetrator>(iNPC);
					...
					if(npc.m_iAttacksTillReload > 0)
					{
						...
						if(BaseSquad_InFireRange(vecDirShooting[1], eyePitch[1]))
						{
							...
							for(int i; i < 5; i++)
							{
								...
								FireBullet(npc.index, npc.m_iWearable1, vecMe, vecDir, 200000.0, 9000.0, DMG_BULLET, "bullet_tracer01_red");
							}
...
void CombinePenetrator_NPCDeath(int entity)
{
	CombinePenetrator npc = view_as<CombinePenetrator>(entity);
