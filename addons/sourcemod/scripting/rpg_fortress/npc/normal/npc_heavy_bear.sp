...
public void HeavyBear_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Heavy Bear");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_heavy_bear");
...
{
	return HeavyBear(vecPos, vecAng, team);
}
...
methodmap HeavyBear < CClotBody
{
	...
	public HeavyBear(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = HeavyBear_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = HeavyBear_OnTakeDamage;
		func_NPCThink[npc.index] = HeavyBear_ClotThink;
