...
public void ScoutHyper_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Scout Hyper Zombie");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_scout_hyper");
...
methodmap ScoutHyper < CClotBody
{
	...
	public void PlayMeleeSound() {
		EmitSoundToAll(g_MeleeAttackSounds[GetRandomInt(0, sizeof(g_MeleeAttackSounds) - 1)], this.index, SNDCHAN_STATIC, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME, 120);
	...
	public void PlayMeleeMissSound() {
		EmitSoundToAll(g_MeleeMissSounds[GetRandomInt(0, sizeof(g_MeleeMissSounds) - 1)], this.index, SNDCHAN_STATIC, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME, 120);
	...
	public ScoutHyper(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = ScoutHyper_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = ScoutHyper_OnTakeDamage;
		func_NPCThink[npc.index] = ScoutHyper_ClotThink;
		...
		switch(npc.m_iState)
		{
			...
			{			
				...
				if(IsValidEntity(Enemy_I_See) && IsValidEnemy(npc.index, Enemy_I_See))
				{
					...
					npc.AddGesture("ACT_MP_ATTACK_STAND_MELEE",_,_,_,0.8);
...
public Action ScoutHyper_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;

	ScoutHyper npc = view_as<ScoutHyper>(victim);
...
public void ScoutHyper_NPCDeath(int entity)
{
	ScoutHyper npc = view_as<ScoutHyper>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();
	}
