...
public void SniperAccuracy_OnMapStart_NPC()
{
...
methodmap SniperAccuracy < CClotBody
{
	...
	public SniperAccuracy(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = SniperAccuracy_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = SniperAccuracy_OnTakeDamage;
		func_NPCThink[npc.index] = SniperAccuracy_ClotThink;
...
public Action SniperAccuracy_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;

	SniperAccuracy npc = view_as<SniperAccuracy>(victim);
...
public void SniperAccuracy_NPCDeath(int entity)
{
	SniperAccuracy npc = view_as<SniperAccuracy>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();
	}
