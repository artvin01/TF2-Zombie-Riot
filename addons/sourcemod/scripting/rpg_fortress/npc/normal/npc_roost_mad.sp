...
public void MadRoost_OnMapStart_NPC()
{
...
methodmap MadRoost < CClotBody
{
	public void PlayIdleSound()
	{
		...
		EmitSoundToAll(g_IdleSound[GetRandomInt(0, sizeof(g_IdleSound) - 1)], this.index, SNDCHAN_VOICE, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME,GetRandomInt(100, 115));
	...
	public void PlayHurtSound()
	{
		...
		EmitSoundToAll(g_HurtSound[GetRandomInt(0, sizeof(g_HurtSound) - 1)], this.index, SNDCHAN_VOICE, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME,GetRandomInt(100, 115));
	}
	
	public void PlayDeathSound() 
	{
		EmitSoundToAll(g_DeathSounds[GetRandomInt(0, sizeof(g_DeathSounds) - 1)], this.index, SNDCHAN_VOICE, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME,GetRandomInt(100, 115));
	}
	public void PlayKilledEnemySound() 
	{
		EmitSoundToAll(g_KilledEnemySound[GetRandomInt(0, sizeof(g_KilledEnemySound) - 1)], this.index, SNDCHAN_VOICE, NORMAL_ZOMBIE_SOUNDLEVEL, _, NORMAL_ZOMBIE_VOLUME,GetRandomInt(100, 115));
	...
	public MadRoost(float vecPos[3], float vecAng[3], int ally)
	{
		...
		SDKHook(npc.index, SDKHook_OnTakeDamage, MadRoost_OnTakeDamage);
		SDKHook(npc.index, SDKHook_Think, MadRoost_ClotThink);
		...
		npc.m_iWearable4 = npc.EquipItem("head","models/workshop/weapons/c_models/c_holymackerel/c_holymackerel.mdl");
		SetVariantString("1.0");
...
public Action MadRoost_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;

	MadRoost npc = view_as<MadRoost>(victim);
...
public void MadRoost_NPCDeath(int entity)
{
	MadRoost npc = view_as<MadRoost>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();
	}
	SDKUnhook(entity, SDKHook_OnTakeDamage, MadRoost_OnTakeDamage);
	SDKUnhook(entity, SDKHook_Think, MadRoost_ClotThink);
