...
public void CaveBowmen_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Cave Bowmen");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_cave_bowmen");
...
methodmap CaveBowmen < CClotBody
{
	...
	public CaveBowmen(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = CaveBowmen_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = CaveBowmen_OnTakeDamage;
		func_NPCThink[npc.index] = CaveBowmen_ClotThink;
		
		int skin = GetRandomInt(0, 1);
		SetEntProp(npc.index, Prop_Send, "m_nSkin", skin);
	
		npc.m_iWearable1 = npc.EquipItem("head", "models/workshop_partner/weapons/c_models/c_bow_thief/c_bow_thief.mdl");
...
public void CaveBowmen_ClotThink(int iNPC)
{
	...
	Npc_Base_Thinking(iNPC, 500.0, "ACT_MP_RUN_ITEM2", "ACT_MP_STAND_ITEM2", 260.0, gameTime);
...
public Action CaveBowmen_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;

	CaveBowmen npc = view_as<CaveBowmen>(victim);
...
public void CaveBowmen_NPCDeath(int entity)
{
	CaveBowmen npc = view_as<CaveBowmen>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();
	}
