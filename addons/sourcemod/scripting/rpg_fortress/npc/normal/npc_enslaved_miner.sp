...
public void EnslavedMiner_OnMapStart_NPC()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Enslaved Miner");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_enslaved_miner");
...
methodmap EnslavedMiner < CClotBody
{
	...
	public EnslavedMiner(float vecPos[3], float vecAng[3], int ally)
	{
		...
		func_NPCDeath[npc.index] = EnslavedMiner_NPCDeath;
		func_NPCOnTakeDamage[npc.index] = EnslavedMiner_OnTakeDamage;
		func_NPCThink[npc.index] = EnslavedMiner_ClotThink;
...
public Action EnslavedMiner_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	//Valid attackers only.
	if(attacker <= 0)
		return Plugin_Continue;

	EnslavedMiner npc = view_as<EnslavedMiner>(victim);
...
public void EnslavedMiner_NPCDeath(int entity)
{
	EnslavedMiner npc = view_as<EnslavedMiner>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();
	}
