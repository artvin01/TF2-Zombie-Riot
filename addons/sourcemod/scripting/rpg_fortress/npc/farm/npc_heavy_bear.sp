...
public void FarmBear_OnMapStart_NPC()
{
...
public Action FarmBear_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	return Plugin_Handled; //IMMORTALITY!!!!!
}

public void FarmBear_NPCDeath(int entity)
{
...
bool HeavyBear_Interact(int client, int weapon)
{
	bool result;
	...
	if(kv)
	{
		if(kv.GetNum("farm_interact", 0) == 2) //This item can be used to interact with farm animals., it also specificies the type, cow is 1.
		{
			result = true;
