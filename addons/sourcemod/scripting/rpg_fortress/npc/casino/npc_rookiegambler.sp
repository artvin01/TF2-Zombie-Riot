...
methodmap RookieGambler < CClotBody
{
	...
	public RookieGambler(int client, float vecPos[3], float vecAng[3], int ally)
	{
		RookieGambler npc = view_as<RookieGambler>(CClotBody(vecPos, vecAng, "models/player/spy.mdl", "1.0", "300", ally, false));
...
static void ClotThink(int iNPC)
{
	...
	Npc_Base_Thinking(npc.index, 800.0, "ACT_MP_RUN_MELEE", "ACT_MP_STAND_MELEE", 250.0, gameTime);
...
static void ClotTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	if(attacker > 0)
	{
		RookieGambler npc = view_as<RookieGambler>(victim);
