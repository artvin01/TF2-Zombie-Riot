...
static void ClotThink(int iNPC)
{
			...
			if(IsValidEnemy(npc.index, target))
			{
				...
				if(npc.DoSwingTrace(swingTrace, target))
				{
					...
					if(target > 0) 
					{
						...
						SDKHooks_TakeDamage(target, npc.index, npc.index, CasinoShared_GetDamage(npc, 1.35), DMG_CLUB, _, _, vecHit);
					}
				}
				delete swingTrace;
			}
	...
	if(target > 0)
	{
		...
		if(distance < (NORMAL_ENEMY_MELEE_RANGE_FLOAT_SQUARED * 3) && npc.m_flNextMeleeAttack < gameTime)
		{
